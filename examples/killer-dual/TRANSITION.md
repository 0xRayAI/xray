...
| 2 | Live `preCompact` | `cursor-precompact.json` `event_class=cursor-host-precompact`; probe `event=preCompact`; Station `Compact:` / `Usage:` | Stock heat (Intent/Plan/Git/Working) | Treating a chat “context constraints” summary as host fire (this run: a later summary had **no** third probe line) |
...
Arm S vs Arm B harness compare stays in `COMPARE.md` (different axis: suited vs stripped).

## Next (not another Cursor FILL)

1. This map + eval cases — the collaboration return.
2. Constrained Grok Bot chat-seat pass of cases 3 and 6 only (no fake hooks).
3. Window fields when the host sends them — still no FILL.
