# HOST vs mill — what compact can score

**Chat may lose early turns. Disk must not lose the ticket.**
...
| Can score | Cannot score |
|---|---|
| Cursor summarized the chat and did **not** invent `.xray/state/STATION.md` | “The agent still remembers the pre-compact turns” |
| Planted files kept the same bytes | Recalling a ticket / id / snap-after rule after compact |
| This is still the same cloud, not a new launch | A hidden buffer of earlier tool dumps or file reads |

After compact, three things are re-fed at once:

1. A long session summary (often already contains the ticket, hashes, `bc` id, and snap-after rule)
2. The files on disk (`TICKET.txt`, `PROTOCOL.md`, Station, receipts, …)
3. The host banner that the conversation was summarized

If the successor then names `COMPACT-PLAIN-001` or runs `./snap.sh`, that can come from the summary, from disk, or from this turn’s instructions. Those sources are **not separable**. A “you continued” row that treats same-bc plus still knowing the ticket as **memory** overclaims.
