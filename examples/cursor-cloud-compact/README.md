...
| `../killer-dual/ARM-B-BARE-COMPACT.md` | `cursor-host-precompact` (probe) · Station write FAIL | Arm B bare. Long work thread, not FILL. Host summary + **2×** invoke-probe `event=preCompact`. `STATION.md` / `cursor-precompact.json` still absent: Node adapter cannot load without `dist/nucleus/delegation-gate.js`. |

EVAL: do not upgrade FAIL to `cursor-host-precompact` without `.xray/state/cursor-precompact.json` (or invoke-probe `event=preCompact`) written by the **host**. Arm B is probe-positive. Remaining gap on that run is the Station writer (dist miss), not host emit. A session that boots before `.cursor/hooks.json` exists will not bind project hooks mid-run; boot-cached hooks can still spawn after a mid-run strip.
