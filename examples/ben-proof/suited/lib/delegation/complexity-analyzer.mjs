/**
 * Complexity Analyzer — ported from xray src/delegation/complexity-analyzer.ts
 */
import {
  DEFAULT_THRESHOLDS,
  OPERATION_WEIGHTS,
...
export class ComplexityAnalyzer {
  ...
  constructor() {
    ...
    this.operationWeights = { ...OPERATION_WEIGHTS };
    this.riskMultipliers = { ...RISK_MULTIPLIERS };
  ...
  calculateFileCount(context) {
    const ctx = context;
    return ctx?.files?.length || ctx?.fileCount || 1;
  }

  calculateChangeVolume(context) {
    const ctx = context;
    if (ctx?.linesChanged !== undefined) return ctx.linesChanged;
    if (ctx?.changeVolume !== undefined) return ctx.changeVolume;
  ...
  calculateDependencies(context) {
    const ctx = context;
    if (ctx?.dependencyCount !== undefined) return ctx.dependencyCount;
...
/** Wear pipeline hook: analyze bench task through nucleus-compatible metrics */
export function analyzeWearTaskComplexity(task, ctx = {}) {
  const analyzer = getComplexityAnalyzer();
  const metrics = analyzer.analyzeComplexity(task, {
    fileCount: ctx.fileCount ?? 1,
    dependencyCount: ctx.dependencyCount ?? 0,
    linesChanged: String(task ?? '').length,
    riskLevel: String(task ?? '').toLowerCase().includes('trap') ? 'high' : 'low',
  });
  return analyzer.calculateComplexityScore(metrics);
}
