...
export class ComplexityAnalyzer {
    ...
    calculateFileCount(context) {
        ...
        if (desc.includes("single file") || desc.includes("one file"))
            return 1;
        if (desc.includes("couple files") || desc.includes("few files"))
            return 2;
        if (desc.includes("several files"))
            return 4;
        if (desc.includes("multiple files") || desc.includes("various files"))
            return 5;
        if (desc.includes("many files"))
            return 8;
    ...
    calculateChangeVolume(context) {
        ...
        if (desc.includes("large") || desc.includes("extensive"))
            return 500;
        if (desc.includes("medium") || desc.includes("significant"))
            return 200;
        if (desc.includes("small") || desc.includes("minor"))
            return 50;
        if (desc.includes("tiny") || desc.includes("trivial"))
            return 10;
        // Operation-based defaults
        if (opLower.includes("create"))
            return 100; // Creating new code
        if (opLower.includes("modify"))
            return 75; // Modifying existing
        if (opLower.includes("refactor"))
            return 150; // Refactoring tends to be larger
        if (opLower.includes("analyze"))
            return 25; // Analysis may involve some changes
        if (opLower.includes("debug"))
            return 50; // Debugging fixes
        if (opLower.includes("test"))
            return 80; // Tests can be substantial
        if (opLower.includes("documentation"))
            return 30; // Docs
        return 50; // default
    }
    ...
    calculateDependencies(context) {
        ...
        if (desc.includes("independent"))
            return 0;
        if (desc.includes("depends on"))
            return 3;
        if (desc.includes("multiple dependencies"))
            return 5;
        if (desc.includes("complex dependencies"))
            return 8;
        return 1;
    }
    /**
    ...
    generateReasoning(metrics, score, level) {
        const reasoning = [];
        ...
        if (metrics.operationType === "refactor" ||
            ...
            reasoning.push(`Operation type '${metrics.operationType}' typically requires specialized handling`);
        }
        if (metrics.dependencies > 3) {
            reasoning.push(`Multiple dependencies (${metrics.dependencies}) necessitate coordinated execution`);
        }
        if (metrics.riskLevel === "high" || metrics.riskLevel === "critical") {
            reasoning.push(`Risk level '${metrics.riskLevel}' requires careful orchestration`);
        }
        if (metrics.estimatedDuration > 60) {
            reasoning.push(`Estimated duration (${metrics.estimatedDuration}min) suggests complex operation`);
        }
        reasoning.push(`Overall complexity score: ${Math.round(score)} (${level} level)`);
        return reasoning;
    }
}
