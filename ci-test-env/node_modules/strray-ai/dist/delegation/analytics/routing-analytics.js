...
export class RoutingAnalytics {
    ...
    getDailySummary() {
        ...
        const averageSuccessRate = totalRoutings > 0 ? totalSuccesses / totalRoutings : 0;
        const insights = [];
    ...
    applyRoutingRefinements(apply) {
        ...
        for (const data of stats) {
            if (data.successRate < 0.5 && data.total > 10) {
                changes.push({
                    type: 'removed',
                    reason: `Low success rate (${(data.successRate * 100).toFixed(1)}%) with ${data.total} attempts`,
    ...
    generateInsights() {
        ...
        const topAgent = stats.reduce((prev, current) => prev.total > current.total ? prev : current);
        insights.push(`Most used agent: ${topAgent.agent} (${topAgent.total} routes)`);
        // Find most successful agent
        const bestAgent = stats.reduce((prev, current) => prev.successRate > current.successRate ? prev : current);
        insights.push(`Best performing agent: ${bestAgent.agent} (${(bestAgent.successRate * 100).toFixed(1)}% success rate)`);
        ...
        if (agentsWithData.length > 0) {
            const worstAgent = agentsWithData.reduce((prev, current) => prev.successRate < current.successRate ? prev : current);
            if (worstAgent.successRate < 0.7) {
                insights.push(`Agent needing attention: ${worstAgent.agent} (${(worstAgent.successRate * 100).toFixed(1)}% success rate)`);
            }
        }
        // Overall statistics
    ...
    compareToBaseline() {
        const stats = this.tracker.getStats();
        const currentRate = stats.length > 0
            ? stats.reduce((sum, s) => sum + s.successRate, 0) / stats.length
            : 0;
...
//# sourceMappingURL=routing-analytics.js.map