...
export class KeywordMatcher {
     ...
     * @returns Routing result or null if no match
     */
    matchMultiWord(taskDescription) {
        const descLower = taskDescription.toLowerCase();
     ...
     */
    detectReleaseWorkflow(taskDescription) {
        const descLower = taskDescription.toLowerCase();
        ...
        const isRelease = releaseKeywords.some(keyword => descLower.includes(keyword.toLowerCase()));
        if (!isRelease) {
            return { isRelease: false, bumpType: 'patch', createTag: false };
        }
        // Extract version bump type if specified
        let bumpType = 'patch';
        if (descLower.includes('major'))
            bumpType = 'major';
        else if (descLower.includes('minor'))
            bumpType = 'minor';
        else if (descLower.includes('patch'))
            bumpType = 'patch';
