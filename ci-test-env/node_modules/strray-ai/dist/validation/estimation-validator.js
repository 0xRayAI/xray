...
export class EstimationValidator {
     ...
     */
    startEstimate(taskId, description, category, estimatedMinutes, confidence = 'medium') {
     ...
     */
    getCalibratedEstimate(category, baseEstimate, confidence = 'medium') {
     ...
     */
    validateEstimate(category, estimate) {
        const calibration = this.calibrations.get(category);
        if (!calibration || calibration.sampleSize < 3) {
            return {
                isReasonable: true,
    ...
    getAccuracyReport() {
        ...
        const categoryBreakdown = categories.map(cat => {
            ...
            if (!calibration)
                return null;
            let trend = 'accurate';
            if (calibration.avgRatio > 1.2)
                trend = 'under';
            if (calibration.avgRatio < 0.8)
                trend = 'over';
            return {
                category: cat,
        ...
        if (underEstimators.length > 0) {
            recommendations.push(`You're consistently underestimating: ${underEstimators.map(c => c.category).join(', ')}`);
        }
        const overEstimators = categoryBreakdown.filter(c => c.trend === 'over');
        if (overEstimators.length > 0) {
            recommendations.push(`You're consistently overestimating: ${overEstimators.map(c => c.category).join(', ')}`);
        }
        const lowConfidence = categoryBreakdown.filter(c => c.sampleSize < 3);
        if (lowConfidence.length > 0) {
            recommendations.push(`Need more data for: ${lowConfidence.map(c => c.category).join(', ')}`);
        }
        return { overallAccuracy, categoryBreakdown, recommendations };
    }
    updateCalibration(estimation) {
...
export function getEstimationValidator() {
 ...
 */
export function trackEstimate(taskId, description, category, estimatedMinutes) {
...
//# sourceMappingURL=estimation-validator.js.map