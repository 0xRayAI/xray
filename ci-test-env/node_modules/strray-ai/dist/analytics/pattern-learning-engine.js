...
import { emergingPatternDetector } from "./emerging-pattern-detector.js";
export class PatternLearningEngine {
     ...
     */
    learnFromData(outcomes, existingMappings) {
        const result = {
            newPatterns: [],
            modifiedPatterns: [],
            removedPatterns: [],
        ...
        const agentSkillMetrics = new Map();
        ...
        if (this.config.enableAutoAddition) {
            ...
            result.newPatterns = this.generateNewPatterns(emergentResult.emergentPatterns, existingMappings);
        }
        ...
        frameworkLogger.log("pattern-learning-engine", "learning-complete", "info", {
     ...
     */
    generatePatternModifications(metrics) {
        const updates = [];
     ...
     */
    generatePatternRemovals(metrics, existingMappings) {
        const removals = [];
        for (const [patternId, metric] of metrics.entries()) {
            // Skip if not enough data
            if (metric.totalUsages < 10)
                continue;
     ...
     */
    generateThresholdUpdates(metrics) {
        const updates = [];
     ...
     */
    generateNewPatterns(emergentPatterns, existingMappings) {
        const newPatterns = [];
        for (const emergent of emergentPatterns) {
            // Skip if confidence is too low
            if (emergent.confidence < this.config.minConfidenceForAddition)
                continue;
            // Check if pattern already exists
            const keywords = emergent.pattern.split(' | ');
            const existingMatch = existingMappings.find(m => m.keywords.some(kw => keywords.includes(kw)));
            if (existingMatch)
    ...
    updateConfig(newConfig) {
        ...
        frameworkLogger.log("pattern-learning-engine", "config-updated", "info", { config: this.config }, undefined);
    }
    /**
...
//# sourceMappingURL=pattern-learning-engine.js.map