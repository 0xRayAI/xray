...
class RoutingPerformanceAnalyzer {
    ...
    generatePerformanceReport() {
        ...
        const recommendations = this.generateRecommendations(agentMetrics, keywordEffectiveness, confidenceMetrics);
        return {
     ...
     */
    calculateAgentMetrics(outcomes, promptData) {
        ...
        return Array.from(agentMap.entries())
            ...
            const avgExecutionTime = metrics.executionTimes.length > 0
                ...
                : 0;
            const timestamps = metrics.timestamps.sort((a, b) => a.getTime() - b.getTime());
            return {
                agent,
                skill: metrics.skill,
     ...
     */
    analyzeKeywordEffectiveness(decisions, outcomes) {
     ...
     */
    analyzeConfidenceThresholds(outcomes) {
        const promptData = routingOutcomeTracker.getPromptData();
        ...
        return thresholds.map((threshold) => {
            ...
            const successfulRoutings = belowThresholdOutcomes.filter((o) => o.success === true).length;
            const successRate = totalRoutings > 0 ? successfulRoutings / totalRoutings : 0;
     ...
     */
    calculateOverallStats(outcomes) {
    ...
    calculateTimeRange(outcomes) {
        ...
        const sorted = [...outcomes].sort((a, b) => getTime(a.timestamp) - getTime(b.timestamp));
        const now = new Date();
        ...
        return {
            start: firstTs instanceof Date ? firstTs : (firstTs ? new Date(firstTs) : now),
            end: lastTs instanceof Date ? lastTs : (lastTs ? new Date(lastTs) : now),
        };
    }
    /**
     * Generate recommendations based on analysis
     */
    generateRecommendations(agentMetrics, keywordEffectiveness, confidenceMetrics) {
        const recommendations = [];
        const lowPerformingAgents = agentMetrics.filter((a) => a.successRate < 0.7);
        if (lowPerformingAgents.length > 0) {
            recommendations.push(`Review performance of ${lowPerformingAgents.length} agents with low success rates: ${lowPerformingAgents.map((a) => a.agent).join(", ")}`);
        }
        const ineffectiveKeywords = keywordEffectiveness.filter((k) => k.successRate < 0.6 && k.totalMatches >= 5);
        if (ineffectiveKeywords.length > 0) {
            recommendations.push(`Consider removing or refining ${ineffectiveKeywords.length} keywords with low effectiveness: ${ineffectiveKeywords.map((k) => k.keyword).slice(0, 5).join(", ")}`);
        }
        const highEscalationThresholds = confidenceMetrics.filter((m) => m.escalatedCount > m.totalRoutings * 0.5 && m.totalRoutings > 5);
        if (highEscalationThresholds.length > 0) {
            const threshold = highEscalationThresholds[0]?.threshold ?? 0.75;
            recommendations.push(`Threshold ${threshold} shows high escalation rate - consider adjusting keyword mappings or confidence scoring`);
        }
        ...
        if (optimalAgents.length > 0) {
            recommendations.push(`Top performing agents: ${optimalAgents.map((a) => `${a.agent} (${(a.successRate * 100).toFixed(1)}%)`).join(", ")}`);
        }
        ...
        if (lowConfidenceKeywords.length > 0) {
            recommendations.push(`Keywords with consistently low confidence: ${lowConfidenceKeywords.map((k) => k.keyword).join(", ")} - consider reviewing routing logic`);
        }
    ...
    generateFormattedReport() {
        ...
        const lines = [];
        lines.push("╔════════════════════════════════════════════════════════════╗");
        lines.push("║      0xRay Routing Performance Analytics Report      ║");
        lines.push("╚════════════════════════════════════════════════════════════╝");
        lines.push("");
        ...
        lines.push("");
        lines.push("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        lines.push("Generated by 0xRay Routing Performance Analyzer");
        return lines.join("\n");
    }
...
//# sourceMappingURL=routing-performance-analyzer.js.map