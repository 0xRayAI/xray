...
class PromptPatternAnalyzer {
    ...
    analyzePromptPatterns() {
        ...
        const templateMatches = promptData.filter((p) => p.templatePrompt && p.templatePrompt.length > 0);
        const nonTemplatePrompts = promptData.filter((p) => !p.templatePrompt || p.templatePrompt.length === 0);
        ...
        return {
            ...
            templateMatchRate: promptData.length > 0 ? templateMatches.length / promptData.length : 0,
            gaps,
            emergingPatterns,
            topMissedKeywords,
            agentCoverage,
     ...
     */
    detectTemplateGaps(promptData, outcomes) {
        const gaps = [];
     ...
     */
    identifyEmergingPatterns(promptData, outcomes) {
        ...
        for (const prompt of promptData) {
            const keywords = this.extractKeywords(prompt.userRequest || '');
            const significantKeywordsInPrompt = keywords.filter((kw) => significantKeywords.some(([sigKw]) => kw === sigKw));
            if (significantKeywordsInPrompt.length === 0)
                continue;
            ...
            if (patternMap.has(patternKey)) {
                ...
                if (outcome.success !== undefined) {
                    const totalSuccesses = existing.successRate * (existing.frequency - 1) +
                        (outcome.success ? 1 : 0);
        ...
        return Array.from(patternMap.values())
            .filter((pattern) => pattern.frequency >= this.emergingPatternMinSamples &&
     ...
     */
    analyzeMissedKeywords(promptData) {
        const missedKeywords = new Map();
     ...
     */
    calculateAgentCoverage(promptData) {
     ...
     */
    generateOptimizationSuggestions(comparisonResult) {
        const suggestions = [];
        for (const pattern of comparisonResult.emergingPatterns) {
            if (pattern.avgConfidence >= 0.85 &&
    ...
    generateReport(comparisonResult) {
        const lines = [];
        lines.push("╔════════════════════════════════════════════════════════════╗");
        lines.push("║        0xRay Prompt Pattern Analytics Report        ║");
        lines.push("╚════════════════════════════════════════════════════════════╝");
        lines.push("");
        lines.push(`Total Prompts Analyzed: ${comparisonResult.totalPrompts}`);
        lines.push(`Template Match Rate: ${(comparisonResult.templateMatchRate * 100).toFixed(1)}%`);
        ...
        if (comparisonResult.gaps.length > 0) {
            ...
            lines.push(`Found ${comparisonResult.gaps.length} recurring gaps (frequency ≥ ${this.minFrequencyThreshold})`);
        ...
        if (comparisonResult.emergingPatterns.length > 0) {
            ...
            lines.push(`Found ${comparisonResult.emergingPatterns.length} emerging patterns`);
        ...
        if (comparisonResult.agentCoverage.size > 0) {
            ...
            for (const [agent, stats] of Array.from(comparisonResult.agentCoverage.entries()).sort((a, b) => b[1].total - a[1].total)) {
                const coverage = ((stats.withTemplate / stats.total) * 100).toFixed(1);
                lines.push(`  ${agent}: ${coverage}% template coverage (${stats.withTemplate}/${stats.total})`);
            }
        }
        lines.push("");
        lines.push("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        lines.push("Generated by 0xRay Prompt Pattern Analyzer");
        return lines.join("\n");
    }
    classifyGapType(prompt, outcome) {
        if (prompt.templatePrompt && prompt.templatePrompt.length > 0) {
...
//# sourceMappingURL=prompt-pattern-analyzer.js.map