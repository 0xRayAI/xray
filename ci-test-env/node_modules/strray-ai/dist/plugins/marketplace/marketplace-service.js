...
export class PluginMarketplaceService {
    ...
    async getPlugin(id) {
        ...
        const sanitizedId = validation.sanitizedValue;
        try {
    ...
    async getPluginsByAuthor(authorId) {
        ...
        const sanitizedAuthorId = validation.sanitizedValue;
        try {
            const plugins = Array.from(this.plugins.values()).filter((plugin) => plugin.author.id === sanitizedAuthorId &&
    ...
    async getPluginsByCategory(category) {
        ...
        const sanitizedCategory = validation.sanitizedValue;
        try {
    ...
    async getFeaturedPlugins() {
        try {
            const plugins = Array.from(this.plugins.values()).filter((plugin) => plugin.status === "published" && plugin.stats.rating >= 4.0);
     ...
     */
    async getRecommendedPlugins(userId) {
        try {
            ...
            const plugins = Array.from(this.plugins.values()).filter((plugin) => plugin.status === "published" && plugin.stats.rating >= 3.5);
    ...
    async downloadPlugin(id, version) {
        ...
        const { id: sanitizedId, version: sanitizedVersion } = validation.sanitizedValue;
        try {
            const plugin = this.plugins.get(sanitizedId);
            if (!plugin) {
                return {
    ...
    async reportPlugin(id, report) {
        ...
        const { id: sanitizedId, report: sanitizedReport } = validation.sanitizedValue;
        try {
            const plugin = this.plugins.get(sanitizedId);
            if (!plugin) {
    ...
    async performSearch(query) {
        let candidates = Array.from(this.plugins.values()).filter((plugin) => plugin.status === "published");
        // Apply text search
        if (query.query) {
        ...
        if (query.tags && query.tags.length > 0) {
            candidates = candidates.filter((plugin) => query.tags.some((tag) => plugin.tags.includes(tag)));
        }
        // Apply rating filter
        if (query.minRating) {
     ...
     * Apply advanced filters
     */
    applyAdvancedFilters(plugins, filters) {
        return plugins.filter((plugin) => {
            // Verified filter
            ...
            if (filters.security) {
                ...
                if (filters.security === "high" && securityScore < 80)
                    return false;
                if (filters.security === "medium" && securityScore < 60)
                    return false;
            }
            // Compatibility filters
            if (filters.compatibility && filters.compatibility.length > 0) {
                const compatible = filters.compatibility.some((version) => plugin.compatibility.strRayVersions.includes(version));
                if (!compatible)
                    return false;
            }
            // License filters
            if (filters.license && filters.license.length > 0) {
                if (!filters.license.includes(plugin.license))
                    return false;
            }
            // Language filters
            if (filters.language && filters.language.length > 0) {
                const hasLanguage = filters.language.some((lang) => plugin.metadata.languages.includes(lang));
                if (!hasLanguage)
                    return false;
            }
            // Platform filters
            if (filters.platform && filters.platform.length > 0) {
                const hasPlatform = filters.platform.some((platform) => plugin.metadata.supportedPlatforms.includes(platform));
                if (!hasPlatform)
                    return false;
            }
            return true;
     ...
     * Sort search results
     */
    sortResults(plugins, sortBy, sortOrder) {
     ...
     */
    calculateTrendScore(plugin, since) {
     ...
     */
    calculateRelevanceScore(plugin) {
     ...
     */
    calculateSecurityScore(plugin) {
        let score = 50; // Base score
        if (plugin.security.verified)
            score += 20;
        if (plugin.security.compliance.owasp)
            score += 10;
        if (plugin.security.compliance.gdpr)
            score += 5;
        if (plugin.security.compliance.hipaa)
            score += 5;
        if (plugin.security.compliance.soc2)
            score += 5;
        if (plugin.security.compliance.iso27001)
            score += 5;
        if (plugin.security.vulnerabilities.length === 0)
            score += 10;
        else {
     ...
     * Generate secure download token
     */
    generateDownloadToken(pluginId, version) {
