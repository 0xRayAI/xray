...
class StringRayEnforcerToolsServer {
    ...
    async ruleValidation(args) {
        const { operation, files, newCode, existingCode, dependencies, tests } = args;
    ...
    async codexEnforcement(args) {
        ...
        const codexCheck = await this.simulateCodexValidation(operation, files, newCode, focusAreas);
        return {
            content: [
                {
                    type: "text",
    ...
    async contextAnalysisValidation(args) {
        ...
        const contextValidation = await this.simulateContextValidation(files, operation, checkPatterns);
        return {
            content: [
                {
                    type: "text",
    ...
    async qualityGateCheck(args) {
        ...
        const qualityCheck = await this.performQualityGateCheck(operation, context, strictMode);
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({
                        ...
                        autoFixesApplied: qualityCheck.fixes?.filter((f) => f.type === "auto")
                            .length || 0,
                        timestamp: new Date().toISOString(),
                    }, null, 2),
                },
    ...
    async getEnforcementStatus(args) {
        ...
        const status = await this.simulateEnforcementStatus(includeHistory, focusAreas);
        return {
            content: [
                {
                    type: "text",
    ...
    async runPreCommitValidation(args) {
        const { files, operation = "commit", autoFix = true, strictBlocking = true, } = args;
        // Pre-commit validation - no logging to console (results returned to agent)
        // Run comprehensive pre-commit validation
        const preCommitResult = await this.performPreCommitValidation(files, operation, autoFix, strictBlocking);
        return {
            content: [
                {
                    type: "text",
    ...
    async simulateRuleValidation(context) {
        ...
        if (context.newCode && context.existingCode) {
            ...
            for (const [filePath, existingContent] of Object.entries(context.existingCode)) {
    ...
    async simulateCodexValidation(operation, files, newCode, focusAreas) {
        ...
        if (newCode) {
            ...
            if (newCode.includes("any") || newCode.includes("@ts-ignore")) {
                violations.push('Codex violation: Type safety first - avoid "any" types and ts-ignore');
            }
            if (newCode.includes("console.log") && !newCode.includes("// DEBUG")) {
                warnings.push("Codex warning: Remove console.log statements in production code");
            }
            ...
            if (!newCode.includes("interface") &&
                !newCode.includes("type") &&
                newCode.length > 200) {
                warnings.push("Codex warning: Consider using TypeScript interfaces for complex objects");
            }
        }
            ...
            try {
                if (fs.existsSync(file)) {
                    ...
                    if (content.includes("CodebaseContextAnalyzer") &&
                        ...
                        warnings.push(`${file}: Consider using memory configuration for CodebaseContextAnalyzer`);
                    }
                    if (content.includes("ASTCodeParser") &&
                        !content.includes("try") &&
                        !content.includes("catch")) {
                        errors.push(`${file}: ASTCodeParser initialization should handle missing ast-grep gracefully`);
                    }
                    if (content.includes("DependencyGraphBuilder") &&
                        !content.includes("contextAnalyzer")) {
                        errors.push(`${file}: DependencyGraphBuilder requires proper context analyzer integration`);
                    }
                }
            }
            catch (error) {
                warnings.push(`${file}: Could not validate context integration - ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    ...
    async performQualityGateCheck(operation, context, strictMode) {
        ...
        return {
            ...
            overallScore: Math.round(((ruleCheckResult.errors?.length === 0 && ruleCheckResult.warnings?.length === 0 ? 100 : Math.max(0, 100 - (ruleCheckResult.errors?.length || 0) * 20 - (ruleCheckResult.warnings?.length || 0) * 5))
                + codexCheck.score
                + contextCheck.score) / 3),
        };
    }
    ...
    async performPreCommitValidation(files, operation, autoFix, strictBlocking) {
        ...
        const qualityCheck = await this.performQualityGateCheck(operation, context, strictBlocking);
        ...
        if (autoFix && qualityCheck.fixes) {
            ...
            qualityCheck.fixesApplied = autoFixes.filter((f) => f.applied).length;
        }
        return {
            ...qualityCheck,
