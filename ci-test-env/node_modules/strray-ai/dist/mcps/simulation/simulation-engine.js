...
export class SimulationEngine {
     ...
     */
    registerSimulator(serverName, toolName, simulator) {
     ...
     */
    registerServerSimulators(serverName, simulators) {
     ...
     */
    canSimulate(serverName, toolName) {
        return this.simulators.get(serverName)?.has(toolName) || false;
    }
    /**
     * Execute a simulation
     */
    async simulate(serverName, toolName, args) {
     ...
     */
    unregisterSimulator(serverName, toolName) {
        const serverSimulators = this.simulators.get(serverName);
        if (!serverSimulators) {
            return false;
        }
