 ...
 */
function sanitizeTermDescription(description) {
...
function parseJsonContent(content) {
    ...
    try {
        ...
        if (jsonData.terms) {
            Object.entries(jsonData.terms).forEach(([key, term]) => {
                // Sanitize term description to prevent runaway expansion
 ...
 */
function extractSectionList(content, sectionName) {
 ...
 */
export function parseCodexContent(content, sourcePath) {
    // Validate inputs
    if (!content || content.trim() === "") {
        return {
            success: false,
 ...
 */
export function extractCodexMetadata(content) {
 ...
 */
export async function validateTypeScriptSyntax(code, filePath) {
    const errors = [];
    const warnings = [];
    try {
 ...
 */
export async function validateBeforeModification(content, filePath) {
    const errors = [];
    const warnings = [];
