...
export class InferenceTuner {
    ...
    start() {
        ...
        frameworkLogger.log("inference-tuner", "starting", "info", { config: this.config });
        this.tuningInterval = setInterval(() => {
            this.runTuningCycle().catch((err) => {
                frameworkLogger.log("inference-tuner", "tuning-error", "error", { error: String(err) });
            });
    ...
    async runTuningCycle() {
        ...
        try {
            ...
            if (outcomes.length < 5 && patterns.length < 3) {
                frameworkLogger.log("inference-tuner", "skipping-insufficient-data", "debug", { outcomes: outcomes.length, patterns: patterns.length });
                return;
            }
            frameworkLogger.log("inference-tuner", "tuning-cycle-start", "info", { outcomes: outcomes.length, patterns: patterns.length });
            const results = await this.performTuning(outcomes, patterns);
            if (results.mappingsUpdated) {
                frameworkLogger.log("inference-tuner", "tuning-cycle-complete", "info", { mappingsAdded: results.mappingsAdded });
            }
     ...
     */
    async performTuning(outcomes, patterns) {
        const result = {
            mappingsAdded: 0,
            mappingsModified: 0,
        ...
        try {
            ...
            if (this.config.autoUpdateMappings) {
                ...
                for (const mapping of newMappings.slice(0, this.config.maxMappingsToAdd)) {
                    const added = this.addKeywordMapping(mapping.keyword, mapping.agent, mapping.skill, mapping.confidence);
                    if (added) {
            ...
            try {
                ...
                if (configUpdate.newMappings.length > 0) {
                    for (const suggestion of configUpdate.newMappings.slice(0, this.config.maxMappingsToAdd)) {
                        const added = this.addKeywordMapping(suggestion.keyword, suggestion.targetAgent, suggestion.targetSkill, suggestion.suggestedConfidence);
                        if (added) {
                            result.mappingsAdded++;
                        }
                    }
                ...
                frameworkLogger.log("inference-tuner", "refiner-error", "warning", { error: String(refinerError) });
            }
        }
        catch (error) {
            frameworkLogger.log("inference-tuner", "tuning-error", "error", { error: String(error) });
        }
        return result;
    }
    /**
    ...
    saveMappings(mappings) {
        ...
        try {
            ...
            frameworkLogger.log("inference-tuner", "mappings-save-error", "error", { error: String(error), path: mappingsPath });
            return false;
        }
    }
    /**
     ...
     */
    addKeywordMapping(keyword, agent, skill, confidence) {
        ...
        for (const mapping of mappings) {
            if (mapping.keywords.some(k => k === normalizedKeyword)) {
                if (mapping.agent !== agent) {
                    ...
                    frameworkLogger.log("inference-tuner", "mapping-conflict", "debug", { keyword: normalizedKeyword, existingAgent: mapping.agent, newAgent: agent });
                    return false;
                }
                ...
                if (confidence > mapping.confidence) {
                    ...
                    frameworkLogger.log("inference-tuner", "mapping-confidence-updated", "info", { keyword: normalizedKeyword, agent, oldConfidence: mapping.confidence, newConfidence: confidence });
                    return saved;
                }
        ...
        if (existingMapping) {
            if (!existingMapping.keywords.includes(normalizedKeyword)) {
                ...
                const saved = this.saveMappings(mappings);
                frameworkLogger.log("inference-tuner", "keyword-added-to-mapping", "info", { keyword: normalizedKeyword, agent, skill });
                return saved;
            }
            return false;
        }
        ...
        const saved = this.saveMappings(mappings);
        if (saved) {
            frameworkLogger.log("inference-tuner", "new-mapping-created", "info", { keyword: normalizedKeyword, agent, skill, confidence });
        }
        return saved;
    }
    /**
...
//# sourceMappingURL=inference-tuner.js.map