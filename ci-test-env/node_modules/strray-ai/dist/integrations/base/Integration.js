...
import { DEFAULT_INTEGRATION_CONFIG, createIntegrationStats, createUnhealthyResult, IntegrationNotInitializedError, IntegrationAlreadyInitializedError, IntegrationInitializationError, } from "./types.js";
/**
...
export class BaseIntegration extends EventEmitter {
     ...
     */
    constructor(name, version, initialConfig) {
        super();
        this.name = name;
    ...
    async initialize(config) {
        ...
        try {
            ...
            throw new IntegrationInitializationError(this.name, error instanceof Error ? error : new Error(errorMessage));
        }
    }
    /**
    ...
    async healthCheck() {
        ...
        if (this._status !== "initialized") {
            return createUnhealthyResult(`${this.name} is not initialized (status: ${this._status})`, { status: this._status });
        }
     ...
     */
    async log(status, message, details) {
        ...
        try {
            await frameworkLogger.log(this.name, message, loggerStatus, {
                ...details,
     ...
     */
    static isBaseIntegration(value) {
 ...
 * ```
 */
export function createSimpleIntegration(name, version, healthCheck, shutdown = async () => { }) {
