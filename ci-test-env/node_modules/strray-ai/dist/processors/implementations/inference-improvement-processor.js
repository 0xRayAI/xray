...
export class InferenceImprovementProcessor extends PostProcessor {
    ...
    async run(context) {
        ...
        await frameworkLogger.log("inference-improvement", "preparing_workflow_context", "info", { directory });
        try {
            ...
            await frameworkLogger.log("inference-improvement", "workflow_triggered", "info", {
            ...
            await frameworkLogger.log("inference-improvement", "error", "error", { error: error instanceof Error ? error.message : String(error) });
            return { success: false, error: String(error) };
        }
    }
    async prepareWorkflowContext(directory) {
    ...
    async saveWorkflowContext(directory, context) {
        ...
        fs.writeFileSync(path.join(workflowPath, filename), JSON.stringify(context, null, 2));
        fs.writeFileSync(path.join(workflowPath, "latest-workflow.json"), JSON.stringify(context, null, 2));
    ...
    async generateAgentPrompts(directory, context) {
        ...
        await frameworkLogger.log("inference-improvement", "agent_prompts_generated", "info", { count: Object.keys(prompts).length });
    }
    generateResearcherPrompt(context) {
...
`;
    ...
    async triggerAgentWorkflow(context) {
        ...
        await frameworkLogger.log("inference-improvement", "agent_workflow_initiated", "info", {
...
//# sourceMappingURL=inference-improvement-processor.js.map