...
export class AgentsMdValidationProcessor {
    ...
    async validateAgentsMd() {
        ...
        if (!content.includes("Kernel Patterns") && !content.includes("P9")) {
            result.warnings.push("Consider adding Kernel Patterns section (v1.7+)");
        }
        // Check 8: Reflection system section (required for v1.7.2+)
        if (!content.includes("Reflection System")) {
            result.warnings.push("Consider adding Reflection System section (v1.7.2+)");
        }
        // Check 9: Total agent count (should have 20+)
        const agentCount = (content.match(/@\[?[a-z-]+\]?/gi) || []).length;
        if (agentCount < 15) {
            result.warnings.push(`Only ${agentCount} agents found (recommend 20+ for v1.7+)`);
        }
        return result;
    }
    /**
