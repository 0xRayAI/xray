...
export class PerformanceCIGates {
    ...
    async runPerformanceGates(testSuite) {
        ...
        await frameworkLogger.log("performance-ci-gates", "budget-check-result", result.budgetCheck.passed ? "success" : "error", {
        ...
        await frameworkLogger.log("performance-ci-gates", "regression-check-result", result.regressionCheck.passed ? "success" : "error", {
            message: `   Regression Check: ${result.regressionCheck.passed ? "✅ PASSED" : "❌ FAILED"}`,
            passed: result.regressionCheck.passed,
        });
        await frameworkLogger.log("performance-ci-gates", "overall-result", result.success ? "success" : "error", {
        ...
        if (!result.success) {
            ...
            if (!result.budgetCheck.passed) {
                await frameworkLogger.log("performance-ci-gates", "-budget-violations-result-budgetcheck-violations-r", "info", {
                    message: `   - Budget violations: ${result.budgetCheck.violations} (${result.budgetCheck.criticalViolations} critical)`,
                });
            }
            if (!result.regressionCheck.passed) {
                await frameworkLogger.log("performance-ci-gates", "-failed-regression-tests-result-regressioncheck-fa", "error", {
                    message: `   - Failed regression tests: ${result.regressionCheck.failedTests}`,
                });
                await frameworkLogger.log("performance-ci-gates", "-average-deviation-result-regressioncheck-averaged", "info", {
                    message: `   - Average deviation: ${result.regressionCheck.averageDeviation.toFixed(2)}%`,
                });
            }
        }
        return result;
    ...
    async runBudgetCheck() {
        ...
        await frameworkLogger.log("performance-ci-gates", "-bundle-size-report-bundlesize-totalsize-1024-tofi", "info", {
            message: `   📦 Bundle Size: ${(report.bundleSize.totalSize / 1024).toFixed(2)} KB (${(report.bundleSize.gzippedSize / 1024).toFixed(2)} KB gzipped)`,
        });
        await frameworkLogger.log("performance-ci-gates", "-memory-usage-report-runtime-memoryusage-heapused-", "info", {
            message: `   🧠 Memory Usage: ${(report.runtime.memoryUsage.heapUsed / 1024 / 1024).toFixed(2)} MB`,
        });
        await frameworkLogger.log("performance-ci-gates", "-startup-time-report-runtime-startuptime-tofixed-2", "info", {
            message: `   ⚡ Startup Time: ${report.runtime.startupTime.toFixed(2)}ms`,
        });
        await frameworkLogger.log("performance-ci-gates", "violations-summary", passed ? "info" : "error", {
    ...
    async runRegressionCheck(testSuite) {
        ...
        if (suiteResult.summary.failed > 0) {
            await frameworkLogger.log("performance-ci-gates", "tests-failed", "error", { message: `   ❌ Failed: ${suiteResult.summary.failed}` });
        }
        await frameworkLogger.log("performance-ci-gates", "-average-deviation-suiteresult-summary-averagedevi", "info", {
            message: `   📈 Average Deviation: ${suiteResult.summary.averageDeviation.toFixed(2)}%`,
        });
        await frameworkLogger.log("performance-ci-gates", "-status-passed-passed-failed-", "info", { message: `   📊 Status: ${passed ? "PASSED" : "FAILED"}` });
        return {
            passed,
