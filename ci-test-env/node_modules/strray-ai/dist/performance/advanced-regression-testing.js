...
export class AdvancedRegressionTestingSystem extends EventEmitter {
    ...
    alerts = [];
    statisticalWindow = 100; // number of recent results to analyze
    anomalyThreshold = 0.95; // z-score threshold for anomalies
    constructor(baselineFile = "./performance-baselines-advanced.json") {
        super();
     ...
     */
    async runEnhancedRegressionTest(test) {
        const startTime = performance.now();
        try {
            // Run the test with timeout
            await Promise.race([
                test.testFunction(),
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Test timeout after ${test.timeout}ms`)), test.timeout)),
            ]);
            ...
            const anomalyScore = this.calculateAnomalyScore(duration, statisticalAnalysis);
            ...
            if (baseline) {
                ...
                const statisticalSignificance = this.calculateStatisticalSignificance(duration, baseline.statisticalModel);
            ...
            const confidenceLevel = this.calculateConfidenceLevel(statisticalAnalysis, history.length);
            // Generate historical context
            const historicalContext = this.generateHistoricalContext(duration, history);
            // Generate alerts
            const alerts = this.generateAlerts(test.name, duration, baseline, statisticalAnalysis, trendAnalysis);
            const result = {
                testName: test.name,
                duration,
    ...
    performStatisticalAnalysis(data) {
        ...
        const skewness = data.reduce((sum, val) => sum + Math.pow((val - mean) / stdDev, 3), 0) /
            n;
        const kurtosis = data.reduce((sum, val) => sum + Math.pow((val - mean) / stdDev, 4), 0) /
            n -
            3;
        return {
            mean,
     ...
     */
    calculateAnomalyScore(value, stats) {
     ...
     */
    calculateStatisticalSignificance(value, baselineStats) {
        if (baselineStats.standardDeviation === 0)
            return 1;
        const zScore = Math.abs((value - baselineStats.mean) / baselineStats.standardDeviation);
     ...
     */
    calculateConfidenceLevel(stats, sampleSize) {
        ...
        const varianceConfidence = Math.max(0, 1 - stats.standardDeviation / stats.mean);
        return (sampleConfidence + varianceConfidence) / 2;
    }
    /**
     * Generate historical context
     */
    generateHistoricalContext(current, history) {
        if (history.length < 2) {
            return {
                percentileRank: 50,
                comparedToLastWeek: 0,
        ...
        const percentileRank = (sorted.findIndex((val) => val >= current) / sorted.length) * 100;
        ...
        return {
            ...
            comparedToLastMonth: older.length > 0 ? ((current - olderAvg) / olderAvg) * 100 : 0,
        };
    }
    /**
     * Generate alerts based on analysis
     */
    generateAlerts(testName, current, baseline, stats, trends) {
        const alerts = [];
        if (!baseline)
            return alerts;
        // Regression alert
        const deviation = ((current - baseline.averageDuration) / baseline.averageDuration) * 100;
        if (Math.abs(deviation) > baseline.tolerance) {
            const significance = this.calculateStatisticalSignificance(current, baseline.statisticalModel);
            if (significance < 0.05) {
                alerts.push({
                    ...
                    testName,
                    type: "regression",
                    severity: Math.abs(deviation) > baseline.tolerance * 2 ? "high" : "medium",
     ...
     */
    updateBaselineIfNeeded(testName, newValue, result) {
        ...
        if (history.length < baseline.minimumSamples)
            return;
        ...
        const recentDeviation = Math.abs((recentStats.mean - baseline.averageDuration) / baseline.averageDuration);
     ...
     * Create initial baseline
     */
    createInitialBaseline(testName, initialValue) {
        const baseline = {
            testName,
    ...
    exportAnalysis() {
        return {
            ...
            statistics: {
                ...
                averageConfidence: Array.from(this.baselines.values()).reduce((sum, b) => sum +
                    (b.updateHistory[b.updateHistory.length - 1]?.confidence || 0), 0) / this.baselines.size,
            },
        };
    }
}
...
//# sourceMappingURL=advanced-regression-testing.js.map