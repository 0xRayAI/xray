...
function extractRepoSlug(input) {
    // Handle URLs
    ...
    if (/^[a-zA-Z0-9_-]+$/.test(input))
        return { owner: "htafolla", repo: input };
    return null;
}
function cloneRepo(url, targetDir) {
    ...
    if (hasGh) {
        try {
            ...
            const stderr = ghErr?.stderr?.toString("utf-8") || "";
    ...
    for (const branch of ["main", "master"]) {
        ...
        try {
            ...
            if (existsSync(repoDir))
                return repoDir;
            const dirs = readdirSync(targetDir).filter((f) => f.startsWith(slug.repo + "-") && f !== "_repo.tar.gz");
            if (dirs.length > 0)
    ...
    for (const [proto, gitUrl] of [["HTTPS", httpsUrl], ["SSH", sshUrl]]) {
        try {
            execSync(`git clone --depth 1 ${gitUrl} "${extracted}"`, { stdio: "pipe", timeout: 30000 });
            return extracted;
        }
        catch (gitErr) {
            const stderr = gitErr?.stderr?.toString("utf-8") || "";
            if (stderr.includes("Authentication failed") || stderr.includes("Invalid username")) {
                ...
                errors.push(`  git ${proto}: ${stderr.split("\n").filter((l) => l.includes("fatal:"))[0]?.trim() || gitErr.message.split("\n")[0]}`);
            }
        }
    }
    console.log("");
...
function installFlatMd(repoDir, skillsDir, sourcePrefix, sourceUrl, license) {
    ...
    for (const cat of subdirs) {
        ...
        const mdFiles = readdirSync(catDir).filter((f) => f.endsWith(".md") && f !== "README.md" && f !== "CONTRIBUTING.md" && f !== "LICENSE");
...
export async function skillRegistryCommand(action, args) {
    if (!action || action === "list") {
        const registry = getRegistry();
        const localPath = getLocalRegistryPath();
        const localNames = existsSync(localPath)
            ? new Set(JSON.parse(readFileSync(localPath, "utf-8")).sources.map((s) => s.name))
            : new Set();
        console.log("\n  Skills Registry\n  ===============\n");
    ...
    if (action === "remove") {
        ...
        const existing = JSON.parse(readFileSync(localPath, "utf-8"));
        const idx = existing.sources.findIndex((s) => s.name === name);
        if (idx < 0) {
            console.log(`  "${name}" not found in local registry.`);
            return;
