...
function getAgentConfig(agentName, cwd) {
    const configPaths = [
...
function createAgentManifest(agentName, configPath, version, cwd) {
    ...
    return {
        ...
        skills: [],
        permissions: extractYamlField(content, "permissions")?.split(",").map((p) => p.trim()) || [],
        tags: extractYamlField(content, "tags")?.split(",").map((t) => t.trim()) || [],
    };
}
function packageAgent(agentName, configPath, version, cwd) {
    ...
    writeFileSync(join(outputDir, "AGENT.md"), readFileSync(configPath, "utf-8"));
    return outputDir;
}
export async function publishAgentCommand() {
    ...
    if (!configPath) {
        ...
        if (existsSync(agentsDir)) {
            ...
            if (agents.length > 0) {
                agents.forEach((a) => console.log(`  • ${a.replace(/\.(yml|yaml)$/, "")}`));
            }
            else {
...
//# sourceMappingURL=publish-agent.js.map