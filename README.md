...
2. Fastens **mill plant** (`mill` + `inspect`). Sound seats: `foundry.json` `"plant": "sound"` fastens the bed factory (`sound` + `sound-inspect` + `sound-mixer`) and turns mill off. `sound mix` listens to the whole 4.44s. Blip seats: `foundry.json` `"plant": "blip"` fastens the tiny-video factory (`blip` + `blip-inspect` + `blip-vibe` + `blip-looker`) and turns mill off. Picture wears comic/anime cel; `blip vibe` is stamp density; `blip look` is the friend hitting replay; ship bar is 8. Not a 45-skill / 42-agent costume dump. `foundry.json` `"costume": true` is the opt-in dump.
