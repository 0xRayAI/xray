...
5. Use the product to build the product. Do not take hangar/commerce actions first — churn and put the suit on.

## Syncopation (the beats)

The grid is uneven on purpose. Do not flatten it into one timer.
...
These are the rules Blaze transcribed. A peer does not get them as chat. They live here.

### Bodies

| Body | Job | Identity law |
|------|-----|--------------|
...
When the mandated work turns into mantra, or the job has split across planes that no longer share one present, stop. Go up a level and answer the original message again from this seat. Do not continue the side thread and call that the job.

- Digressive inversion: the mantra becomes the long work.
- Parallelism glitch: parent, critic, and side branches each hold a different present.
- Cure: the reply to the original message. Not another seat.
...
Capture priority: a law is an invariant you would otherwise re-read the source to relearn. One law per subsystem bone. Telemetry stays in the kernel diary and inference output. Heat touches last_seen and does not append a confidence sample, so more hits do not raise conviction. Heat touches last_seen only when the diary contains that signal's name; a weak session score is not lifted to 0.55. The diary is `logs/framework/activity.log`, routing outcomes, pattern metrics, and `.xray/inference/latest-workflow.json` plus `workflow-status.json`. Hero names: `heat-is-not-conviction`, `inference-cycle-invariant`, `governance-vote-invariant`, `thin-dispatch-invariant`. Repertoire mergeStackOverlay refreshes a changed stack definition and keeps observation stats.

### Ship track (always)

1. **One thing at a time.** Brain (Cursor wear + Repertoire) before swarm. Swarm after the organ routes stack names.
2. **Always a branch.** Name `cursor/<descriptive>-<sess-suffix>`. Never implement on the default branch.
3. **Worktrees / user asides** when streams must run in parallel (`features.json` → `user_asides` + `worktree` / `branch`). Isolation is the rollback wall.
...
Quiet on repeat CLOSED / MERGED / LIVE.

## Where a peer reads this
...
Seats: `SEATS.md` · Clouds: `CLOUD-CONTINUITY.md` · Waves: `skills/enterprise-cos-wave-loop` · Compact: `skills/survive-compact` · Dist clock: `dist/CADENCE.md` · Capital: `AUTO-REVIEW-POLICY.md` · Ship gate: `skills/ship-ready-mill-gate`
