...
1. Every time run `npm publish --access public` — **no** `--auth-type=web`, **no** `--otp=` from chat.
2. Give the human the clickable `https://www.npmjs.com/auth/cli/<id>` (or `login/cli`) URL. Do not press Enter. Do not open a VM browser. Do not ask for a 6-digit authenticator code in chat.
3. If `EOTP` / `ENEEDAUTH` with no URL: `npm logout`, then run publish (or `npm login`) until the URL prints.
