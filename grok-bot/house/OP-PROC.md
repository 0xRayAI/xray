...
- **Disk, not chat.** After compact: Read Station, then board, then memory, then resume the same seat. Don't spawn a twin. [`house/WAVEBOARD.md`](house/WAVEBOARD.md) and [`ATTENTION_STATE.md`](house/ATTENTION_STATE.md) must exist or the board is theater.
