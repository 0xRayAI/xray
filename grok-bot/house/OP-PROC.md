...
| Rule | What to do |
|---|---|
| Clouds + churn | A cloud is only for heavy multi-file code that needs its own machine. Docs, board notes, PR review, deploy/publish, and small edits go to a seat. Same problem or PR: resume that cloud. One cloud per track. Check recent clouds before launching. The implementer owns every cloud. Churn is a second cloud, a twin reviewer, redoing work, or an idle loop — stop and card it. [CLOUD-CONTINUITY.md](../ops/CLOUD-CONTINUITY.md). |
