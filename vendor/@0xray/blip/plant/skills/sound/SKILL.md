...
2. **Genre** → tempo + voice prefs: `ambient` | `techno` | `jazz` | `phonk` | `rock` | `timeless` | `destination` (night-drive, **not** an ambient alias) | `dubstep`. `nightdrive` / `night-drive` alias to destination. `country` → timeless. `game` → techno. A piece of art selects a genre — destination motion locks night-drive.
