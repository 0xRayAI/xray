...
describe('sound inventory — art locks a genre', () => {
  it('lists live bodies, needed sounds, and destination night-drive', () => {
    const invPath = path.join(mill, 'plant/sounds/inventory.json');
    expect(existsSync(invPath)).toBe(true);
    const inv = JSON.parse(read('plant/sounds/inventory.json')) as {
      law: string;
      seedPick: string[];
    ...
    expect(read('plant/sounds/SOUND-INVENTORY.md')).toMatch(/night-drive/);
    expect(read('sound-rippel.cjs')).not.toMatch(/destination:\s*"ambient"/);
  });
  ...
  it('locks destination art to night-drive and inspects that bed', { timeout: 20000 }, () => {
    const { resolveGenre, GENRE_ALIASES, renderVinylDust, renderStaticDrop, renderChoppedVocal } =
      requireCjs(path.join(mill, 'sound-rippel.cjs')) as {
    ...
    const { resolveArtGenre, resolveGenreKind } = requireCjs(path.join(mill, 'blip-rippel.cjs')) as {
    ...
    const sound = requireCjs(path.join(mill, 'sound-bed.cjs')) as {
  ...
  it('tastes every live body at ship bar 8 and inspects dubstep', { timeout: 40000 }, () => {
    const { tasteMatrix, SHIP_BAR } = requireCjs(path.join(mill, 'sound-taste.cjs')) as {
      tasteMatrix: () => {
        status: string;
        shipBar: number;
  ...
  it('scores every live voice at ship bar 8', { timeout: 20000 }, () => {
    const { voiceMatrix, SHIP_BAR } = requireCjs(path.join(mill, 'sound-voice.cjs')) as {
      voiceMatrix: () => {
        status: string;
        shipBar: number;
        seats: Array<{ id: string; score: number; status: string; fails: string[] }>;
      };
      SHIP_BAR: number;
    };
    expect(SHIP_BAR).toBe(8);
    const report = voiceMatrix();
    expect(report.status, JSON.stringify(report.seats.filter((s) => s.status === 'FAIL'))).toBe(
      'PASS',
    );
    expect(report.seats.every((s) => s.score + 1e-9 >= 8)).toBe(true);
    const inv = JSON.parse(read('plant/sounds/inventory.json')) as {
      voices: Array<{ id: string; taste: number }>;
    };
    expect(inv.voices.every((v) => v.taste + 1e-9 >= 8)).toBe(true);
  });

  it('lists picture seats at bar 8', () => {
    const inv = JSON.parse(read('plant/pictures/inventory.json')) as {
      bar: number;
      motions: Array<{ id: string; vibe: number; looker: number }>;
      strokes: Array<{ id: string; vibe: number; looker: number }>;
    };
    expect(inv.bar).toBe(8);
    expect(inv.motions.map((m) => m.id)).toEqual(
      expect.arrayContaining(['still', 'destination', 'kapow', 'orb']),
    );
    expect(inv.motions.every((m) => m.vibe >= 8 && m.looker >= 8)).toBe(true);
    expect(inv.strokes.every((s) => s.vibe >= 8 && s.looker >= 8)).toBe(true);
    expect(read('plant/pictures/PICTURE-INVENTORY.md')).toMatch(/night-drive/);
  });
});
