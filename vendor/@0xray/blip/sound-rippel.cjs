...
const GENRE_ALIASES = {
  ...
  nightdrive: "destination",
  "night-drive": "destination",
  country: "timeless",
  game: "techno",
};
...
function renderReese({ sampleRate, freq, velocity, hold }) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const detune = 1.018 + 0.004 * Math.sin(2 * Math.PI * 0.37 * t);
    const growl = 0.08 * Math.sin(2 * Math.PI * 48 * t);
    const mix = 0.48 * saw(ph0) + 0.44 * saw(ph1) + growl;
    out[i] = velocity * env * lp.step(mix);
    ph0 += (2 * Math.PI * freq) / sampleRate;
    ph1 += (2 * Math.PI * freq * detune) / sampleRate;
  }
  return out;
}
...
function renderCowbell({ sampleRate, velocity }) {
  ...
  const dur = p.attack + p.decay + (p.release || 0.3);
  const n = Math.floor(dur * sampleRate);
  const out = new Float64Array(n);
  const clang = renderMetal({
    sampleRate,
    freq: 845,
    ...
    decay: p.decay,
    release: p.release,
    velocity: (velocity || 0.4) * 0.72,
  });
  let phA = 0;
  let phB = 0;
  for (let i = 0; i < n; i++) {
    const t = i / sampleRate;
    const env = oneshotAmp(t, p.attack, p.decay + (p.release || 0.3) * 0.55);
    const partial = 0.62 * Math.sin(phA) + 0.38 * Math.sin(phB);
    const metal = i < clang.length ? clang[i] : 0;
    out[i] = (velocity || 0.4) * env * partial * 0.55 + metal;
    phA += (2 * Math.PI * 845) / sampleRate;
    phB += (2 * Math.PI * 560) / sampleRate;
  }
  return out;
}

function renderVinylDust({ sampleRate, seconds, velocity }) {
  const n = Math.max(1, Math.floor((seconds || 4.44) * sampleRate));
  const out = new Float64Array(n);
  const noise = noiseLcg(0x51c10ad);
  const hp = highpass(1400, sampleRate);
  const lp = lowpass(6800, sampleRate);
  const rumble = lowpass(180, sampleRate);
  const vel = velocity || 0.16;
  let popHold = 0;
  for (let i = 0; i < n; i++) {
    const raw = noise() * 2 - 1;
    const roll = noise();
    if (roll > 0.984) popHold = 0.72 + (roll - 0.984) * 8;
    const tick = popHold > 0 ? raw * popHold : raw * 0.055;
    if (popHold > 0) popHold *= 0.86;
    const bed = rumble.step(raw * 0.12) + lp.step(hp.step(tick));
    out[i] = bed * vel;
  }
  return out;
}

function renderStaticDrop({ sampleRate, velocity }) {
  const dur = 0.62;
  const n = Math.floor(dur * sampleRate);
  const out = new Float64Array(n);
  const noise = noiseLcg(0x57a71c);
  const hp = highpass(900, sampleRate);
  const lp = lowpass(5200, sampleRate);
  const vel = velocity || 0.5;
  for (let i = 0; i < n; i++) {
    const t = i / sampleRate;
    const rise = t < 0.08 ? t / 0.08 : 1;
    const fall = t > 0.22 ? Math.max(0, 1 - (t - 0.22) / 0.4) : 1;
    const grit = noise() * 2 - 1;
    out[i] = lp.step(hp.step(grit)) * vel * rise * fall;
  }
  return out;
}
...
function renderRockGuitar({ sampleRate, freq, velocity, hold }) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    out[i] = (velocity || 0.4) * env * lp.step(mid.step(grit) * 0.88 + grit * 0.2);
    ph += (2 * Math.PI * hz) / sampleRate;
  }
  return out;
}
...
function renderFormant({ sampleRate, freq, velocity, hold }) {
  ...
  const lp = lowpass(2400, sampleRate);
  let phase = 0;
  const breath = noiseLcg((freq * 31) | 0);
  for (let i = 0; i < n; i++) {
    const t = i / sampleRate;
    const env = oneshotAmp(t, attack, decay + release * 0.4) * velocity;
    const vib = 1 + 0.012 * Math.sin(2 * Math.PI * 5.4 * t);
    const inc = (2 * Math.PI * freq * vib) / sampleRate;
    const raw = squareBlep(phase, inc);
    const air = (breath() * 2 - 1) * Math.exp(-t / 0.05) * 0.08;
    const voice = f1.step(raw) * 0.45 + f2.step(raw) * 0.35 + f3.step(raw) * 0.2;
    out[i] = lp.step((voice + air) * env);
    phase += inc;
  }
  return out;
}
...
function masterEnvelope(samples, sampleRate, seconds, lock, genreId) {
  const blip = Boolean(lock) && seconds <= 5;
  const introHold = blip ? 0 : 0.5;
  const introEase = blip ? 0.03 : 0.35;
  const introShelf = 0.75;
  const introFloor = genreId === "rock" ? 0.3 : MIXER.introFloor;
...
function sectionGain(t, seconds, lock, grid) {
  ...
  if (lock && seconds <= 5) {
    ...
    return Math.max(
      0.62,
      0.74 * phrase.hookEase +
        1.16 * phrase.turnEase +
        0.82 * phrase.tagEase +
        0.2 * (phrase.turnHit || 0) +
        0.12 * (phrase.tagHit || 0),
    );
  }
...
function mixPhraseDrop({ kickBus, hatBus, colorBus, sampleRate, seconds, grid, genreId, seedHex, lock }) {
  ...
  if (marks.turnAt < seconds - 0.05) {
    ...
    const crashVel = genreId === "timeless" ? 0.22 : rockCrash ? 0.5 : MIXER.crashVel;
    ...
    if (genreId === "dubstep") {
      ...
      mixInto(
        ...
        renderMetal({
          ...
          resonance: 2400,
          attack: 0.001,
          decay: rockCrash ? 0.16 : 0.42,
          release: rockCrash ? 0.06 : 0.16,
          velocity: crashVel,
        }),
        Math.floor(marks.turnAt * sampleRate),
        1,
    ...
    mixInto(
      ...
      renderMetal({
        ...
        attack: 0.001,
        decay: genreId === "timeless" ? 0.4 : rockCrash ? 0.1 : 0.26,
        release: rockCrash ? 0.05 : 0.1,
        velocity: crashVel * (genreId === "timeless" ? 0.9 : rockCrash ? 0.48 : 0.7),
      }),
      Math.floor(marks.turnAt * sampleRate),
      1,
    );
    mixInto(kickBus, kickClick(sampleRate, genreId === "phonk" || genreId === "destination" ? 0.28 : 0.18), Math.floor(marks.turnAt * sampleRate), 1);
    if (genreId === "phonk" || genreId === "destination") {
      mixInto(
        kickBus,
        render808Slide({
...
function renderRippelBed({ brief, genre, seconds, seedHex, rng, sampleRate, syncopate }) {
  ...
  if (g.id === "techno") {
    ...
    for (const t of schedule(seconds, beat * 2, 0.18)) {
      ...
      mixInto(
        ...
        renderReese({
          sampleRate,
          freq: scale[(beatIdx % 2 === 0 ? 0 : 3) % scale.length],
          velocity: 0.4,
          hold: beat * 1.15,
        }),
    ...
    for (const t of schedule(seconds, beat * 2, 0.12)) {
      mixInto(
        colorBus,
        renderCowbell({ sampleRate, velocity: 0.14 }),
        Math.floor((t + beat * 1.5) * sampleRate),
        1,
      );
    }
  ...
  if (g.id === "rock") {
    mixInto(padBus, renderPad(n, sampleRate, padTones("rock", scale), rng, 0.28), 0, 0.36);
    const kick = RIPPEL.technoKick;
    const hat = RIPPEL.technoHat;
    for (const t of schedule(seconds, beat, 0.08)) {
      const hit = renderMembrane({
        ...
        decay: 0.36,
        release: 0.5,
        velocity: 0.72 * sectionGain(t, seconds, lock, grid),
        floorHz: CRYSTAL.kickFloorHz,
      });
      mixInto(kickBus, hit, Math.floor(t * sampleRate), 1);
      mixInto(kickBus, kickClick(sampleRate, 0.2), Math.floor(t * sampleRate), 1);
  ...
  const kickCh = applyBus(kickBus, sampleRate, {
    hp: g.id === "phonk" || g.id === "destination" ? RIPPEL.phonk808.hp : 30,
    lp: g.id === "phonk" || g.id === "destination" ? RIPPEL.phonk808.lp : 9000,
    eq: g.id === "phonk" || g.id === "destination" ? RIPPEL.phonk808.eq : RIPPEL.technoKick.eq,
    dist: g.id === "phonk" || g.id === "destination" ? RIPPEL.phonk808.dist : RIPPEL.technoKick.dist,
    comp: g.id === "phonk" || g.id === "destination" ? RIPPEL.phonk808.comp : RIPPEL.technoKick.comp,
    volDb: g.id === "ambient" || g.id === "timeless" ? -4 : g.id === "jazz" ? RIPPEL.jazzKick.volDb : g.id === "phonk" || g.id === "destination" ? RIPPEL.phonk808.volDb : RIPPEL.technoKick.volDb,
  });
  const hatCh = applyBus(hatBus, sampleRate, {
    hp: g.id === "techno" || g.id === "phonk" || g.id === "rock" || g.id === "destination" ? CRYSTAL.hatAirHz : 7200,
    lp: 16000,
  ...
  const colorCh = applyBus(colorBus, sampleRate, {
    ...
    lp: g.id === "phonk" || g.id === "destination" ? 7000 : 9000,
    eq: g.id === "techno" || g.id === "rock" ? RIPPEL.technoBass.eq : g.id === "phonk" || g.id === "destination" ? RIPPEL.phonkReese.eq : { low: -2, mid: 2, high: -3 },
    dist: g.id === "techno" || g.id === "rock" ? RIPPEL.technoBass.dist : g.id === "phonk" || g.id === "destination" ? RIPPEL.phonkReese.dist : 0.08,
    comp: RIPPEL.technoBass.comp,
    volDb: g.id === "techno" ? RIPPEL.technoBass.volDb : g.id === "phonk" || g.id === "destination" ? RIPPEL.phonkReese.volDb : -8,
  });
  ...
  for (let i = 0; i < n; i++) {
    ...
    mix[i] =
      ...
      plate[i] * MIXER.plateSend * (g.id === "rock" ? 0.26 : 1) * (0.4 + 0.6 * duck) +
      glue[i];
  }
  ...
  const shaped = masterEnvelope(glued, sampleRate, seconds, lock, g.id);
