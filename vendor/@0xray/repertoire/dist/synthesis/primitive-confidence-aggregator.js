export function aggregateWeightedPrimitives(entries) {
    const totals = new Map();
    for (const entry of entries) {
        const confidenceMap = entry.match_confidence;
        if (!confidenceMap || !entry.matched_primitives?.length)
            continue;
        for (const name of entry.matched_primitives) {
            ...
            current.count += 1;
            if (entry.governance_forced)
                current.forcedCount += 1;
...
export function formatWeightedPrimitivesSection(entries, limit = 8) {
    ...
    return weighted
        .map((primitive, index) => `${index + 1}. ${primitive.name} — avg_confidence=${primitive.avgConfidence.toFixed(3)}, occurrences=${primitive.occurrenceCount}, weighted_score=${primitive.weightedScore.toFixed(3)}, governance_forced_hits=${primitive.governanceForcedCount}`)
        .join('\n');
}
//# sourceMappingURL=primitive-confidence-aggregator.js.map