...
export class WebhookTrigger {
    ...
    app;
    config;
    constructor(postProcessor, config // Make config optional
    ) {
    ...
    async initialize() {
        ...
        frameworkLogger.log('webhook-trigger', 'initialized', 'info', {
        ...
        this.app.post(webhookPath, async (req, res) => {
            try {
                ...
                if (this.config.verifySignature && this.config.secret) {
                    ...
                    if (!signature) {
                        frameworkLogger.log('webhook-trigger', 'signature-verification-failed', 'info', { provider: this.config.provider, reason: 'no-signature-header' });
                        return res.status(401).json({ error: 'Signature required' });
                    }
                    if (!this.verifySignature(req.body, signature)) {
                        frameworkLogger.log('webhook-trigger', 'signature-verification-failed', 'info', { provider: this.config.provider || 'unknown', signature });
                ...
                if (this.config.events && this.config.events.length > 0 && !this.config.events.includes(event)) {
                    frameworkLogger.log('webhook-trigger', 'event-not-allowed', 'info', { provider: this.config.provider, event, allowed: this.config.events });
                    return res.status(200).json({ received: true, ignored: true });
                }
                // Parse payload
                const payloadStr = req.body.toString('utf-8');
                const payload = JSON.parse(payloadStr);
                frameworkLogger.log('webhook-trigger', 'webhook-received', 'info', { provider: this.config.provider, event, payloadKeys: Object.keys(payload) });
                ...
                frameworkLogger.log('webhook-trigger', 'webhook-error', 'error', { provider: this.config.provider, error: error instanceof Error ? error.message : String(error) });
    ...
    verifySignature(payload, signature) {
        ...
        try {
            ...
            frameworkLogger.log('webhook-trigger', 'signature-verification-error', 'error', { provider, error: errorMessage });
            return false;
        }
    }
    /**
    ...
    extractFilesFromPayload(payload, event) {
        try {
            ...
            frameworkLogger.log('webhook-trigger', 'file-extraction-failed', 'info', { provider: this.config.provider, error: error instanceof Error ? error.message : String(error) });
            return [];
        }
    }
