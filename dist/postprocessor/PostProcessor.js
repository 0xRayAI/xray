...
export class PostProcessor {
    ...
    async loadProcessorConfig() {
        ...
        try {
            ...
            await frameworkLogger.log("-post-processor", "-processor-config-load-failed", "info", { message: `Could not load processor config: ${error}` });
        }
        ...
        while (attempts < maxAttempts) {
            ...
            if (monitoringResult.overallStatus === "success") {
                ...
                if (hasAgentChanges || process.env.ENABLE_AGENTS_AUTO_UPDATE === "always") {
                    try {
                        const { researcherAgentsUpdater } = await import("../agents/librarian-agents-updater.js");
