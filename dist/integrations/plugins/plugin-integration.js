 ...
 */
export function parseYamlManifest(content) {
...
export class PluginIntegration extends BaseIntegration {
    ...
    childProcess = null;
    pluginPath = "";
    pluginType = PluginType.MCP_SERVER;
    autoStart = true;
    ...
    resourceLimits = {};
    permissions = {};
    constructor(name, version, pluginPath, type = PluginType.MCP_SERVER, config) {
        super(name, version, {
    ...
    async startPlugin() {
        ...
        try {
            ...
            this.childProcess.on("error", (err) => {
                ...
                frameworkLogger.log("plugin-integration", "process-error", "error", { name: this.name, error: err.message });
            });
            this.childProcess.on("exit", (code, signal) => {
                const wasUnexpected = signal !== "SIGTERM" && signal !== "SIGKILL";
                if (wasUnexpected) {
                    this.restarts++;
                    frameworkLogger.log("plugin-integration", "process-crashed", "error", { name: this.name, code, signal, restarts: this.restarts });
                }
                else {
                    frameworkLogger.log("plugin-integration", "process-exit", "info", { name: this.name, code, signal });
                }
            ...
            if (this.childProcess.stdout) {
                this.childProcess.stdout.on("data", (data) => {
                    if (this.config.debug) {
                        frameworkLogger.log("plugin-integration", "stdout", "debug", { name: this.name, output: data.toString().trim() });
                    }
                });
            }
            if (this.childProcess.stderr) {
                this.childProcess.stderr.on("data", (data) => {
                    frameworkLogger.log("plugin-integration", "stderr", "warning", { name: this.name, output: data.toString().trim() });
                });
            }
     ...
     */
    async disable() {
        if (this.pluginState === PluginState.UNINSTALLED)
            return;
