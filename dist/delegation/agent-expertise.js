 ...
 * @since 2026-04-16
 */
export const AGENT_EXPERTISE_LEVELS = {
    architect: {
        name: "architect",
        expertiseLevel: 10,
...
export function getAgentExpertise(agentName) {
    return AGENT_EXPERTISE_LEVELS[agentName];
}
export function getAgentExpertiseLevel(agentName) {
    return AGENT_EXPERTISE_LEVELS[agentName]?.expertiseLevel ?? 5;
}
export function getVotingWeight(agentName) {
    const level = getAgentExpertiseLevel(agentName);
    return level * 10;
}
export function getAgentsWithExpertiseDomain(domain) {
    return Object.values(AGENT_EXPERTISE_LEVELS).filter((agent) => agent.domain === domain || agent.specialties.some((s) => domain.includes(s)));
}
export function getTopExpertsForDomain(domain, count = 3) {
    const domainExperts = getAgentsWithExpertiseDomain(domain);
    return domainExperts
        .sort((a, b) => b.expertiseLevel - a.expertiseLevel)
        .slice(0, count);
}
//# sourceMappingURL=agent-expertise.js.map