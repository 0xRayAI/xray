...
export class DependencyGraphBuilder {
    codebaseAnalyzer;
    astParser;
    constructor(codebaseAnalyzer, astParser) {
    ...
    async buildDependencyGraph() {
        ...
        await frameworkLogger.log("dependency-graph-builder", "analysis-start", "info", {
            jobId,
        ...
        for (const [filePath, fileInfo] of Array.from(codebaseAnalysis.structure.fileGraph)) {
            graph.set(filePath, {
                file: filePath,
                module: this.getModuleName(filePath, codebaseAnalysis.structure.modules),
        ...
        await this.enhanceWithASTAnalysis(graph, codebaseAnalysis.structure.fileGraph, jobId);
        ...
        await frameworkLogger.log("dependency-graph-builder", "analysis-complete", "success", {
            jobId,
            files: graph.size,
     ...
     */
    async enhanceWithASTAnalysis(graph, fileGraph, jobId) {
        for (const [filePath, fileInfo] of Array.from(fileGraph)) {
            if (this.isSourceFile(filePath)) {
                try {
                    ...
                    if (node) {
                        node.imports = Array.from(new Set([
                            ...node.imports,
                            ...astAnalysis.structure.imports.map((imp) => imp.module),
                        ]));
                        node.exports = Array.from(new Set([
                            ...node.exports,
                    ...
                    await frameworkLogger.log("dependency-graph-builder", "ast-enhancement-failed", "info", {
                        jobId,
                        filePath,
     ...
     */
    markCircularDependencies(graph, circularChains) {
     ...
     */
    calculateDependencyMetrics(graph, chains) {
        const allImports = Array.from(graph.values()).flatMap((node) => node.imports);
        const uniqueModules = new Set(allImports).size;
        const maxDepth = Math.max(...Array.from(graph.values()).map((node) => node.depth));
        const circularDependencies = chains.filter((chain) => chain.type === "circular").length;
        const orphanFiles = Array.from(graph.values()).filter((node) => node.dependents.length === 0 &&
            (!node.imports || node.imports.length === 0)).length;
        const tightlyCoupledFiles = Array.from(graph.values()).filter((node) => node.dependents.length > 10 ||
            (node.imports && node.imports.length > 10)).length;
        const averageDependenciesPerFile = allImports.length / graph.size;
        // Calculate coupling index (0-100, higher = more coupled)
        const couplingIndex = Math.min(100, averageDependenciesPerFile * 10 +
     ...
     */
    identifyDependencyIssues(graph, chains, metrics) {
        const issues = [];
        ...
        for (const chain of chains.filter((c) => c.type === "circular")) {
            issues.push({
                ...
                suggestion: "Refactor to break the circular dependency using dependency injection or event-driven patterns",
            });
        }
        // Orphan files
        if (metrics.orphanFiles > 0) {
            ...
            issues.push({
                ...
                suggestion: "Review orphan files for removal or integration into the codebase",
            });
        }
        // Tight coupling
        if (metrics.tightlyCoupledFiles > 0) {
            const tightlyCoupled = Array.from(graph.values())
                .filter((node) => node.dependents.length > 10 || node.imports.length > 10)
                .map((node) => node.file);
            issues.push({
                type: "tight-coupling",
                severity: "high",
                ...
                suggestion: "Consider breaking down tightly coupled files into smaller, focused modules",
            });
        }
    ...
    generateRecommendations(issues, metrics) {
        ...
        if (metrics.couplingIndex > 70) {
            recommendations.push("High coupling detected - consider implementing dependency injection to reduce tight coupling");
        }
        if (metrics.circularDependencies > 0) {
            recommendations.push("Break circular dependencies by introducing interfaces or event-driven communication");
        }
        if (metrics.maxDepth > 3) {
            recommendations.push("Flatten deep dependency chains by creating facade classes or reducing abstraction layers");
        }
        if (metrics.orphanFiles > metrics.totalDependencies * 0.1) {
            recommendations.push("High number of orphan files - conduct code cleanup to remove dead code");
        }
        if (issues.some((issue) => issue.type === "tight-coupling")) {
            recommendations.push("Break down tightly coupled components by implementing facade pattern or adapter pattern");
        }
        return recommendations;
    }
    // Helper methods
    getModuleName(filePath, modules) {
