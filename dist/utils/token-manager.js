...
export class TokenManager {
     ...
     */
    pruneContext(context, targetTokens = this.config.maxPromptTokens) {
    ...
    extractCriticalSections(context) {
        ...
        if (frameworkSection)
            criticalSections.push(frameworkSection);
        return criticalSections;
    }
    extractSection(lines, sectionTitle) {
    ...
    extractNonCriticalContent(context) {
        const criticalSections = this.extractCriticalSections(context);
        let result = context;
    ...
    pruneNonCriticalContent(content, maxTokens) {
     ...
     */
    generateWarning(limitCheck) {
        if (limitCheck.withinLimit && !limitCheck.warning)
            return "";
        const messages = [];
