 ...
 */
function validateSafeString(str, operationType) {
...
export async function batchReplace(files, operations) {
    ...
    if (config.prefer_sed_for_replacements &&
        operations.length === 1 &&
        firstOperation) {
        return executeSedBatch(files, firstOperation, startTime);
    }
    if (config.parallel_file_updates &&
 ...
 */
async function executeSedBatch(files, operation, startTime) {
    const errors = [];
    let filesModified = 0;
...
async function executeParallelBatch(files, operations, maxConcurrent, startTime) {
    const errors = [];
    let filesModified = 0;
    const chunks = [];
...
async function executeSequential(files, operations, startTime) {
    const errors = [];
    let filesModified = 0;
    for (const file of files) {
        try {
...
export async function batchFileOperations(operations) {
    const startTime = Date.now();
    const errors = [];
    let filesModified = 0;
    for (const op of operations) {
...
export function findFilesWithPattern(directory, pattern, extensions = [".ts", ".tsx", ".js", ".jsx", ".json"]) {
    const results = [];
...
export async function bulkRename(directory, oldName, newName, extensions = [".ts", ".tsx", ".js", ".jsx"]) {
