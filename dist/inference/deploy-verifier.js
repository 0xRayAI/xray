...
export class DeployVerifier {
    ...
    verify() {
        ...
        const cliCheck = this.runCheck("cli-health", () => {
            ...
            if (!fs.existsSync(cliPath))
                throw new Error("CLI not found in installed package");
        ...
        for (const e2e of E2E_TEST_PATTERNS) {
            const testCheck = this.runCheck(`e2e-${e2e.name}`, () => {
                const output = this.exec(`npx vitest run --reporter=verbose -- ${e2e.pattern}`, this.projectRoot, 120000);
    ...
    cleanup(installDir, tarballPath) {
        try {
    ...
    result(success, version, tarballPath, installDir, checks, startTime) {
        return {
            success,
...
//# sourceMappingURL=deploy-verifier.js.map