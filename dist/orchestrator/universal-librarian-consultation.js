...
export class UniversalLibrarianConsultation {
    ...
    async consultBeforeAction(action) {
        ...
        if (this.isLibrarianOperation(action)) {
            await frameworkLogger.log("universal-librarian-consultation", "pre-action-consultation-skipped", "info", {
                reason: "researcher operation detected - skipping to prevent recursion",
                actionType: action.type,
                scope: action.scope,
            });
            return {
     ...
     */
    isLibrarianOperation(action) {
        return (action.scope === "documentation" ||
