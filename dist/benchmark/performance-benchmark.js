...
export class StringRayPerformanceBenchmark {
     ...
     */
    startBenchmark(operation, metadata = {}) {
     ...
     */
    endBenchmark(benchmarkId, success = true, error) {
        const activeBenchmark = this.activeBenchmarks.get(benchmarkId);
        if (!activeBenchmark) {
            frameworkLogger.log("performance-benchmark", "benchmark-not-found", "warning", { benchmarkId });
            return null;
        }
    ...
    async benchmarkOrchestratorTasks(_orchestrator, taskCount = 10, concurrentTasks = 3) {
        ...
        try {
            ...
            await frameworkLogger.log("performance-benchmark", "orchestrator-benchmarking-failed", "error", { error: String(error) });
        }
        return result;
    }
    /**
    ...
    async benchmarkSessionOperations(_sessionStateManager, operationCount = 20) {
        ...
        try {
            ...
            await frameworkLogger.log("performance-benchmark", "session-benchmarking-failed", "error", { error: String(error) });
        }
        return result;
    }
    /**
     ...
     */
    calculateMetrics(operationFilter) {
        ...
        const medianDuration = durations.length > 0 ? durations[medianIndex] : 0;
        const p95Index = Math.floor(durations.length * 0.95);
        const p95Duration = durations.length > 0 ? durations[p95Index] : 0;
        const p99Index = Math.floor(durations.length * 0.99);
        const p99Duration = durations.length > 0 ? durations[p99Index] : 0;
