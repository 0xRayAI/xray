...
export class ViolationFixer {
     ...
     * ```
     */
    async fixViolations(violations, context) {
        const fixes = [];
        for (const violation of violations) {
            try {
                await frameworkLogger.log('violation-fixer', 'attempting-fix', 'info', {
                    rule: violation.rule,
                    message: violation.message,
                });
                const strategy = this.getFixStrategy(violation.rule);
                if (!strategy) {
                    await frameworkLogger.log('violation-fixer', 'no-strategy-found', 'error', {
                        rule: violation.rule,
                ...
                await frameworkLogger.log('violation-fixer', 'fix-attempted', 'success', {
                    rule: violation.rule,
                    agent,
                    skill,
                ...
                await frameworkLogger.log('violation-fixer', 'fix-failed', 'error', {
                    rule: violation.rule,
                    error: errorMessage,
     ...
     * ```
     */
    registerFixStrategy(ruleId, strategy) {
...
//# sourceMappingURL=violation-fixer.js.map