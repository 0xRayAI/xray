...
class StringRayProcessorPipelineServer {
    ...
    constructor() {
        ...
        this.server = new Server({
            name: "processor-pipeline", version: "1.22.47",
        }, {
            capabilities: {
                tools: {},
            },
    ...
    async loadCodexRules() {
        try {
            if (await this.codexLoader.isAvailable()) {
                this.codexRules = await this.codexLoader.load();
                await frameworkLogger.log("mcps/processor-pipeline", "codex-rules-loaded", "success", { message: `Loaded ${this.codexRules.length} codex rules for MCP validation` });
            }
        }
        catch (error) {
