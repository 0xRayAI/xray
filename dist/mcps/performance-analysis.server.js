...
class StringRayPerformanceAnalysisServer {
    ...
    constructor() {
        this.server = new Server({
            name: "performance-analysis", version: "1.22.47",
        }, {
            capabilities: {
                tools: {},
            },
    ...
    async handlePerformanceAnalysis(args) {
        ...
        try {
            ...
            if (scope === "resources" || scope === "full") {
                ...
                analysisResults.recommendations.push(...resourceMetrics.recommendations);
            }
            ...
            if (scope === "bottlenecks" || scope === "full") {
                ...
                analysisResults.recommendations.push(...bottleneckResults.recommendations);
            }
            ...
            analysisResults.bottlenecks.push(`Analysis error: ${error instanceof Error ? error.message : String(error)}`);
        }
...
**Overall Status:** ${analysisResults.passed ? "✅ PERFORMANCE ACCEPTABLE" : "❌ PERFORMANCE ISSUES DETECTED"}`;
    ...
    async handleBottleneckDetection(args) {
        ...
        jobId);
        try {
            const results = await this.analyzeSpecificBottlenecks(operation, threshold);
            return {
                content: [
                    {
                        type: "text",
...
**Status:** ${results.critical ? "❌ CRITICAL BOTTLENECKS" : "✅ NO CRITICAL ISSUES"}`,
    ...
    async analyzeRuntimePerformance(duration) {
        const results = {
            responseTime: 0,
            throughput: 0,
            memoryUsage: 0,
        ...
        try {
            ...
            if (results.memoryUsage > 256) {
                results.bottlenecks.push(`High memory usage: ${results.memoryUsage}MB > 256MB threshold`);
                results.recommendations.push("Optimize memory usage through garbage collection and object pooling");
            }
            // CPU usage (simplified)
            const cpuUsage = os.loadavg()[0] || 0;
            results.cpuUsage = Math.round(cpuUsage * 100) / 100;
            if (cpuUsage > 0.8) {
                results.bottlenecks.push(`High CPU usage: ${results.cpuUsage} > 80% threshold`);
                results.recommendations.push("Optimize CPU-intensive operations and consider load balancing");
            }
            ...
            if (fs.existsSync("src/index.ts")) {
                ...
                if (initTime > 100) {
                    results.bottlenecks.push(`Slow initialization: ${initTime}ms > 100ms threshold`);
                    results.recommendations.push("Optimize framework initialization and lazy loading");
                }
            }
        }
        catch (error) {
            results.bottlenecks.push(`Runtime analysis error: ${error instanceof Error ? error.message : String(error)}`);
        }
        return results;
    }
            ...
            if (fs.existsSync("dist")) {
                try {
                    ...
                    if (results.bundleSize > 2.0) {
                        results.bottlenecks.push(`Large bundle size: ${results.bundleSize}MB > 2MB threshold`);
                        results.recommendations.push("Implement code splitting and tree shaking to reduce bundle size");
                    }
                }
                catch (error) {
                    // Size check failed
                ...
                results.recommendations.push("No build output found - run build process to analyze performance");
            }
        }
        catch (error) {
            results.bottlenecks.push(`Build analysis error: ${error instanceof Error ? error.message : String(error)}`);
        }
        return results;
    }
    async analyzeResourceUsage() {
        ...
        try {
            ...
            if (memUsagePercent > 90) {
                results.bottlenecks.push(`High system memory usage: ${Math.round(memUsagePercent)}%`);
                results.recommendations.push("Monitor memory-intensive processes and consider memory optimization");
            }
            ...
            results.bottlenecks.push(`Resource analysis error: ${error instanceof Error ? error.message : String(error)}`);
        }
        return results;
    }
    async detectBottlenecks() {
        ...
        try {
            ...
            results.bottlenecks.push(`Bottleneck detection error: ${error instanceof Error ? error.message : String(error)}`);
        }
        return results;
    }
    async analyzeSpecificBottlenecks(operation, threshold) {
        const results = {
            bottlenecks: [],
            recommendations: [],
            impact: "Low",
        ...
        try {
            ...
            if (operationTime > threshold) {
                results.bottlenecks.push(`${operation} exceeds threshold: ${operationTime}ms > ${threshold}ms`);
                results.impact = operationTime > threshold * 2 ? "High" : "Medium";
                results.critical = operationTime > threshold * 3;
                results.recommendations.push(`Optimize ${operation} performance - current time: ${operationTime}ms`);
            }
        }
        catch (error) {
            results.bottlenecks.push(`Operation analysis error: ${error instanceof Error ? error.message : String(error)}`);
    ...
    async identifyCommonBottlenecks() {
        const results = {
            bottlenecks: [],
            recommendations: [],
        };
        ...
        if (largeFiles.length > 0) {
            results.bottlenecks.push(`Large files detected: ${largeFiles.length} files > 10MB`);
            results.recommendations.push("Consider compressing or splitting large files");
        }
        ...
        if (fs.existsSync("package.json")) {
            try {
                ...
                if (depCount > 100) {
                    results.bottlenecks.push(`High dependency count: ${depCount} packages`);
                    results.recommendations.push("Audit and remove unnecessary dependencies");
                }
            }
            catch (error) {
    ...
    checkPerformanceThresholds(results) {
        ...
        return !bottlenecks.some((b) => criticalIndicators.some((indicator) => b.includes(indicator)));
    }
...
//# sourceMappingURL=performance-analysis.server.js.map