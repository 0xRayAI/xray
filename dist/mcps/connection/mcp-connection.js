...
export class McpConnection extends EventEmitter {
    ...
    async connect() {
        ...
        try {
            frameworkLogger.log('mcp-connection', `Connecting to ${this.serverName}`, 'info', { jobId });
            ...
            frameworkLogger.log('mcp-connection', `Connected to ${this.serverName}`, 'success', { jobId });
        }
        catch (error) {
            frameworkLogger.log('mcp-connection', `Failed to connect to ${this.serverName}: ${error instanceof Error ? error.message : String(error)}`, 'error', { jobId, error });
            this.cleanup();
            throw error;
        }
    }
    ...
    async disconnect() {
        ...
        frameworkLogger.log('mcp-connection', `Disconnecting from ${this.serverName}`, 'info', { jobId });
        ...
        frameworkLogger.log('mcp-connection', `Disconnected from ${this.serverName}`, 'info', { jobId });
    }
    /**
    ...
    handleStdout(data) {
        ...
        for (const line of lines) {
            ...
            try {
                ...
                frameworkLogger.log('mcp-connection', `Received non-JSON data: ${trimmedLine.substring(0, 100)}`, 'debug');
            }
        }
    }
    /**
    ...
    handleResponse(response) {
        ...
        if (!pending) {
            frameworkLogger.log('mcp-connection', `Received response for unknown request ID: ${response.id}`, 'warning');
            return;
        }
     ...
     */
    handleStderr(data) {
        frameworkLogger.log('mcp-connection', `stderr from ${this.serverName}: ${data.substring(0, 500)}`, 'debug');
     ...
     */
    handleError(error) {
        frameworkLogger.log('mcp-connection', `Process error for ${this.serverName}: ${error.message}`, 'error', { error: error.message });
        // Reject all pending requests
        Array.from(this.pendingRequests.entries()).forEach(([id, { reject }]) => {
     ...
     */
    handleClose(code) {
        frameworkLogger.log('mcp-connection', `Process for ${this.serverName} closed with code ${code}`, 'info');
        // Reject all pending requests
        Array.from(this.pendingRequests.entries()).forEach(([id, { reject }]) => {
