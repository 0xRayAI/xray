...
class StringRayLibrarianServer {
    ...
    constructor() {
        this.server = new Server({
            name: "researcher", version: "1.22.23",
        }, {
            capabilities: {
                tools: {},
            },
    ...
    async findImplementation(args) {
        ...
        try {
            ...
            const searchIn = (dir, depth = 0) => {
                if (depth > 5 || implementations.length >= 5)
                    return;
                if (!fs.existsSync(dir))
                    return;
                let entries;
    ...
    async getDocumentation(args) {
        ...
        try {
            ...
            for (const docPath of docFiles) {
                ...
                const lines = content.split("\n");
                const targetLines = lines.filter((line) => line.toLowerCase().includes(target.toLowerCase()));
...
//# sourceMappingURL=researcher.server.js.map