...
class BugTriageSpecialistServer {
    server;
    constructor() {
        this.server = new Server({ name: "bug-triage-specialist", version: "1.22.32" }, { capabilities: { tools: {} } });
    ...
    analyzeStackTrace(stackTrace, sourceMap) {
        ...
        return {
            ...
            analysis: {
                entryPoint: frames[0],
                lastUserCode: frames.find((f) => "file" in f &&
                    f.file &&
    ...
    suggestFixes(bugId, language, existingCode) {
        return {
            bugId,
            language,
            fix: {
    ...
    prioritizeIssues(bugs, sprintVelocity) {
    ...
    findRelatedIssues(newBug, existingBugs) {
    ...
    determineRootCause(log, category) {
        const causes = {
    ...
    assessImpact(severity, category) {
        if (severity === "critical")
            return "System outage or data loss";
        if (severity === "high")
            return "Major functionality broken";
        if (severity === "medium")
            return "Feature impaired but usable";
        return "Minor issue, low user impact";
    }
    estimateFixComplexity(category) {
        const complexities = {
            type: "simple",
        ...
        return complexities[category] || "moderate";
    }
    ...
    calculatePriorityScore(bug) {
        ...
        return ((severityWeight[bug.severity] || 10) +
            (impactWeight[bug.impact] || 10));
    }
    estimateEffort(bug) {
        const effortMap = {
            simple: 1,
...
server.run();
//# sourceMappingURL=bug-triage-specialist.server.js.map