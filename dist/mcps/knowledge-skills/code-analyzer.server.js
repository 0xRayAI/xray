...
class CodeAnalyzerServer {
    ...
    tools = [
        ...
        {
            name: "analyze_code",
            description: "Perform comprehensive static analysis on source code files",
            inputSchema: {
                type: "object",
                properties: {
                    files: {
        ...
        },
        {
            ...
            description: "Calculate cyclomatic complexity and other code metrics",
            inputSchema: {
                type: "object",
                properties: {
                    code: {
        ...
        },
        {
            ...
            description: "Detect code smells, anti-patterns, and potential issues",
            inputSchema: {
                type: "object",
                properties: {
                    files: {
        ...
        },
        {
            ...
            description: "Extract detailed code metrics (lines, functions, classes, etc.)",
            inputSchema: {
                type: "object",
                properties: {
                    filePath: {
        ...
        // ===== EXPLORE FEATURES =====
        {
            ...
            description: "Explore codebase structure and find files matching patterns. Fast file search.",
            inputSchema: {
                type: "object",
                properties: {
                    scope: {
        ...
        {
            name: "find_patterns",
            description: "Find code patterns, functions, classes, or specific text across the codebase",
            inputSchema: {
                type: "object",
                properties: {
                    pattern: {
        ...
        {
            name: "find_function",
            description: "Locate specific function definitions across the codebase",
            inputSchema: {
                type: "object",
                properties: {
                    functionName: {
        ...
        },
        {
            ...
            description: "Get directory structure and file tree for a given path",
            inputSchema: {
                type: "object",
                properties: {
                    path: {
        ...
        // ===== DEPENDENCY ANALYSIS =====
        {
            ...
            description: "Analyze import/export dependencies and generate dependency graph",
            inputSchema: {
                type: "object",
                properties: {
                    rootDir: {
    ...
    constructor() {
        this.server = new Server({ name: "code-analyzer", version: "1.22.42" }, { capabilities: { tools: {} } });
    ...
    calculateCodeMetrics(content, language) {
    ...
    handleCalculateComplexity(args) {
            ...
            try {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (inFunc) {
                        ...
                        braceCount -= (line.match(/}/g) || []).length;
                        if (i - funcStart > maxFunctionLength && braceCount === 0) {
                            smells.push({ file, type: "long-function", line: i + 1 });
                            inFunc = false;
                        }
                    }
                    if ((line.match(/{/g) || []).length > maxNestingDepth)
                        smells.push({ file, type: "deep-nesting", line: i + 1 });
                }
                const paramMatches = content.match(/function\s+\w+\s*\(([^)]*)\)/g);
                if (paramMatches)
                    for (const m of paramMatches) {
                        const params = m.replace(/function\s+\w+\s*\(/, "").replace(/\)/, "");
                        if (params.split(",").filter(p => p.trim()).length > maxParameters)
                            smells.push({ file, type: "too-many-params" });
                    }
            }
            catch { /* skip */ }
        }
    ...
    handleExtractMetrics(args) {
        const { filePath } = args;
        const metrics = { file: filePath, analyzedAt: new Date().toISOString() };
        try {
            const stat = fs.statSync(filePath);
            metrics.size = stat.size;
            if (stat.isFile())
                metrics.codeMetrics = this.calculateCodeMetrics(fs.readFileSync(filePath, "utf-8"), "typescript");
        }
        catch (e) {
            ...
            try {
                for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
                    ...
                    if (entry.isDirectory() && !["node_modules", ".git", "dist"].includes(entry.name))
                        walk(full, depth + 1);
                    else if (entry.isFile() && patterns.some((p) => this.matchesPattern(entry.name, p)))
            ...
            try {
                ...
                for (let i = 0; i < lines.length && results.length < maxMatches; i++) {
                    ...
                    if (line && regex.test(line))
                        results.push({ file: path.relative(root, file), line: i + 1, content: line.trim().substring(0, 100) });
                }
            }
            catch { /* skip */ }
        };
        const walk = (dir, depth = 0) => {
            if (results.length >= maxMatches || depth > 10)
                return;
            try {
                for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        const search = (file) => {
            try {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    for (const p of patterns) {
                        if (p.test(line)) {
                            results.push({ file, line: i + 1, context: lines[Math.max(0, i - 1)]?.trim().substring(0, 80) || "" });
                            break;
                        }
                    }
                }
    ...
    handleGetFileStructure(args) {
        ...
        const build = (dir, depth) => {
            if (depth > maxDepth)
                return null;
            try {
                const node = { type: "directory", name: path.basename(dir), children: [] };
                for (const entry of fs.readdirSync(dir, { withFileTypes: true }).filter(e => e.isDirectory() ? !["node_modules", ".git", "dist"].includes(e.name) : includeFiles).sort((a, b) => (a.isDirectory() ? -1 : 1) - (b.isDirectory() ? -1 : 1))) {
                    ...
                    if (entry.isDirectory()) {
                        const child = build(full, depth + 1);
                        if (child)
                            node.children.push(child);
                    }
                    else
                        try {
                            node.children.push({ type: "file", name: entry.name, size: fs.statSync(full).size });
                        }
                        catch { /* skip */ }
                }
                return node;
                ...
                return { type: "error", name: path.basename(dir) };
            }
        };
    ...
    handleFindDuplicates(args) {
        const { directory, minLength = 10 } = args;
        const blocks = new Map();
        const extract = (content) => {
            const b = [], lines = content.split("\n");
            let current = [];
...
if (import.meta.url === `file://${process.argv[1]}`) {
    new CodeAnalyzerServer().run().catch(() => { });
}
export default CodeAnalyzerServer;
//# sourceMappingURL=code-analyzer.server.js.map