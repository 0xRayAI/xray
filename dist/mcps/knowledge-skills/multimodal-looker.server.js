...
class MultimodalLookerServer {
    server;
    constructor() {
        this.server = new Server({ name: "multimodal-looker", version: "1.22.33" }, { capabilities: { tools: {} } });
            ...
            tools: [
                {
                    ...
                    description: "Extract UI elements from screenshots. Identifies components, layout structure, and provides detailed element analysis for code generation.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            imageData: {
                ...
                },
                {
                    ...
                    description: "Parse architecture and flow diagrams. Extracts nodes, edges, relationships for understanding system design.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            imageData: {
                ...
                },
                {
                    ...
                    description: "Evaluate UI mockups against best practices. Checks consistency, accessibility, and provides improvement suggestions.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            imageData: {
                ...
                },
                {
                    ...
                    description: "Convert visual content to design specifications. Extracts colors, spacing, typography, and other design tokens.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            imageData: {
                ...
                },
                {
                    ...
                    description: "Compare two designs for consistency. Identifies differences in colors, layout, typography, and components.",
                    inputSchema: {
                        type: "object",
                        properties: {
                            imageA: {
     ...
     * Tool Handlers
     * -------------------------------------------------------------------------- */
    handleAnalyzeScreenshot(params) {
        const imageData = params.imageData || "";
        const extractComponents = params.extractComponents ?? true;
        const checkAccessibility = params.checkAccessibility ?? true;
        const analyzeLayout = params.analyzeLayout ?? true;
        const result = this.analyzeScreenshot(imageData, extractComponents, checkAccessibility, analyzeLayout);
        return {
            content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
        };
    }
    handleDescribeDiagram(params) {
        const imageData = params.imageData || "";
        const diagramType = params.diagramType || "auto";
    ...
    handleReviewUIMockup(params) {
        const imageData = params.imageData || "";
        const framework = params.framework || "react";
        const checkResponsive = params.checkResponsive ?? true;
        const result = this.reviewUIMockup(imageData, framework, checkResponsive);
        return {
            content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
        };
    }
    handleExtractVisualSpecs(params) {
        const imageData = params.imageData || "";
    ...
    handleCompareDesigns(params) {
        const imageA = params.imageA || "";
        const imageB = params.imageB || "";
     ...
     * -------------------------------------------------------------------------- */
    analyzeScreenshot(imageData, extractComponents, checkAccessibility, analyzeLayout) {
    ...
    reviewUIMockup(imageData, framework, checkResponsive) {
        ...
        const overallScore = Math.round((accessibility.score * 0.3 + consistency.score * 0.25 + responsiveness.score * 0.25 + usability.score * 0.2));
        const findings = [
    ...
    extractVisualSpecs(imageData, includeValues, format) {
    ...
    compareDesigns(imageA, imageB, tolerance) {
    ...
    extractUIComponents(imageData) {
        ...
        if (components.length === 0) {
            components.push({ id: "el-0", type: "header", position: { x: 0, y: 0, width: 1200, height: 60 } }, { id: "el-1", type: "main", position: { x: 0, y: 60, width: 1200, height: 540 } });
        }
        return components;
    }
    ...
    inferNodeType(label, diagramType) {
        const lower = label.toLowerCase();
        if (lower.includes("start") || lower.includes("end"))
            return "terminal";
        if (lower.includes("decision") || lower.includes("if") || lower.includes("?"))
            return "decision";
        if (lower.includes("data") || lower.includes("db") || lower.includes("database"))
            return "data";
        if (lower.includes("user") || lower.includes("actor") || lower.includes("client"))
            return "actor";
        if (lower.includes("service") || lower.includes("api") || lower.includes("server"))
            return "component";
        return "process";
    }
    extractDiagramEdges(data, nodes) {
        const edges = [];
        const arrowPatterns = [
    ...
    calculateConfidence(nodes, edges) {
        let confidence = 50;
    ...
    generateDiagramInsights(type, nodes, edges) {
        const insights = [];
        if (type === "flowchart") {
            const hasDecision = nodes.some((n) => n.type === "decision");
            if (hasDecision)
                insights.push("Contains conditional logic");
            const terminals = nodes.filter((n) => n.type === "terminal");
            if (terminals.length >= 2)
                insights.push("Has clear start and end points");
        }
        if (type === "sequence") {
            ...
            if (biDirectional > 0)
                insights.push("Contains bidirectional communication");
        }
        if (type === "architecture") {
            insights.push(`System consists of ${nodes.length} components`);
            const services = nodes.filter((n) => n.type === "component").length;
            if (services > 3)
                insights.push("Microservices architecture detected");
        }
        if (type === "erd") {
            insights.push(`Data model with ${nodes.length} entities`);
            const relationships = edges.length;
            if (relationships > 0)
                insights.push(`Contains ${relationships} relationships`);
        }
        if (nodes.length > 10)
            insights.push("Complex diagram - consider breaking into smaller parts");
        return insights;
    }
    analyzeLayoutStructure(components, data) {
        ...
        return { type: layoutType, sections, gridColumns };
    }
    inferGridColumns(components) {
    ...
    detectColorScheme(data) {
        ...
        if (darkCount > lightCount * 1.5)
            return "dark";
        if (lightCount > darkCount * 1.5)
            return "light";
        return "mixed";
    }
    performAccessibilityAudit(components, data) {
        const issues = [];
        const lower = data.toLowerCase();
    ...
    assessUsability(data, components) {
        const issues = [];
        const lower = data.toLowerCase();
    ...
    generateMockupRecommendations(findings, framework) {
    ...
    generateRecommendations(components, issues, layout) {
        const recommendations = [];
    ...
    extractColors(data) {
        ...
        if (colors.length < 4) {
            colors.push({ name: "primary", value: "#007bff", role: "Primary actions" }, { name: "secondary", value: "#6c757d", role: "Secondary elements" }, { name: "success", value: "#28a745", role: "Success states" }, { name: "danger", value: "#dc3545", role: "Error states" });
        }
        return colors;
    }
    extractSpacing(data) {
        ...
        if (spacing.length < 4) {
            spacing.push({ name: "xs", value: "4px", usage: "Tight spacing" }, { name: "sm", value: "8px", usage: "Small gaps" }, { name: "md", value: "16px", usage: "Standard spacing" }, { name: "lg", value: "24px", usage: "Large gaps" });
        }
        return spacing;
    }
    extractTypography(data) {
        const typography = [];
        const typeMap = {
        ...
        if (typography.length < 4) {
            typography.push({ name: "body", size: "16px", weight: "400", lineHeight: "1.5", usage: "Body text" }, { name: "h3", size: "24px", weight: "600", lineHeight: "1.3", usage: "Headings" }, { name: "caption", size: "12px", weight: "400", lineHeight: "1.4", usage: "Captions" });
        }
        return typography;
    }
    extractShadows(data) {
        ...
        if (shadows.length === 0) {
            shadows.push("0 1px 3px rgba(0, 0, 0, 0.12)", "0 4px 6px rgba(0, 0, 0, 0.1)", "0 10px 20px rgba(0, 0, 0, 0.15)");
        }
        return shadows;
    }
    extractBorderRadius(data) {
        const radius = [];
    ...
    convertToTailwindTokens(colors, spacing, typography) {
        const colorObj = {};
    ...
    compareComponents(componentsA, componentsB) {
        const differences = [];
    ...
    compareTypography(typographyA, typographyB) {
        const differences = [];
    ...
    calculateSimilarityScore(differences, tolerance) {
        const severityWeights = { critical: 25, major: 15, minor: 5 };
        const totalPenalty = differences.reduce((acc, d) => acc + severityWeights[d.severity], 0);
        return Math.max(0, Math.min(100, 100 - totalPenalty + tolerance));
    }
    generateComparisonRecommendations(differences) {
        const recommendations = [];
...
//# sourceMappingURL=multimodal-looker.server.js.map