...
class StrategistServer {
    ...
    constructor() {
        this.server = new Server({
            name: "strray/strategist", version: "1.22.36",
        }, {
            capabilities: {
                tools: {},
            },
    ...
    async handleStrategicGuidance(args) {
        const { context, options = [], criteria = [] } = args;
        const analysis = [
            `## Strategic Analysis for: ${context}`,
            "",
            "### Key Considerations:",
            ...criteria.map((c) => `- ${c}: Need to evaluate against this criterion`),
            "",
    ...
    async handleArchitectureReview(args) {
        ...
        const analysis = [
            ...
            "",
            "### Considerations:",
            ...requirements.map((r) => `- ${r}: Ensure this requirement is met`),
            "",
            "### Recommendations:",
...
//# sourceMappingURL=strategist.server.js.map