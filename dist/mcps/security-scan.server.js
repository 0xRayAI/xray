...
class StringRaySecurityScanServer {
    ...
    constructor() {
        this.server = new Server({
            name: "security-scan", version: "1.22.35",
        }, {
            capabilities: {
                tools: {},
            },
...
${results.recommendations.map((r) => `• ${r}`).join("\n") || "No recommendations"}`,
    ...
    detectPackageManager(requested) {
        if (requested !== "auto")
            return requested;
    ...
    async scanDependencies(auditLevel, includeOutdated, packageManager) {
        ...
        try {
            ...
            try {
                const auditArgs = packageManager === "yarn"
                ...
                if (auditData.vulnerabilities &&
                    ...
                    results.secure = false;
                    results.vulnerabilities.push(`${Object.keys(auditData.vulnerabilities).length} ${pmCmd} vulnerabilities found`);
                    results.recommendations.push(`Run "${pmCmd} audit fix" to address vulnerabilities`);
                }
            }
            catch (error) {
                ...
                if (auditData &&
                    ...
                    if (errorMsg.includes("vulnerabilities") || errorMsg.includes("ELIFECYCLE")) {
                        ...
                        results.recommendations.push(`Run "${pmCmd} audit" for details and "${pmCmd} audit fix" to resolve`);
                    }
                }
            }
    ...
    findCodeFiles(maxDepth = 10) {
        ...
        const langConfig = projectLanguage
            ? LANGUAGE_CONFIGS.find((c) => c.language === projectLanguage.language)
            : null;
    ...
    analyzeFileForSecurity(content, filePath) {
        const issues = {
            vulnerabilities: [],
            threats: [],
            recommendations: [],
        ...
        if (/(__tests__|__mocks__|\.test\.|\.spec\.|_test\.|_spec\.|tests?\/|specs?\/|\.stories\.)/i.test(normalizedPath) ||
        ...
        for (const pattern of patterns) {
            ...
            if (pattern.exclude && pattern.exclude.test(filteredContent)) {
                ...
                const excludePositions = new Set(excludeMatches.map((m) => m.index));
                const validMatches = allMatches.filter((m) => !excludePositions.has(m.index));
...
//# sourceMappingURL=security-scan.server.js.map