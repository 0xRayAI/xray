        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    if (errorMatches?.[1])
                        errorCount = parseInt(errorMatches[1]);
                    if (warningMatches?.[1])
                        warningCount = parseInt(warningMatches[1]);
                }
                ...
                if (!parsedJson) {
                    ...
                    if (errorMatches)
                        results.issues.errors = parseInt(errorMatches[1]);
                    if (warningMatches)
        ...
        try {
            if (rules.length === 0) {
                ...
                for (const rule of rules) {
                    try {
                        execFileSync("npx", ["eslint", ...fileArgs, "--rule", `${rule}:error`, "--format", "json"], {
                            encoding: "utf8",
                            cwd: process.cwd(),
                            stdio: "pipe",
                            timeout: 30000,
                        ...
                        const errorOutput = error instanceof Error
                            ...
                            : String(error);
                        results.details.push(`Rule ${rule}: FAIL - ${errorOutput.substring(0, 200)}`);
                    }
                }
            }
        }
