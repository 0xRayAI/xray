...
class StringRayFrameworkComplianceAuditServer {
    ...
    constructor() {
        this.server = new Server({
            name: "framework-compliance-audit", version: "1.22.35",
        }, {
            capabilities: {
                tools: {},
            },
        ...
        void frameworkLogger.log("framework-compliance-audit.server", "-strray-framework-compliance-audit-mcp-server-init", "info", { message: "0xRay Framework Compliance Audit MCP Server initialized" });
    }
    setupToolHandlers() {
    ...
    async handleFrameworkComplianceAudit(args) {
        ...
        await frameworkLogger.log("framework-compliance-audit.server", "-mcp-performing-framework-compliance-audit-scope-d", "info", {
...
**Overall Status:** ${auditResults.passed ? "✅ COMPLIANT" : "❌ NON-COMPLIANT"}`;
    ...
    async handleCodexValidation(args) {
        ...
        await frameworkLogger.log("framework-compliance-audit.server", "-mcp-performing-codex-validation-terms-terms-lengt", "info", {
...
**Status:** ${results.passed ? "✅ COMPLIANT" : "❌ VIOLATIONS DETECTED"}`,
    ...
    async auditConfigurationIntegrity() {
        const results = {
            passed: true,
            issues: [],
            score: "PASS",
    ...
    async auditPerformanceThresholds() {
        const results = {
            passed: true,
            warnings: [],
            score: "PASS",
    ...
    async validateCodexTerms(specificTerms, strict) {
        const results = {
            validatedCount: 0,
            totalTerms: 43,
        ...
        try {
            const termsToValidate = specificTerms.length > 0
                ? specificTerms
    ...
    checkTermImplementation(term, content) {
...
- Compliance Areas: ${Object.keys(results.complianceScores).length}`;
    ...
    async getDetailedFindings(results) {
    ...
    async run() {
        ...
        void frameworkLogger.log("framework-compliance-audit.server", "-strray-framework-compliance-audit-mcp-server-star", "info", { message: "0xRay Framework Compliance Audit MCP Server started" });
    }
}
...
//# sourceMappingURL=framework-compliance-audit.server.js.map