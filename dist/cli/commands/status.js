...
function getSkillsList(cwd) {
    const skills = [];
    const configDir = getConfigDir(cwd);
    const integrationsPath = join(configDir, "integrations");
    if (existsSync(integrationsPath)) {
        const integrationDirs = readdirSync(integrationsPath).filter((f) => existsSync(join(integrationsPath, f, "SKILL.md")));
        skills.push(...integrationDirs);
    }
    const skillsPath = join(configDir, "skills");
    if (existsSync(skillsPath)) {
        const skillDirs = readdirSync(skillsPath).filter((f) => existsSync(join(skillsPath, f, "SKILL.md")));
...
function getInferenceStatus(cwd) {
    let active = false;
    let lastTuning = null;
    let outcomesCount = 0;
    let patternsCount = 0;
...
export function getStatusReport(cwd = process.cwd()) {
...
export function printStatus(report) {
    ...
    console.log(`⚙️  Inference: ${inferenceStatus} (${report.inference.outcomesCount} outcomes, ${report.inference.patternsCount} patterns)`);
