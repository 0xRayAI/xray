...
export async function executeTestExecution(context) {
...
export async function executeTypeScriptTests(context, cwd) {
    let testPattern = "";
    if (context.filePath) {
...
export async function executeGenericTests(context, cwd, projectLanguage) {
    let testFilePath;
    if (context.filePath) {
...
export async function runTestCommand(command, cwd) {
    let stdout = "";
    let stderr = "";
    let exitCode = 0;
    try {
...
export function parseTestOutput(output, type) {
    const patterns = [
...
//# sourceMappingURL=processor-test-executor.js.map