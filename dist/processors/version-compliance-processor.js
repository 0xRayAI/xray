...
export class VersionComplianceProcessor {
    ...
    async execute(context) {
        try {
            ...
            if (!relevantOperations.includes(context.tool) &&
            ...
            if (!result.compliant) {
                await frameworkLogger.log("version-compliance-processor", "-version-compliance-failed-", "error", {
            ...
            // Log success
            await frameworkLogger.log("version-compliance-processor", "-version-compliance-passed-", "info", {
                npmVersion: result.npmVersion,
                uvmVersion: result.uvmVersion,
                pkgVersion: result.pkgVersion,
                warnings: result.warnings,
            ...
            await frameworkLogger.log("version-compliance-processor", "-validation-error-", "error", { message: errorMessage });
            return {
                success: false,
                blocked: true,
     ...
     */
    async validateVersionCompliance() {
        this.errors = [];
        this.warnings = [];
        this.fixes = [];
        // Get all versions
        ...
        if (npmVersion !== "NOT_PUBLISHED" && npmVersion !== "ERROR") {
            ...
            if (uvmVersion !== this.formatVersion(expectedUvm)) {
                this.errors.push(`Version manager not 1 ahead of npm (NPM: ${npmVersion}, UVM: ${uvmVersion}, Expected: ${this.formatVersion(expectedUvm)})`);
                this.fixes.push({
                    type: "update-uvm",
        ...
        if (pkgVersion !== uvmVersion) {
            this.warnings.push(`package.json version (${pkgVersion}) doesn't match UVM (${uvmVersion}) - run "npm version [patch|minor|major]" to sync`);
        }
        ...
        if (sourceVersion && sourceVersion !== uvmVersion) {
            this.errors.push(`Source files not synchronized to UVM version (${sourceVersion} vs ${uvmVersion})`);
            this.fixes.push({
                type: "sync-source",
        ...
        if (readmeVersion &&
            ...
            this.warnings.push(`README version (${readmeVersion}) may be outdated (UVM: ${uvmVersion}, package: ${pkgVersion})`);
            this.fixes.push({
                type: "update-readme",
