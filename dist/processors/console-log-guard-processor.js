...
// ---------------------------------------------------------------------------
export async function runConsoleLogGuard(context) {
    const start = performance.now();
    try {
        const content = context.data ?? "";
        const filePath = context.filesChanged?.[0];
        const processor = new ConsoleLogGuardProcessor();
        const violations = processor.checkCode(content, filePath);
        const hasViolations = violations.length > 0;
        if (hasViolations) {
            frameworkLogger.log("console-log-guard-processor", "violations_found", "warning", { violations, filePath });
        }
        return {
        ...
        frameworkLogger.log("console-log-guard-processor", "check_failed", "error", { error: error.message });
        return {
            success: false,
            processorName: "console-log-guard-processor",
            duration: performance.now() - start,
...
//# sourceMappingURL=console-log-guard-processor.js.map