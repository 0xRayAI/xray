...
import { getTestFilePath, LANGUAGE_CONFIGS, } from "../utils/language-detector.js";
...
function findRecentlyModifiedTsFile(dir, maxAgeSeconds = 60) {
    // Check cache first
...
export const testAutoCreationProcessor = {
    ...
    async execute(context) {
        ...
        try {
            ...
            const { tool, args, directory, filePath: contextFilePath, operation, } = innerContext;
            ...
            if (!filePath) {
                await frameworkLogger.log("test-auto-creation", "skipped-no-filepath", "info", {
            ...
            if (!isWriteOperation) {
                await frameworkLogger.log("test-auto-creation", "skipped-not-write", "info", { message: `Skipped: not a write operation`, tool, operation });
                return {
                    success: true,
                    processorName: "testAutoCreation",
                    duration: Date.now() - startTime,
            ...
            const testFilePath = getTestFilePath(filePath, langConfig?.language || "TypeScript");
            ...
            if (!fullSourcePath.startsWith(checkDir + path.sep) && fullSourcePath !== checkDir) {
                await frameworkLogger.log("test-auto-creation", "skipped-path-traversal", "warning", {
            ...
            if (!fs.existsSync(fullSourcePath)) {
                await frameworkLogger.log("test-auto-creation", "skipped-source-not-found", "info", {
            ...
            await frameworkLogger.log("test-auto-creation", "generating-tests", "info", {
            ...
            await frameworkLogger.log("test-auto-creation", "test-stub-created", "success", {
 ...
 */
async function createBasicTestStub(testFilePath, sourceFile, exports) {
 ...
 */
function generateTestStub(importPath, exports, sourceFile) {
 ...
 */
function generateSampleArgs(params) {
    if (!params.trim())
        return "";
 ...
 */
function generateReturnCheck(returnType) {
