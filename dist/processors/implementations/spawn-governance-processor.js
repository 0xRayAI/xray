...
export class SpawnGovernanceProcessor {
    ...
    static DEFAULT_RATE_LIMIT_WINDOW_MS = 10000;
    static DEFAULT_MAX_SPAWNS_PER_WINDOW = 10;
    static DEFAULT_MEMORY_THRESHOLD = 0.8;
    config;
    activeSpawns = new Set();
    spawnTimestamps = [];
    agentSpawnTimestamps = new Map();
    blockedCount = 0;
    ...
    // Core API
    // -----------------------------------------------------------------------
    checkSpawnAllowed(agentName) {
        const now = Date.now();
        // 1. Concurrent agent limit (#54)
        if (this.activeSpawns.size >= this.config.maxConcurrent) {
            this.blockedCount++;
            frameworkLogger.log("spawn-governance", "concurrent-limit-exceeded", "warning", {
                agentName,
        ...
        if (depth > 0) {
            this.blockedCount++;
            frameworkLogger.log("spawn-governance", "recursive-subagent-blocked", "warning", { agentName, depth });
        ...
        const recentAgentSpawns = agentTimes.filter((t) => now - t < this.config.infiniteSpawnWindowMs);
        if (recentAgentSpawns.length >= this.config.infiniteSpawnThreshold) {
            this.blockedCount++;
            frameworkLogger.log("spawn-governance", "infinite-spawn-pattern-detected", "warning", {
                agentName,
        ...
        if (this.spawnTimestamps.length >= this.config.maxSpawnsPerWindow) {
            this.blockedCount++;
            frameworkLogger.log("spawn-governance", "rate-limit-exceeded", "warning", {
                agentName,
        ...
        if (memRatio > this.config.memoryThreshold) {
            frameworkLogger.log("spawn-governance", "memory-threshold-exceeded", "warning", {
                agentName,
    ...
    recordSpawn(agentName) {
        ...
        frameworkLogger.log("spawn-governance", "spawn-recorded", "info", { agentName, activeSpawns: this.activeSpawns.size });
    }
    recordSpawnComplete(agentName) {
        this.activeSpawns.delete(agentName);
        this.subagentDepth.delete(agentName);
        frameworkLogger.log("spawn-governance", "spawn-completed", "info", { agentName, activeSpawns: this.activeSpawns.size });
    }
    // -----------------------------------------------------------------------
    // Metrics
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    emergencyCleanup() {
        frameworkLogger.log("spawn-governance", "emergency-cleanup-started", "warning", { activeSpawns: this.activeSpawns.size, recentSpawns: this.spawnTimestamps.length });
        ...
        frameworkLogger.log("spawn-governance", "emergency-cleanup-completed", "info", {});
    }
    // -----------------------------------------------------------------------
    // Subagent depth management (for recursive prevention)
    // -----------------------------------------------------------------------
    setSubagentDepth(agentName, depth) {
...
// ---------------------------------------------------------------------------
export async function runSpawnGovernance(context) {
    const processor = new SpawnGovernanceProcessor(context.config);
...
//# sourceMappingURL=spawn-governance-processor.js.map