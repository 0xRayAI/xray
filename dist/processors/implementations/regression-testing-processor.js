...
export class RegressionTestingProcessor extends PostProcessor {
    ...
    async run(context) {
        ...
        try {
            ...
            const baseline = JSON.parse(fs.readFileSync(testCountPath, "utf-8"));
            ...
            if (result.regressions > 0) {
                frameworkLogger.log("regression-testing-processor", "regressions-detected", "warning", { regressions: result.regressions, issues: result.issues });
            }
            else {
                frameworkLogger.log("regression-testing-processor", "no-regressions", "info", { baseline: result.baseline, current: result.current });
            }
        }
        catch (error) {
            frameworkLogger.log("regression-testing-processor", "error", "error", { error: error instanceof Error ? error.message : String(error) });
        }
        return result;
    }
