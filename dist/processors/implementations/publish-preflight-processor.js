...
export class PublishPreflightProcessor extends PostProcessor {
    ...
    loadConfig() {
        try {
            ...
            frameworkLogger.log("publish-preflight-processor", "config-load-failed", "warning", { error: e instanceof Error ? e.message : String(e) });
        }
    }
    async run(context) {
        const projectRoot = process.cwd();
        await frameworkLogger.log("publish-preflight-processor", "starting", "info", { operation: context.operation });
        const checks = [];
        ...
        let requiredPipelineTests = pipelineConfig.min_pipeline_tests;
        if (fs.existsSync(pipelineInventoryPath)) {
            try {
                ...
                frameworkLogger.log("publish-preflight-processor", "inventory-parse-failed", "warning", { error: e instanceof Error ? e.message : String(e) });
            }
        }
        ...
        if (!compliant) {
            await frameworkLogger.log("publish-preflight-processor", "failed", "error", { failedChecks: failedRequired.map(c => c.name) });
        }
        return {
            compliant,
            checks,
...
//# sourceMappingURL=publish-preflight-processor.js.map