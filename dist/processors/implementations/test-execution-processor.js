...
export class TestExecutionProcessor extends PostProcessor {
    ...
    async run(context) {
        ...
        const data = ctx.data;
        await frameworkLogger.log("test-execution-processor", "executing-tests", "info", { operation, filePath: filePath?.slice(0, 100) });
        try {
            // Lazy load required modules
            const { exec } = await import("child_process");
            const { promisify } = await import("util");
            const { detectProjectLanguage } = await import("../../utils/language-detector.js");
            const execAsync = promisify(exec);
            const cwd = process.cwd();
            // Detect project language
            const projectLanguage = detectProjectLanguage(cwd);
            if (!projectLanguage) {
                await frameworkLogger.log("test-execution-processor", "language-detection-failed", "info", { message: "Could not detect project language" });
                return {
                    testsExecuted: 0,
                    passed: 0,
                    failed: 0,
            ...
            await frameworkLogger.log("test-execution-processor", "language-detected", "info", {
            ...
            if (projectLanguage.language === "TypeScript" ||
                ...
                testCommand = this.buildGenericTestCommand(projectLanguage, filePath);
            }
            await frameworkLogger.log("test-execution-processor", "running-tests", "info", { command: testCommand });
            // Execute tests
            let stdout = "";
            let stderr = "";
            let exitCode = 0;
            ...
            await frameworkLogger.log("test-execution-processor", "tests-completed", success ? "success" : "error", {
                total,
                passed,
                failed,
                exitCode,
            ...
            await frameworkLogger.log("test-execution-processor", "execution-error", "error", { error: errorMessage });
            return {
                testsExecuted: 0,
                passed: 0,
                failed: 0,
    ...
    async buildTypeScriptTestCommand(filePath) {
        if (filePath) {
    ...
    buildGenericTestCommand(projectLanguage, filePath) {
...
//# sourceMappingURL=test-execution-processor.js.map