...
export class ConsentManager {
     ...
     */
    async enableConsent(categories) {
        if (!this.config) {
        ...
        categoriesToEnable.forEach(cat => {
            if (this.config && cat in this.config.categories) {
                this.config.categories[cat] = true;
            }
        });
    ...
    async disableConsent() {
        ...
        Object.keys(this.config.categories).forEach(cat => {
            this.config.categories[cat] = false;
        });
        await this.saveConfig();
    ...
    async loadConfig() {
        try {
            ...
            if (this.config.consentDate) {
                this.config.consentDate = new Date(this.config.consentDate);
            }
            if (this.config.lastOptOut) {
                this.config.lastOptOut = new Date(this.config.lastOptOut);
            }
        }
        catch (error) {
