 ...
 * @since 2026-03-05
 */
import { promptPatternAnalyzer, } from "./prompt-pattern-analyzer.js";
...
class RoutingRefiner {
    ...
    generateRefinementReport() {
        ...
        const configurationUpdate = this.generateConfigurationUpdate(promptAnalysis, performanceReport);
     ...
     */
    generateConfigurationUpdate(promptAnalysis, performanceReport) {
        const newMappings = this.suggestNewKeywordMappings(promptAnalysis, performanceReport);
        ...
        return {
            ...
            summary: {
                ...
                estimatedImprovement: this.estimateImprovement(newMappings, optimizations),
            },
            newMappings,
            optimizations,
            warnings,
     ...
     */
    suggestNewKeywordMappings(promptAnalysis, performanceReport) {
        const suggestions = [];
        for (const pattern of promptAnalysis.emergingPatterns) {
            if (pattern.frequency >= this.minSamplesForSuggestion &&
                ...
                for (const keyword of topKeywords) {
                    suggestions.push({
                        ...
                        suggestedConfidence: Math.min(pattern.avgConfidence + 0.05, 0.95),
        ...
        for (const gap of promptAnalysis.gaps) {
            if (gap.frequency >= this.minSamplesForSuggestion &&
        ...
        for (const missedKeyword of promptAnalysis.topMissedKeywords.slice(0, 15)) {
            if (missedKeyword.count >= this.minSamplesForSuggestion &&
                missedKeyword.suggestedMappings.length > 0) {
                const targetAgent = this.selectBestAgentForMapping(missedKeyword.suggestedMappings, performanceReport.agentMetrics);
                suggestions.push({
     ...
     */
    suggestMappingOptimizations(promptAnalysis, performanceReport) {
        const optimizations = [];
     ...
     */
    generateWarnings(newMappings, optimizations) {
        const warnings = [];
        const lowPrioritySuggestions = newMappings.filter((m) => m.priority === "low");
        if (lowPrioritySuggestions.length > newMappings.length * 0.5) {
            warnings.push(`High proportion (${((lowPrioritySuggestions.length / newMappings.length) * 100).toFixed(1)}%) of suggestions are low priority - review carefully before applying`);
        }
        const agentConflictMap = new Map();
        ...
        for (const [keyword, mappings] of agentConflictMap.entries()) {
            ...
            if (uniqueAgents.size > 1) {
                warnings.push(`Keyword "${keyword}" suggested for multiple agents: ${Array.from(uniqueAgents).join(", ")}. Manual review required.`);
            }
        }
        if (optimizations.length > 20) {
            warnings.push("Large number of optimization suggestions - consider applying in batches");
        }
        return warnings;
    }
    /**
     ...
     */
    generateImplementationSteps(configurationUpdate) {
        const steps = [];
    ...
    generateFormattedReport() {
        ...
        lines.push("");
        lines.push("=== Summary ===");
        lines.push(`New Mappings: ${report.configurationUpdate.summary.newMappings}`);
        lines.push(`Optimized Mappings: ${report.configurationUpdate.summary.optimizedMappings}`);
        lines.push(`Removed Mappings: ${report.configurationUpdate.summary.removedMappings}`);
        lines.push(`Estimated Improvement: ${report.configurationUpdate.summary.estimatedImprovement}`);
        lines.push("");
    ...
    calculatePriority(confidence, successRate, frequency) {
    ...
    selectBestAgentForMapping(agents, agentMetrics) {
        const agentPerformance = new Map(agentMetrics.map((m) => [m.agent, m.successRate]));
    ...
    deduplicateSuggestions(suggestions) {
        ...
        for (const suggestion of suggestions) {
            ...
            if (!seen.has(key)) {
                ...
                if (suggestion.evidence.frequency > existing.evidence.frequency ||
    ...
    calculateRemovals(optimizations) {
        return optimizations.filter((o) => o.optimizationType === "remove_keywords").length;
    }
    estimateImprovement(newMappings, optimizations) {
        const highPriorityMappings = newMappings.filter((m) => m.priority === "high");
        const confidenceAdjustments = optimizations.filter((o) => o.optimizationType === "adjust_confidence").length;
        const estimatedAccuracyImprovement = (highPriorityMappings.length * 2) + (confidenceAdjustments * 1);
...
//# sourceMappingURL=routing-refiner.js.map