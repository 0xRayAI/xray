    ...
    learn(outcome) {
        ...
        for (const [id, assumption] of this.assumptions.entries()) {
            ...
            if (matched) {
                ...
                const current = this[key] ?? 0.5;
                this[key] = Math.min(current + 0.05, 1.0);
            }
        }
        ...
        for (const [id] of this.assumptions.entries()) {
            ...
            if (!matched) {
                const current = this[key] ?? 0.5;
                this[key] = Math.max(current - 0.02, 0.1);
            }
        }
        ...
        if (outcome.success) {
            ...
            const current = this[key];
            if (current !== undefined) {
                this[key] = Math.max(current - 0.1, 0.1);
            }
        }
    }
    getConfig() {
...
//# sourceMappingURL=kernel-patterns.js.map