...
import { getKernel } from "./kernel-patterns.js";
export class KernelOrchestrator {
    ...
    async executeTask(task) {
        ...
        if (kernelInsights.cascadePatterns?.some(p => p.id === 'P7')) {
            ...
            if (p7Pattern) {
                frameworkLogger.log("orchestrator", "kernel-guided-release-readiness", "info", {
                    taskId,
                    taskType: task.type,
        ...
        try {
            ...
            const result = await this.delegateToSubagent(task.subagentType || "default-agent", task);
            ...
            const resultValue = typeof result.result === "object"
                ? result.result
                : { value: result.result };
            return {
                success: true,
    ...
    async executeSingleTask(task, jobId) {
        ...
        try {
            ...
            const duration = Date.now() - startTime;
            const errorMessage = error instanceof Error ? error.message : String(error);
            await frameworkLogger.log("orchestrator", "complex-task-failed", "error", { jobId, taskId: task.id, error: errorMessage });
            return {
                success: false,
                taskId: task.id,
                agentUsed: task.subagentType || "default-agent",
    ...
    validateTaskDependencies(tasks) {
        ...
        for (const task of tasks) {
            if (task.dependencies && task.dependencies.length > 0) {
                for (const dep of task.dependencies) {
                    if (!taskIds.has(dep)) {
                        errors.push(`Task "${task.id}" depends on "${dep}" which is NOT in this orchestrator's task batch.\n` +
        ...
        if (errors.length > 0) {
            const errorMessage = `[TEST ARCHITECTURE ERROR] Cross-orchestrator dependencies detected.\n\n` +
                `${errors.join("\n\n")}\n\n` +
                `This usually means:\n` +
    ...
    async executeComplexTaskSingle(task) {
        const taskId = this.generateTaskId();
        this.taskQueue.set(taskId, task);
        this.activeTasks.add(taskId);
        frameworkLogger.log("orchestrator", "complex-task-started", "info", {
    ...
    resolveConflicts(conflicts) {
        ...
        const bestConflict = conflicts.reduce((best, current) => (current.expertiseScore || 0) > (best.expertiseScore || 0)
            ? current
            : best);
        return {
     ...
     */
    async delegateToSubagent(agentName, task) {
        ...
        try {
            const result = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Delegation to ${agentName} timed out after ${timeoutMs}ms`)), timeoutMs)),
            ]);
            return result;
        }
        catch (error) {
    ...
    async performDelegation(agentName, task) {
