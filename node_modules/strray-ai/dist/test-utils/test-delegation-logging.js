...
async function testDelegationLogging() {
    ...
    try {
        ...
        await frameworkLogger.log("test-delegation", "execution-completed", "success", {}, undefined, // sessionId
        jobId);
    }
    catch (error) {
        console.log("⚠️  Delegation execution failed (expected in test environment):", error instanceof Error ? error.message : String(error));
    }
    await frameworkLogger.log("test-delegation", "test-completed", "success", {}, undefined, jobId);
}
testDelegationLogging();
//# sourceMappingURL=test-delegation-logging.js.map