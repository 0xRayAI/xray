 ...
 * @since 2026-01-07
 */
import { DEFAULT_THRESHOLDS, OPERATION_WEIGHTS, RISK_MULTIPLIERS, getLevelFromScore, getStrategyForLevel, getAgentCountForLevel, generateReasoning, } from "./complexity-core.js";
/**
 * Complexity Analyzer
...
export class ComplexityAnalyzer {
    ...
    setOperationWeights(weights) {
        for (const [key, value] of Object.entries(weights)) {
            if (typeof value === "number") {
                this.operationWeights[key] = value;
            }
        }
    }
    /**
    ...
    setRiskMultipliers(multipliers) {
        for (const [key, value] of Object.entries(multipliers)) {
            if (typeof value === "number") {
                this.riskMultipliers[key] = value;
            }
        }
    }
    /**
    ...
    determineOperationType(operation) {
        ...
        if (op.includes("refactor"))
            return "refactor";
        if (op.includes("debug"))
            return "debug";
        if (op.includes("test"))
            return "test";
        if (op.includes("analyze"))
            return "analyze";
    ...
    assessRiskLevel(context) {
        const ctx = context;
        if (ctx?.critical || ctx?.riskLevel === "critical")
            return "critical";
        if (ctx?.highRisk || ctx?.riskLevel === "high")
            return "high";
