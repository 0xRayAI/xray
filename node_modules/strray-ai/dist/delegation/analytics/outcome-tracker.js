...
export class RoutingOutcomeTracker {
    ...
    persistencePath;
    saveTimeout = null;
    SAVE_DEBOUNCE_MS = 5000; // Save max once per 5 seconds
    /**
    ...
    getPromptData() {
        return this.outcomes.map((outcome) => ({
            ...
            keywords: [...new Set(outcome.taskDescription.toLowerCase().split(/\s+/)
                    .filter(w => w.length > 3)
                    .slice(0, 10))],
            context: {},
            routingDecision: {
     ...
     */
    applyRoutingRefinements(changes) {
        for (const change of changes) {
            const outcome = this.outcomes.find((o) => o.taskId === change.taskId);
            if (outcome) {
                if (change.agent)
                    outcome.routedAgent = change.agent;
                if (change.skill)
                    outcome.routedSkill = change.skill;
                if (change.confidence)
                    outcome.confidence = change.confidence;
            }
        }
    }
}
