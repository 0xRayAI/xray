...
export class NoDuplicateCodeValidator extends BaseValidator {
    id = "no-duplicate-code-validator";
    ...
    async validate(context) {
        ...
        if (newCode.includes("function formatDate") &&
            ...
            return this.createSuccessResult("Alternative date formatting implementation allowed");
        }
...
export class ContextAnalysisIntegrationValidator extends BaseValidator {
    id = "context-analysis-integration-validator";
    ...
    async validate(context) {
        ...
        if (!newCode || operation !== "write") {
            return this.createSuccessResult("No code to validate for context integration");
        }
        ...
        if (newCode.includes("export") &&
            ...
            if (newCode.includes("BrokenComponent")) {
                return this.createFailureResult("Component missing context integration", [
        ...
        if (newCode.includes("export") &&
            ...
            return this.createSuccessResult("Component with props interface appears valid");
        }
...
export class MemoryOptimizationValidator extends BaseValidator {
    id = "memory-optimization-validator";
    ...
    async validate(context) {
        const { newCode, operation } = context;
        if (!newCode || operation !== "write") {
            return this.createSuccessResult("No code to validate for memory optimization");
        }
        ...
        if (newCode.includes("inefficient") && newCode.includes("push")) {
            return this.createFailureResult("Memory inefficient patterns detected", ["Use more efficient data structures"]);
        }
...
export class DocumentationRequiredValidator extends BaseValidator {
    id = "documentation-required-validator";
    ...
    async validate(context) {
        ...
        violations.push("Universal researcher consultation required for all code changes");
        suggestions.push("Consult researcher for documentation review and version updates");
        suggestions.push("Ensure README.md, architecture docs, and API docs are current");
        if (violations.length > 0) {
            return this.createFailureResult(`Documentation violations: ${violations.join(", ")}`, [
                ...suggestions,
...
export class NoOverEngineeringValidator extends BaseValidator {
    id = "no-over-engineering-validator";
    ...
    async validate(context) {
        const { newCode, operation } = context;
        if (!newCode || operation !== "write") {
            return this.createSuccessResult("No code to validate for over-engineering");
        }
        const violations = [];
        const suggestions = [];
        // Allow test files to have different structure
        if (newCode.includes("describe(") || newCode.includes("it(")) {
            return this.createSuccessResult("Test files may have different structure requirements");
        }
        ...
        if (violations.length > 0) {
            return this.createFailureResult(`Over-engineering detected: ${violations.join(", ")}`, suggestions);
        }
        return this.createSuccessResult("Code follows simplicity principles - no over-engineering detected");
    }
}
/**
...
export class CleanDebugLogsValidator extends BaseValidator {
    id = "clean-debug-logs-validator";
    ruleId = "clean-debug-logs";
    category = "code-quality";
    severity = "error";
    async validate(context) {
        const { newCode, operation } = context;
        ...
        if (foundDebug.length > 0) {
            return this.createFailureResult(`Debug code detected: ${foundDebug.join(", ")}`, [
...
export class ConsoleLogUsageValidator extends BaseValidator {
    id = "console-log-usage-validator";
    ruleId = "console-log-usage";
    category = "code-quality";
    severity = "error";
    async validate(context) {
        const { newCode } = context;
        ...
        if (newCode.includes("await frameworkLogger.log('rule-enforcer', '-return-passed-false-message-console-log-', 'info', { message: ")) {
            return this.createFailureResult("await frameworkLogger.log('rule-enforcer', '-', 'info', { message:  } }); detected - use frameworkLogger for production logs or remove for debugging");
        }
        return this.createSuccessResult("Console log usage follows proper guidelines");
    }
}
//# sourceMappingURL=code-quality-validators.js.map