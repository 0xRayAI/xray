...
import { RuleRegistry, RuleHierarchy, RuleExecutor, ViolationFixer, } from "./core/index.js";
...
export class RuleEnforcer {
    ...
    constructor(options) {
        ...
        this.executor = options?.executor ?? new RuleExecutor(this.registry, this.hierarchy, this.validatorRegistry);
        ...
        try {
            ...
            for (const rule of result.rules) {
                if (existingRuleIds.has(rule.id)) {
                    await frameworkLogger.log("rule-enforcer", "async-rule-skipped-duplicate", "info", { message: `Skipping duplicate rule: ${rule.id}` });
                    skippedCount++;
                    continue;
                }
                this.addRule(rule);
            ...
            await frameworkLogger.log("rule-enforcer", "async-rules-loaded", "success", {
            ...
            await frameworkLogger.log("rule-enforcer", "async-rules-load-failed", "error", {
     ...
     */
    createValidatorDelegate(ruleId) {
