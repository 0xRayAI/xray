 ...
 */
export async function contextAnalysis(projectRoot, files, depth = "detailed") {
    await frameworkLogger.log("architect-tools", "context-analysis-start", "info", {
        projectRoot,
    ...
    try {
        ...
        if (depth !== "overview") {
            ...
            architecturalPatterns = identifyArchitecturalPatterns(analysis, dependencyAnalysis);
            ...
            maintainabilityIndex = calculateMaintainabilityIndex(analysis, dependencyAnalysis);
        ...
        await frameworkLogger.log("architect-tools", "context-analysis-complete", "success", {
            projectRoot,
        ...
        await frameworkLogger.log("architect-tools", "context-analysis-failed", "error", {
            projectRoot,
 ...
 */
export async function codebaseStructure(projectRoot, includeMetrics = true) {
    await frameworkLogger.log("architect-tools", "structure-analysis-start", "info", {
        projectRoot,
        includeMetrics,
    });
    const contextAnalyzer = createCodebaseContextAnalyzer(projectRoot);
    ...
    await frameworkLogger.log("architect-tools", "structure-analysis-complete", "success", {
        projectRoot,
        filesAnalyzed: analysis.structure.totalFiles,
    });
    return structure;
 ...
 */
export async function dependencyAnalysis(projectRoot, focusAreas) {
    await frameworkLogger.log("architect-tools", "dependency-analysis-start", "info", {
        projectRoot,
    ...
    const recommendations = generateDependencyRecommendations(circularDependencies, tightlyCoupledModules, orphanModules, healthScore);
    const result = {
    ...
    await frameworkLogger.log("architect-tools", "dependency-analysis-complete", "success", {
        projectRoot,
 ...
 */
export async function architectureAssessment(projectRoot, assessmentType = "comprehensive") {
    await frameworkLogger.log("architect-tools", "architecture-assessment-start", "info", {
        projectRoot,
        assessmentType,
    });
    // Gather all necessary data
    const contextResult = await contextAnalysis(projectRoot, undefined, assessmentType === "comprehensive" ? "detailed" : "overview");
    ...
    const issues = identifyArchitecturalIssues(contextResult, dependencyResult, scores);
    ...
    await frameworkLogger.log("architect-tools", "architecture-assessment-complete", "success", {
        projectRoot,
        overallHealth,
...
// Helper functions
function identifyArchitecturalPatterns(codebaseAnalysis, dependencyAnalysis) {
    const patterns = [];
...
function identifyBasicPatterns(codebaseAnalysis) {
    const patterns = [];
    // Basic pattern detection based on file structure
    const fileNames = Array.from(codebaseAnalysis.structure.fileGraph?.keys() || []);
    if (fileNames.some((f) => typeof f === "string" && f.includes("controller")))
        patterns.push("Controller Layer Detected");
    if (fileNames.some((f) => typeof f === "string" && f.includes("service")))
        patterns.push("Service Layer Detected");
    if (fileNames.some((f) => typeof f === "string" && (f.includes("model") || f.includes("entity"))))
        patterns.push("Data Models Detected");
    return patterns;
}
function analyzeDependencyHealth(dependencyAnalysis) {
    const issues = [];
    if (dependencyAnalysis.metrics.circularDependencies > 0) {
        issues.push(`${dependencyAnalysis.metrics.circularDependencies} circular dependencies detected`);
    }
    if (dependencyAnalysis.metrics.couplingIndex > 80) {
        issues.push(`High coupling index: ${dependencyAnalysis.metrics.couplingIndex}`);
    }
    if (dependencyAnalysis.metrics.orphanFiles > 0) {
        issues.push(`${dependencyAnalysis.metrics.orphanFiles} orphan files detected`);
    }
    return issues;
}
function assessScalability(codebaseAnalysis, dependencyAnalysis) {
    let score = 100;
    ...
    // Modularity impact
    if (codebaseAnalysis.structure.modules.size < 5 &&
        codebaseAnalysis.structure.totalFiles > 50) {
        score -= 15;
        recommendations.push("Increase modularity for better scalability");
    }
    // File size impact
    const avgFileSize = codebaseAnalysis.structure.totalLinesOfCode /
...
function calculateMaintainabilityIndex(codebaseAnalysis, dependencyAnalysis) {
    let index = 100;
    // Complexity impact
...
function analyzeComplexityPatterns(codebaseAnalysis) {
    return {
...
function hasMVCPattern(analysis) {
    const files = Array.from(analysis.structure.fileGraph?.keys() || []);
    const hasControllers = files.some((f) => typeof f === "string" && f.includes("controller"));
    const hasModels = files.some((f) => typeof f === "string" && f.includes("model"));
    const hasViews = files.some((f) => typeof f === "string" && (f.includes("view") || f.includes("component")));
    return hasControllers && hasModels && hasViews;
}
function hasRepositoryPattern(analysis) {
    const files = Array.from(analysis.structure.fileGraph?.keys() || []);
...
function hasFactoryPattern(analysis) {
    const files = Array.from(analysis.structure.fileGraph?.keys() || []);
    return files.some((f) => typeof f === "string" && f.includes("factory"));
}
function hasMicroservicesIndicators(analysis) {
    const files = Array.from(analysis.structure.fileGraph?.keys() || []);
    return files.some((f) => typeof f === "string" &&
...
function generateDependencyRecommendations(circularDeps, tightlyCoupled, orphans, healthScore) {
    const recommendations = [];
    if (circularDeps.length > 0) {
        recommendations.push("Resolve circular dependencies by introducing interfaces or mediator patterns");
    }
    if (tightlyCoupled.length > 0) {
        recommendations.push("Reduce coupling by applying dependency inversion principle");
    }
    if (orphans.length > 0) {
        recommendations.push("Review orphan modules - they may indicate dead code or missing integration");
    }
    if (healthScore < 70) {
        recommendations.push("Consider architectural refactoring to improve dependency health");
    }
    return recommendations;
}
function calculateArchitecturalScores(context, deps) {
    return {
...
function determineOverallHealth(scores) {
...
function identifyArchitecturalIssues(context, deps, scores) {
    const issues = [];
    if (scores.coupling > 80) {
        issues.push({
            ...
            recommendation: "Apply dependency inversion and interface segregation principles",
        });
    }
...
function generateArchitecturalRecommendations(issues, scores) {
    const recommendations = [];
    ...
    if (scores.scalability < 70) {
        recommendations.push("Improve scalability by reducing coupling and increasing modularity");
    }
    if (scores.testability < 70) {
        recommendations.push("Enhance testability by improving separation of concerns");
    }
    recommendations.push("Regular architectural reviews to maintain code quality");
    return recommendations;
}
function generateComplexityRecommendations(codebaseAnalysis) {
    const recommendations = [];
    if (codebaseAnalysis.metrics.complexity > 80) {
    ...
    if (codebaseAnalysis.structure.totalLinesOfCode > 10000) {
        recommendations.push("Consider splitting large codebases into separate modules");
    }
    return recommendations;
}
...
//# sourceMappingURL=architect-tools.js.map