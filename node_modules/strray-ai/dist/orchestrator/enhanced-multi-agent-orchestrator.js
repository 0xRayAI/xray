...
export class EnhancedMultiAgentOrchestrator {
    ...
    async spawnAgent(request) {
        ...
        try {
            ...
            const error = new Error(`SPAWN GATE BLOCKED: ${gateError instanceof Error ? gateError.message : String(gateError)}`);
            await frameworkLogger.log("enhanced-multi-agent-orchestrator", "spawn-gate-blocked", "error", { message: error.message, agentType: request.agentType });
            throw error;
        }
        ...
        if (!auth.authorized) {
            const error = new Error(`Spawn denied by governance: ${auth.reason}. ` +
                `Warnings: ${(auth.warnings || []).join(", ")}`);
            await frameworkLogger.log("enhanced-multi-agent-orchestrator", "spawn-denied-by-governance", "error", { agentType: request.agentType, reason: auth.reason, trackingId: auth.trackingId });
            throw error;
        }
