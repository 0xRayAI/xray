...
export class EnterpriseMonitoringSystem extends EventEmitter {
    ...
    async start() {
        ...
        await frameworkLogger.log("enterprise-monitoring-system", "-collection-interval-this-config-collectioninterva", "info", {
            message: `   Collection Interval: ${this.config.collectionInterval}ms`,
        });
        await frameworkLogger.log("enterprise-monitoring-system", "-health-check-interval-this-config-healthchecks-in", "info", {
    ...
    async handleAlertTriggered(alert) {
        await frameworkLogger.log("enterprise-monitoring-system", "-alert-alert-severity-touppercase-alert-message-", "info", {
    ...
    async handleMetricsCollected(data) {
        ...
        if (process.env.DEBUG_MONITORING) {
            await frameworkLogger.log("enterprise-monitoring-system", "-metrics-collected-cpu-data-system-cpu-usage-tofix", "info", {
    ...
    async handleProfileCompleted(profileData) {
        await this.recordMetric("agent.profile.completed", {
    ...
    async handleReportGenerated(reportPath) {
