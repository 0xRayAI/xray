...
export class MemoryRegressionTester extends EventEmitter {
    ...
    async runTest(test) {
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Test timeout")), this.config.testTimeout));
     ...
     */
    createStandardTest(name, description, testFunction, options = {}) {
        return {
            name,
            description,
            setup: options.setup,
    ...
    calculateVariance(samples) {
        ...
        const variance = samples.reduce((sum, s) => sum + Math.pow(s - mean, 2), 0) /
            samples.length;
        return variance;
    }
}
...
export const memoryTestTemplates = {
    sessionOperations: (sessionCount = 100) => ({
    ...
    cacheOperations: (cacheSize = 1000) => ({
    ...
    streamingOperations: (messageCount = 1000) => ({
