...
export class PerformanceBudgetEnforcer extends EventEmitter {
     ...
     */
    async analyzeBundleSize(distPath = "./dist") {
        const metrics = {
            totalSize: 0,
            gzippedSize: 0,
            fileCount: 0,
     ...
     */
    checkBudgetCompliance(bundleSize, webVitals, runtime) {
        const violations = [];
        // Bundle size checks
        if (bundleSize.totalSize > PERFORMANCE_BUDGET.bundleSize.uncompressed) {
            violations.push({
                ...
                percentage: (bundleSize.totalSize / PERFORMANCE_BUDGET.bundleSize.uncompressed) *
                    100,
                severity: bundleSize.totalSize >
                    PERFORMANCE_BUDGET.bundleSize.uncompressed * 1.1
                    ? "critical"
                    : "error",
                recommendation: "Reduce bundle size by code splitting, tree shaking, or removing unused dependencies",
            });
        }
        if (bundleSize.gzippedSize > PERFORMANCE_BUDGET.bundleSize.gzipped) {
            violations.push({
                ...
                percentage: (bundleSize.gzippedSize / PERFORMANCE_BUDGET.bundleSize.gzipped) *
                    100,
                severity: bundleSize.gzippedSize > PERFORMANCE_BUDGET.bundleSize.gzipped * 1.1
                    ? "critical"
                    : "error",
                recommendation: "Optimize gzipped bundle size through minification and compression improvements",
            });
        }
        ...
        if (webVitals) {
            if (webVitals.firstContentfulPaint >
                ...
                violations.push({
                    ...
                    percentage: (webVitals.firstContentfulPaint /
                        PERFORMANCE_BUDGET.webVitals.firstContentfulPaint) *
                        100,
                    severity: webVitals.firstContentfulPaint >
            ...
            if (webVitals.timeToInteractive >
                ...
                violations.push({
                    ...
                    percentage: (webVitals.timeToInteractive /
                        PERFORMANCE_BUDGET.webVitals.timeToInteractive) *
                        100,
                    severity: webVitals.timeToInteractive >
                        PERFORMANCE_BUDGET.webVitals.timeToInteractive * 1.1
                        ? "critical"
                        : "error",
                    recommendation: "Reduce JavaScript execution time, optimize main thread work",
                });
            }
        }
        // Runtime checks
        if (runtime.memoryUsage.heapUsed > PERFORMANCE_BUDGET.runtime.memoryUsage) {
            violations.push({
                ...
                percentage: (runtime.memoryUsage.heapUsed /
                    PERFORMANCE_BUDGET.runtime.memoryUsage) *
                    100,
                severity: runtime.memoryUsage.heapUsed >
                    PERFORMANCE_BUDGET.runtime.memoryUsage * 1.1
                    ? "critical"
                    : "warning",
                recommendation: "Optimize memory usage through garbage collection improvements or memory leak fixes",
            });
        }
        return violations;
    }
    ...
    async generatePerformanceReport() {
        ...
        const violations = this.checkBudgetCompliance(bundleSize, webVitals, runtime);
        // Generate recommendations based on violations
        const recommendations = this.generateRecommendations(violations, bundleSize, webVitals, runtime);
     ...
     */
    generateRecommendations(violations, bundleSize, webVitals, runtime) {
        ...
        if (bundleSize.largestFile.size > bundleSize.totalSize * 0.5) {
            recommendations.push(`Largest file (${bundleSize.largestFile.name}) is ${((bundleSize.largestFile.size / bundleSize.totalSize) * 100).toFixed(1)}% of bundle - consider splitting or optimizing`);
        }
        // Web vitals recommendations
        if (webVitals && webVitals.firstContentfulPaint > 1500) {
            recommendations.push("Optimize First Contentful Paint by prioritizing critical CSS and above-the-fold content");
        }
        if (webVitals && webVitals.timeToInteractive > 3000) {
            recommendations.push("Reduce Time to Interactive by minimizing main thread blocking JavaScript");
        }
        // Runtime recommendations
        if (runtime.memoryUsage.heapUsed >
            PERFORMANCE_BUDGET.runtime.memoryUsage * 0.8) {
            recommendations.push("Monitor for memory leaks and consider implementing memory optimization strategies");
        }
     ...
     */
    handleBudgetExceeded(violation) {
    ...
    exportPerformanceData() {
        const summary = {
            ...
            averageViolations: this.reports.length > 0
        ...
        const mostCommon = Object.entries(violationCounts).sort(([, a], [, b]) => b - a)[0];
...
//# sourceMappingURL=performance-budget-enforcer.js.map