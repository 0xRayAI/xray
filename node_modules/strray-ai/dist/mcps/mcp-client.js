...
export class MCPClient extends EventEmitter {
    ...
    async executeRealTool(toolName, args) {
        ...
        const serverConfig = registryConfig ?? {
    ...
    async callTool(toolName, args = {}) {
        ...
        try {
            ...
            if (preferReal) {
                try {
                    const result = await this.executeWithRetry(() => this.executeRealTool(toolName, args), `real:${toolName}`);
            ...
            if (!this.isPureMcpMode && this.simulationEngine.canSimulate(serverName, toolName)) {
                try {
                    const result = await this.executeWithRetry(() => this.simulationEngine.simulate(serverName, toolName, args), `simulate:${toolName}`);
                    const afterEvent = {
                        ...beforeEvent,
                        result,
                        duration: Date.now() - startTime,
