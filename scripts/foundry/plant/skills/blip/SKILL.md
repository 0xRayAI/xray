...
Rippel five are **imports**. Three seed + flag axes: `--look focus|cage`, `--body mill|rippel`, `--genre ambient|techno|phonk|jazz|rock|timeless` (unknown genre FAIL). Suit is void + five hexes + Wu + hook → turn → tag — not a circle pack on every organ. Focus keeps the disc + moons. Mill swirl is Wu mesh, snap is strike, spark is embers. Receipt `lookKind` + `bodyKind` + `genre` + `organ` + `stereoImage`. `kapow` is a growth stub. Unknown id FAIL.
