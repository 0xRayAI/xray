...
async function analyzeFrameworkUsage() {
    ...
    console.log(`✅ Configuration loaded: ${Object.keys(config).length} settings`);
    ...
    for (const scenario of testScenarios) {
        ...
        console.log(`   Complexity Score: ${result.complexity.score} (${result.complexity.level})`);
    ...
    for (const [component, stats] of Array.from(componentStats).sort((a, b) => b[1].count - a[1].count)) {
        const successRate = stats.count > 0
            ? ((stats.success / stats.count) * 100).toFixed(1)
            : "0.0";
        console.log(`   ${component}: ${stats.count} actions (${successRate}% success, ${stats.error} errors)`);
    }
    ...
    const timeStats = recentLogs.reduce((acc, log) => {
    ...
    for (let hour = 0; hour < 24; hour++) {
        ...
        if (count > 0) {
            console.log(`   ${hour.toString().padStart(2, "0")}:00: ${count} actions`);
        }
    }
    ...
    if (errorLogs.length > 0) {
        ...
        errorLogs.slice(-3).forEach((error) => {
            console.log(`     - ${error.component}:${error.action} (${new Date(error.timestamp).toLocaleTimeString()})`);
        });
    }
    // Agent usage analysis
    const agentLogs = recentLogs.filter((log) => log.agent && log.agent !== "sisyphus");
    console.log("\n🤖 AGENT USAGE:");
    if (agentLogs.length > 0) {
        ...
        for (const [agent, count] of Object.entries(agentStats).sort((a, b) => b[1] - a[1])) {
