...
function checkGitStatus() {
  ...
  if (status && status.length > 0) {
    ...
    if (lines.length > 0) {
      ...
      log('Git has uncommitted changes', 'error');
      return false;
    }
...
function checkVersionSync() {
  ...
  if (pkgVersion !== vmVersion) {
    errors.push(`Version mismatch: package.json (${pkgVersion}) != version manager (${vmVersion})`);
    log(`Version mismatch: package.json=${pkgVersion}, version-manager=${vmVersion}`, 'error');
    return false;
  }
