...
#   - When set, sh touches /tmp/xray-force-non-continue-s02 sentinel before gov/processor/status sections,
...
declare -a MCP_SERVERS=(
  "dist/mcps/governance.server.js:xray-governance (govern_proposals, govern_reflection, get_active_codex)"
  "dist/mcps/knowledge-skills/skill-invocation.server.js:xray-skills (invoke-skill, list-skills + 10+ domain skills)"
  "dist/mcps/orchestrator/server.js:xray-orchestrator (orchestrate-task, task planning/execution)"
  "dist/mcps/enforcer-tools.server.js:xray-enforcer (enforcement tools, codex exposure)"
)
...
echo "=== 2. Grok CLI / npx 0xray Registration Surface (full v2 set) ==="
...
if [[ ! -f "$GROK_CLI_SRC" ]]; then
  ...
  for srv in xray-governance xray-skills xray-orchestrator xray-enforcer; do
  ...
  if grep -q 'npx 0xray grok install' "$GROK_CLI_SRC" || grep -q 'installForGrokCLI' "$GROK_CLI_SRC"; then
    echo "✅ Canonical install entrypoint (npx 0xray grok install / installForGrokCLI) documented"
  else
...
if [[ -f "$CLI_SRC" ]]; then
  if grep -qE 'grok.*install|install.*grok' "$CLI_SRC" && grep -q 'npx 0xray install' "$CLI_SRC"; then
    echo "✅ CLI exposes install + grok install paths (npx 0xray install delegates to full surface)"
  fi
fi
...
if [[ -f "$PKG" ]]; then
  if grep -q '"0xray": "dist/cli/index.js"' "$PKG"; then
    echo "✅ bin.0xray points to CLI (enables npx 0xray install)"
  fi
...
if [[ -f "$GOV_SRC" ]]; then
  ...
  echo "✅ xray-governance: ~$GOV_TOOLS+ core tools (govern_proposals, govern_reflection, get_active_codex etc.) statically detected"
else
...
if [[ -f "$SKILLS_SRC" ]]; then
  ...
  echo "✅ xray-skills: ~$SKILL_TOOLS tools (invoke-skill, list-skills + domain skills: code-review, security, etc.)"
else
  # fallback to dir scan
  SKILL_DIR="$PROJECT_ROOT/src/mcps/knowledge-skills"
  if [[ -d "$SKILL_DIR" ]]; then
    SKILL_COUNT=$(find "$SKILL_DIR" -name '*.server.ts' | wc -l | tr -d ' ')
    echo "✅ xray-skills dir: $SKILL_COUNT skill servers present (tool surface via invocation hub)"
  fi
fi
...
if [[ -f "$ORCH_SRC" ]]; then
  ...
  echo "✅ xray-orchestrator: ~$ORCH_TOOLS+ tools (orchestrate-task primary for governed flows)"
else
...
if [[ -f "$ENF_SRC" ]]; then
  ...
  echo "✅ xray-enforcer: ~$ENF_TOOLS tools (codex exposure + enforcement for governed decisions)"
else
...
echo "     - docker run --rm -v $(pwd):/app -w /app node:20 bash -c 'npm ci && npm run build && npx 0xray install && ...'"
    ...
    (async () => {
      try {
        ...
        const initId = sendRequest('initialize', {
          ...
          capabilities: {},
          clientInfo: { name: 'xray-mcp-regression-probe', version: 's05-ext' }
        });
...
if echo "$GOV_OUT" | grep -q 'connected=true' && echo "$GOV_OUT" | grep -q 'get_active_codex'; then
  ...
  echo "   ✅ LIVE PROBE + FLOW (get_active_codex + P2-GOV-BRIDGE-12 execution-intent govern_proposals test) for xray-governance"
else
...
echo ""
echo ">>> Probing xray-skills (list-skills + 3 domain skill tools)..."
...
if echo "$SKILLS_OUT" | grep -q 'connected=true' && echo "$SKILLS_OUT" | grep -q 'list-skills'; then
  LIVE_SUCCESS=$((LIVE_SUCCESS+1))
  echo "   ✅ LIVE PROBE + 4 FLOWS (list-skills + 3 skills) for xray-skills"
else
...
echo ""
echo ">>> Probing xray-orchestrator..."
...
if [[ "${FORCE_NON_CONTINUE_S02:-0}" == "1" ]]; then
  touch /tmp/xray-force-non-continue-s02 2>/dev/null || true
  echo "   P2-GOV-BRIDGE-30: FORCE_NON_CONTINUE_S02=1 detected — sentinel /tmp/xray-force-non-continue-s02 activated (forces real non-cont per-proc S02 verdicts via planner for deterministic ✅ on 28 content-assert + 29 three-layer probes)"
fi
...
if echo "$ORCH_OUT" | grep -q 'connected=true'; then
  LIVE_SUCCESS=$((LIVE_SUCCESS+1))
  echo "   ✅ LIVE PROBE + 2+ FLOWS (orchestrate-task + get-orchestration-status detailed canonical for 7-flow thinDispatch funnel; P2-CLEANUP-02 right-sized detector exercised via delegation) for xray-orchestrator"
else
...
if echo "$ORCH_OUT" | grep -q 'Internal S02 Influence Notes (from processor doing data — P2-GOV-BRIDGE-27)'; then
  ...
  if [[ "${FORCE_NON_CONTINUE_S02:-0}" == "1" ]] || [ -f /tmp/xray-force-non-continue-s02 ] 2>/dev/null; then
...
if echo "$ENF_OUT" | grep -q 'connected=true'; then
  LIVE_SUCCESS=$((LIVE_SUCCESS+1))
  echo "   ✅ LIVE PROBE + FLOW (get-enforcement-status) for xray-enforcer"
else
...
if [[ $MISSING -eq 0 && $REG_OK -eq 1 ]]; then
  ...
  echo "     - Full registration surface (S04 canonical: all grok mcp add + npx 0xray paths)"
  ...
  rm -f /tmp/xray-force-non-continue-s02 2>/dev/null || true
  exit 0
else
