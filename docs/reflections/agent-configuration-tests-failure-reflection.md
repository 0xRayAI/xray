...
**The Reality:**
- 6 agents missing from `opencode.json` (database-engineer, devops-engineer, backend-engineer, frontend-engineer, performance-engineer, mobile-developer)
- 3 agents causing `ProviderModelNotFoundError` due to `model: default` in `.yml` files
...
**The Struggle:**
I spent 2+ hours manually testing each agent with `@agent hello` commands, one by one. The build passed. All unit tests passed. But 9 out of 24 agents failed when actually invoked. This felt like trusting a car's safety rating but discovering the airbags only work if you manually pull the lever.
...
**What Happened:** First 9 agents worked. Then researcher failed with ProviderModelNotFoundError.
...
**What Happened:** All 24 agents eventually worked
...
**Symptom:** 6 agents missing from opencode.json
...
**Symptom:** ProviderModelNotFoundError on 3 agents
...
**Ah-Ha Moment:** Had to manually invoke each of 24 agents to find issues
...
### My Triumph
We fixed all 24 agents. We created a pre-publish guard. We documented the troubleshooting. We now understand the gap between code testing and configuration testing. The victory isn't perfect - the guard still doesn't test agents - but we've closed the first gap.

### My Dichotomy
- I wanted to trust our test suite, but the evidence said don't
- I wanted to believe our 99.6% claim, but 6/24 agents didn't work
