...
v2 proved the federation works (2,2500 tests, 4 plugin E2Es in isolated tmp dirs, 0 failures). v3 crystallizes it into its true shape: the **nucleus** (Dynamo pipeline + orchestrator routing + enforcer) surrounded by **dynamic adapters**.

v2.1.4 is the measured starting state (41 MCP servers / 25 knowledge-skill + XrayKnowledgeSkillBase, 162 files / 2,2500 tests, 4 bridges proven via fresh published-package consumer verification in isolated tmp dirs). The federation is solid; v3 extracts the callable kernel from it.
...
- **Estimate**: 0.5 day
- **Status**: 2500 tests pass. All specified rejection, call, merge, override, and logging cases covered. `npx vitest run src/nucleus/` ✅
...
- **Done when**: Suite exists, passes in CI — 2500 tests in `src/__tests__/integration/self-evolution-e2e.test.ts`
...
- 162 test files, 2,2500 tests, 0 failures at release.
