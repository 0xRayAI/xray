...
Open list: [4.0 left](./v4-left.md). npm is **4.0.18**. Remaining: land the release-gate follow-up (smoke + `scripts.publish` lifecycle + reconcile), optional `@0xray/repertoire@0.2.0` npm.

Do not start a fifth MCP. Do not re-litigate Zen pickle. Do not thicken the Grok exo.

## One line for the next model

**Suit that survives the context window. Four floors attested. npm 4.0.18. Gate follow-up: smoke matches organ-on; do not re-run the gate after the registry PUT.**
