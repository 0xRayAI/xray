...
npm **4.0.20** (`https://www.npmjs.com/package/0xray`). Tag `v4.0.0` on `0xRayAI/xray` `main` (`f80b42c`) is history. Next 4.x fix is `npm run release:npm` — never `release:major` (that becomes 5.0.0).
...
- **Repertoire preferred** — factory seed **0.2.5** vendored. Grok MCP names unprefixed. Not an 8th 0xRay MCP. Do not pin 0.1.8.
...
Open list: [4.0 left](./v4-left.md). npm is **4.0.20**. Remaining: land the release-gate follow-up (smoke + `scripts.publish` lifecycle + reconcile), optional `@0xray/repertoire@0.2.0` npm.

Do not start a fifth MCP. Do not re-litigate Zen pickle. Do not thicken the Grok exo.

## One line for the next model

**Suit that survives the context window. Four floors attested. npm 4.0.20. Gate follow-up: smoke matches organ-on; do not re-run the gate after the registry PUT.**
