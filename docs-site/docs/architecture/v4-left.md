...
| Factory memory organ | `@0xray/repertoire@0.2.8` vendored (seed plus overlays). Isolated tarball wears it without `../repertoire`. Extra MCP `repertoire`, not an 8th `xray-*`. Optional. |
...
2. Dual-ship: mill is nested in the 0xray tarball (4.0.2+). Standalone `@0xray/foundry` remains the mill CLI.
