...
**Blip plant (not costume).** New factory plant sibling to mill + sound (not a mill copy). Hangar shop pair later. `foundry.json` `"plant": "blip"` fastens factory-blip (`blip` + `blip-inspect`) and turns mill off. Motions live in a dynamic registry. v0: still + Rippel five. `kapow` is a growth stub. Rippel v2 motions are Rippel `VisualConfig.circles` at ≥720p — sharp look + tempo/frequency animation on all five — with a mandatory 4.44s audio bed that syncopates to the motion grid. Each mint wears a sparse seed mesh plus field accents (stars, grids, gradients, blinkers); receipt `visualConfig.mesh` + `visualConfig.field` are the fingerprints. `still` is the Power Plant ident (hard-cut plates, not a frozen poster). Last blip receipt (`.xray/blip/receipt.json`) is PASS/FAIL with duration 4.44s, mode id, audio stream, file present. Not 45/42.
