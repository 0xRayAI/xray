...
**Minted from their tree (overlay).** Default SSOT: `xray/codex.json`, `xray/features.json`, `xray/config.json`, `src/skills/<name>/SKILL.md`, `src/opencode/agents/*.yml`. Postinstall copies those onto `.xray/` and **project** skill hangers (`.opencode/skills`, project `.grok/plugins/0xray/skills` if present). Home skill dirs stay mill-45 unless `foundry.json` sets `"homeSkills": true`. `codexMode` / `featuresMode` / `configMode`: `merge` unless `replace`. Mill-fill stays: leftover mill skill/agent names remain unless they overlay the same name. YML agents are the OpenCode hangar. Mill-managed `AGENTS.md` overlays from `xray/AGENTS.md`. `foundry.json` remaps paths. CLI re-wear and `syncBuiltinSkills` remint. `SKILLS.md` is still **not** written to the consumer root. Inventory garment is `overlay` when any facet came from their tree, else `copied-onto-hanger`. Dogfood writes no inventory.
