...
**Blip plant (not costume).** New factory plant sibling to mill + sound (not a mill copy). Hangar shop pair later. `foundry.json` `"plant": "blip"` fastens factory-blip (`blip` + `blip-inspect`) and turns mill off. Motions live in a dynamic registry. v0: still + Rippel five. Last blip receipt (`.xray/blip/receipt.json`) is PASS/FAIL with duration 4.44s, mode id, file present. Not 45/42.
