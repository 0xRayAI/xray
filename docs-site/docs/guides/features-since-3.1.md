...
- Factory-blip plant: new factory plant sibling to mill + sound (not a mill copy). Hangar shop pair later. `foundry.json` `"plant": "blip"` fastens `blip` + `blip-inspect`. Dynamic registry (not a frozen Rippel enum). v0: still + Rippel five. `kapow` is a growth stub. Still generator wears the Power Plant intro plate (`#08090B` `#F5F7FA` `#3DE0E8` `#F5C518` `#4A7FD4`). Unknown id FAIL. No mill package bump.
