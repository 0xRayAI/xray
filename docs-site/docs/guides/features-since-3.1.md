...
- Factory-blip plant: new factory plant sibling to mill + sound (not a mill copy). Hangar shop pair later. `foundry.json` `"plant": "blip"` fastens `blip` + `blip-inspect` + `blip-vibe` + `blip-looker`. `blip vibe` is stamp density. `blip look` is the friend hitting replay (ship bar 8). Comic/anime cel + jewel-cut speed burst. Dynamic registry (not a frozen Rippel enum). v0: still + Rippel five. `kapow` is a design opt (two-tier stamp on the stanza). Rippel v2: converter motions at ≥720p with seed-selected looks (`focus` disc-satellites | `cage` Wu+field) + tempo/frequency animation on all five + mandatory 4.44s bed that syncopates to the motion grid. Receipt `visualConfig.lookKind` + `visualConfig.mesh` + `visualConfig.field` are the NFT fingerprints. `still` is the Power Plant ident (plate dissolves over 4.44s at ≥720p, not a frozen poster) (`#08090B` `#F5F7FA` `#3DE0E8` `#F5C518` `#4A7FD4`). Unknown id FAIL. No mill package bump.
- Factory-sound plant: `foundry.json` `"plant": "sound"` fastens `sound` + `sound-inspect` + `sound-mixer` (mill off). Brief → seed → genre → wav → metrics receipt. `sound mix` listens to the whole 4.44s and levels every genre × tempo × motif. Inspect reports last bed PASS/FAIL. Not costume. No mill package bump.
