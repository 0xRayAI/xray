...
- Factory-blip plant: new factory plant sibling to mill + sound (not a mill copy). Hangar shop pair later. `foundry.json` `"plant": "blip"` fastens `blip` + `blip-inspect`. Dynamic registry (not a frozen Rippel enum). v0: still + Rippel five. `kapow` is a growth stub. Rippel v2: converter motions at ≥720p with sharp look + tempo/frequency animation on all five + mandatory 4.44s bed that syncopates to the motion grid. Sparse seed mesh plus field accents (stars, grids, gradients, blinkers); receipt `visualConfig.mesh` + `visualConfig.field` are the NFT fingerprints. `still` is the Power Plant ident (hard-cut plates over 4.44s at ≥720p, not a frozen poster) (`#08090B` `#F5F7FA` `#3DE0E8` `#F5C518` `#4A7FD4`). Unknown id FAIL. No mill package bump.
