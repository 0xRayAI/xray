...
- **Plates:** one schematic per pipeline under [Plates](../plates/index.md). Station stays the short table and adds one pointer line, `Plate: <id> — .xray/state/plates/<id>.md`, when intent names a pipeline. Pre-compact and the `MEMORY_ROUTING` inject carry that one plate. A lesson is what happened. A plate is the machine.
