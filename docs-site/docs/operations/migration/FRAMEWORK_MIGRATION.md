...
This document describes migration between 0xRay Framework versions, with a focus on the v2.0.0 architecture refactoring which introduced the **Facade Pattern** and delivered significant performance improvements.

**Current Version**: v2.0.0  
**Previous Version**: v1.7.5  
**Migration Type**: Zero Breaking Changes - 100% Backward Compatible

## v2.0.0 Migration Summary

### 🎉 No Breaking Changes!

0xRay v2.0.0 maintains **100% backward compatibility**. All existing code continues to work exactly as before.
...
## v2.0.0 Architecture Benefits

### Facade Pattern Implementation

The v2.0.0 refactoring introduced the **Facade Pattern** for improved maintainability and performance:
...
| Metric | v1.7.5 | v2.0.0 | Improvement |
|--------|--------|--------|-------------|
...
cp .opencode/0xray/config.json .opencode/0xray/config.backup.json
cp .opencode/0xray/hooks/ .opencode/0xray/hooks.backup/ -r
```
...
0xray migrate-config --from v1.0 --to v1.1
```
...
0xray consolidate-hooks --backup
```
...
rm .opencode/0xray/config.backup.json
rm -rf .opencode/0xray/hooks.backup/
```
...
```bash
# Restore from backup
cp .opencode/0xray/config.backup.json .opencode/0xray/config.json

# Run diagnostic
0xray diagnose-config
```
...
```bash
# Complete rollback
0xray rollback --complete

# Selective rollback
0xray rollback --component config
0xray rollback --component hooks
```
...
| Component           | v1.7.5 Compatibility | v2.0.0 Status   | Breaking Changes |
| ------------------- | -------------------- | --------------- | ---------------- |
...
- **Memory Usage**: 25% reduction in framework footprint (pre-v2.0.0)
- **Startup Time**: 30% improvement in initialization (pre-v2.0.0)
...
_This migration guide covers the transition from 0xRay v1.0 to v2.0.0. For current version information, check the main documentation._</content>
