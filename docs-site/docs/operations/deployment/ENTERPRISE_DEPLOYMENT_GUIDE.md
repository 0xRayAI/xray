...
```bash
git clone https://github.com/0xray-framework/stringray.git
cd stringray
npm install
```
...
DATABASE_URL=postgresql://localhost:5432/0xray
...
RUN adduser -S 0xray -u 1001

WORKDIR /app

# Copy built application
COPY --from=build --chown=0xray:nodejs /app/dist ./dist
COPY --from=build --chown=0xray:nodejs /app/package*.json ./
COPY --from=build --chown=0xray:nodejs /app/node_modules ./node_modules
...
# View logs
docker-compose logs -f 0xray

# Scale services
docker-compose up -d --scale 0xray=3
```
...
RUN adduser --system --uid 1001 0xray
...
spec:
  tls:
    - hosts:
        - api.0xray.example.com
      secretName: 0xray-tls
  rules:
    - host: api.0xray.example.com
      http:
        paths:
          - path: /
            pathType: Prefix
...
Resources:
  ...
  0xRayLaunchTemplate:
    ...
    Properties:
      LaunchTemplateName: 0xray-launch-template
      LaunchTemplateData:
        ...
        UserData:
          Fn::Base64: |
            ...
            npm install
            npm run build
            pm2 start dist/server.js --name 0xray
        ...
        "logConfiguration":
          {
            ...
            "options":
              {
                "awslogs-group": "/ecs/0xray-framework",
...
spec:
  template:
    ...
    spec:
      containers:
        - image: gcr.io/project-id/0xray:latest
          ports:
            - name: http1
              containerPort: 3000
          env:
...
BACKUP_DIR="/opt/0xray/backups"
DATE=$(date +%Y%m%d_%H%M%S)

# Database backup
pg_dump -h localhost -U 0xray 0xray > "$BACKUP_DIR/database_$DATE.sql"

# Configuration backup
tar -czf "$BACKUP_DIR/config_$DATE.tar.gz" /opt/0xray/config/

# Application data backup
tar -czf "$BACKUP_DIR/app_$DATE.tar.gz" /opt/0xray/data/

# Upload to cloud storage
aws s3 cp "$BACKUP_DIR/" s3://0xray-backups/ --recursive
...
# Restore database
psql -h localhost -U 0xray 0xray < "backups/database_$BACKUP_DATE.sql"

# Restore configuration
tar -xzf "backups/config_$BACKUP_DATE.tar.gz" -C /opt/0xray/

# Restore application data
tar -xzf "backups/app_$BACKUP_DATE.tar.gz" -C /opt/0xray/
...
Resources:
  0xRayCloudFront:
    ...
    Properties:
      DistributionConfig:
        Origins:
          - DomainName: 0xray-api.example.com
            Id: 0xRayAPI
            CustomOriginConfig:
              HTTPPort: 80
              HTTPSPort: 443
...
# Database backup
docker exec 0xray-postgres pg_dump -U 0xray 0xray > "$BACKUP_DIR/database.sql"

# Configuration backup
tar -czf "$BACKUP_DIR/config.tar.gz" /opt/0xray/config/

# Application logs backup
tar -czf "$BACKUP_DIR/logs.tar.gz" /opt/0xray/logs/

# Upload to cloud storage
aws s3 sync "$BACKUP_DIR/" "s3://0xray-backups/daily/$DATE/"
...
WEEK=$(date +%Y-W%U)
BACKUP_DIR="/opt/0xray/backups/weekly/$WEEK"

mkdir -p "$BACKUP_DIR"

# Full system backup
docker run --rm \
  --volumes-from 0xray-app \
  -v "$BACKUP_DIR:/backup" \
  alpine:latest \
  tar -czf /backup/app.tar.gz -C /app .

# Database full backup
docker exec 0xray-postgres pg_dumpall -U postgres > "$BACKUP_DIR/database_full.sql"

# Upload to cloud storage
aws s3 sync "$BACKUP_DIR/" "s3://0xray-backups/weekly/$WEEK/"
...
BACKUP_DIR="/opt/0xray/backups/$BACKUP_DATE"

# Stop application
docker-compose down

# Restore configuration
tar -xzf "$BACKUP_DIR/config.tar.gz" -C /opt/0xray/
...
BACKUP_FILE="/opt/0xray/backups/database.sql"

# Stop application
docker-compose stop 0xray-app

# Restore database
docker exec -i 0xray-postgres psql -U 0xray 0xray < "$BACKUP_FILE"

# Start application
docker-compose start 0xray-app
...
wal_level = replica
archive_mode = on
archive_command = 'cp %p /opt/0xray/wal/%f'
restore_command = 'cp /opt/0xray/wal/%f %p'
...
BACKUP_FILE="/opt/0xray/backups/base_backup.sql"
WAL_DIR="/opt/0xray/wal"

# Create recovery configuration
cat > /opt/0xray/recovery.conf << EOF
...
docker exec 0xray-app nc -zv database 5432

# Check database logs
docker logs 0xray-postgres

# Verify connection string
docker exec 0xray-app node -e "console.log(process.env.DATABASE_URL)"

# Test database credentials
docker exec 0xray-postgres psql -U 0xray -d 0xray -c "SELECT 1;"
```
...
docker exec 0xray-app node -e "console.log(process.memoryUsage())"

# Enable garbage collection logging
docker run --env NODE_OPTIONS="--expose-gc --max-old-space-size=512" 0xray/0xray
...
docker exec 0xray-app curl -I https://api.openai.com

# Check DNS resolution
docker exec 0xray-app nslookup api.openai.com

# Verify network policies (Kubernetes)
kubectl get networkpolicies

# Test service mesh connectivity
kubectl exec -it 0xray-pod -- curl -f http://other-service:port/api/status
```
...
docker exec 0xray-app node --prof dist/server.js

# Analyze performance logs
docker logs 0xray-app | grep -i "slow\|timeout\|error"

# Check database query performance
docker exec 0xray-postgres psql -U 0xray -d 0xray -c "SELECT * FROM pg_stat_activity;"
...
kubectl describe certificate 0xray-tls
```
...
docker exec 0xray-postgres bash -c 'echo "log_statement = all" >> /var/lib/postgresql/data/postgresql.conf && pg_ctl reload'

# Kubernetes debug logging
kubectl logs -f deployment/0xray-framework --previous
...
grep -r "ERROR\|FATAL" /opt/0xray/logs/

# Analyze access patterns
tail -f /opt/0xray/logs/access.log | grep " 5[0-9][0-9] "
...
```bash
# Restart application
docker-compose restart 0xray-app

# Force restart with cleanup
docker-compose down && docker-compose up -d

# Kubernetes rollout restart
kubectl rollout restart deployment/0xray-framework
```
...
kubectl scale deployment 0xray-framework --replicas=0

# Investigate root cause
kubectl logs --previous deployment/0xray-framework

# Restore service
kubectl scale deployment 0xray-framework --replicas=3
```
