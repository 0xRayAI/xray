...
class SecurityLogger {
  private logFile = "/var/log/0xray/security.log";
...
class SecureConfig {
  private config: any = {};
  private readonly configFile = "/etc/0xray/config.json";
  private readonly backupDir = "/etc/0xray/backups/";
  ...
  private async validateConfigFilePermissions(): Promise<void> {
    ...
    // Check ownership (should be root or 0xray user)
