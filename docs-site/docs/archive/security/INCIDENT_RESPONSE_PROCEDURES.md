...
hostname
uptime
ps aux | grep -E "(0xray|node|npm)" | head -20
...
sudo systemctl stop 0xray-framework
sudo systemctl stop nginx

# Backup evidence before shutdown
sudo mkdir -p /var/forensics/$(date +%Y%m%d_%H%M%S)
sudo cp -r /var/log/0xray/ /var/forensics/$(date +%Y%m%d_%H%M%S)/
```
...
sudo cp -r /opt/0xray/sessions/ /var/forensics/sessions/
sudo cp /opt/0xray/config/security.json /var/forensics/

# Collect audit logs
sudo journalctl --since "1 hour ago" -u 0xray-framework > /var/forensics/system_logs.txt
sudo cp /var/log/0xray/security-audit.log /var/forensics/

# Preserve plugin execution data
sudo cp -r /opt/0xray/plugins/active/ /var/forensics/plugins/
```
...
sudo find /opt/0xray -name "*.sh" -exec grep -l "nc\|netcat\|bash -i" {} \;
sudo find /opt/0xray -name "*.js" -exec grep -l "eval\|Function\|child_process" {} \;
```
...
sudo find /opt/0xray -name "*backdoor*" -delete
...
sudo systemctl stop 0xray-framework
sudo rsync -av --delete /backup/0xray-clean/ /opt/0xray/
sudo chown -R 0xray:0xray /opt/0xray
...
sudo systemctl start 0xray-framework
...
sudo mv /opt/0xray/plugins/active/compromised-plugin /opt/0xray/plugins/quarantined/
...
cp /backup/plugins/clean/compromised-plugin /opt/0xray/plugins/active/
