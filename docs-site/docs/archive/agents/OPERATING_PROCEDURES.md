...
0xray enforce --check compliance
0xray code-reviewer --validate-changes
```
...
required_status_checks:
  contexts:
    - 0xray/enforcer-compliance
    - 0xray/code-reviewer-validation
    - 0xray/security-auditor-scan
```
...
0xray_validation:
  stage: validate
  script:
    - 0xray orchestrator --workflow pr-validation
```
...
const 0xrayExtension = {
  onCodeChange: async (code, context) => {
    const analysis = await 0xray.codeReviewer.analyze(code);
    return {
...
const completionProvider = {
  provideCompletionItems: async (document, position) => {
    const context = await 0xray.architect.analyzeContext(document, position);
...
0xray_analysis:
  stage: analyze
  script:
    - 0xray orchestrator --workflow full-analysis
  artifacts:
    reports:
...
0xray_review:
  stage: review
  script:
    - 0xray code-reviewer --comprehensive-review
    - 0xray security-auditor --full-scan
  allow_failure: false
```
...
performance_monitoring:
  stage: test
  script:
    - 0xray testing-lead --performance-analysis
  artifacts:
    reports:
...
0xray monitor --system-health
0xray monitor --agent-performance
0xray monitor --workflow-status
```
