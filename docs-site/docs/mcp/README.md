...
The framework repo includes additional `.server.ts` implementations. They are **not** part of the 7-server consumer `.mcp.json`. Codex 69: no new MCP / skill / handler surface. Fat list: [trim list](../architecture/v3-museum.md).

## Marketplace

0xRay is listed on the [xAI Plugin Marketplace](https://github.com/xai-org/plugin-marketplace) — `.grok-plugin/plugin.json`.