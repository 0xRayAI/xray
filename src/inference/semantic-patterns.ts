...
export function analyzeStructuralPatterns(
  ...
  if (stabilitySprint) patterns.push(stabilitySprint);
...
function getFileChanges(fromRef: string, toRef: string): FileChange[] {
  try {
    const output = execSync(
      `git diff --numstat ${fromRef}..${toRef} --no-renames`,
...
function getChangeStatus(
  ...
  try {
    const status = execSync(
      `git diff --name-status ${fromRef}..${toRef} -- ${filePath}`,
...
function detectExtractMethod(
  ...
  for (const dir of sharedDirs) {
    ...
    evidence.push(
      `${addedInDir.length} new files + ${modInDir.length} modified in ${dir}/`,
    );
  ...
  const confidence = Math.min(
    0.95,
    0.6 + added.length * 0.05 + sharedDirs.length * 0.1,
  );
  ...
  return {
    ...
    evidence,
    description:
      "Methods extracted from monolithic modules into dedicated files in the same directory. Classic decomposition pattern: one large file sheds responsibilities into focused modules.",
  };
}

function detectRegistryPattern(
  fromRef: string,
  toRef: string,
): StructuralPattern | null {
  try {
    ...
    if (switchRemovals > 0 && mapAdditions > 0) {
      evidence.push(
        `Removed ${switchRemovals} switch/case lines, added ${mapAdditions} Map operations`,
      );
    ...
    const factoryPattern =
      diff.includes("registerProcessor") || diff.includes("register(");
    if (factoryPattern) {
    ...
    return {
      ...
      evidence,
      description:
        "Switch statement replaced with Map-based registry lookup. O(1) dispatch replaces O(n) case matching. New entries require only registration, not code changes to the dispatcher.",
    };
  } catch {
    return null;
  }
...
function detectFacadePattern(
  ...
  try {
    const diff = execSync(
      `git diff ${fromRef}..${toRef} -- ${modified.map((c) => c.path).join(" ")} --no-renames`,
      { encoding: "utf-8", stdio: "pipe", timeout: 5000 },
    );
    ...
    const removedFunctionLines = diff
      ...
      .filter(
        (l) =>
          l.startsWith("-") &&
          (l.includes("async ") || l.includes("function ") || l.includes("private ")),
      ).length;

    if (removedFunctionLines < 3) return null;

    const evidence: string[] = [];
    for (const f of modified) {
      evidence.push(
        `${f.path}: -${f.deletions}/+${f.insertions} lines, ${removedFunctionLines} function signatures removed`,
      );
    }
    ...
    return {
      ...
      evidence,
      description:
        "Monolithic file stripped down to a thin facade while logic moves to dedicated modules. The original becomes a coordinator, not an implementer.",
    };
  } catch {
    return null;
  }
...
function detectConventionOverConfig(
  changes: FileChange[],
  fromRef: string,
  toRef: string,
): StructuralPattern | null {
  const hasImplementations = changes.some(
    (c) => c.status === "A" && c.path.includes("implementations/"),
  );
  if (!hasImplementations) return null;
  ...
  try {
    ...
    const hasDiscovery =
      diff.includes("discover") || diff.includes("auto-register") || diff.includes("scan");
    const hasReflection =
      diff.includes("reflect") || diff.includes("metadata");
    const hasDynamicImport =
      diff.includes("import(") || diff.includes("require(");

    if (!hasDiscovery && !hasDynamicImport) return null;

    const addedImpl = changes.filter(
      (c) => c.status === "A" && c.path.includes("implementations/"),
    );
    const evidence: string[] = [
      `${addedImpl.length} new implementations auto-discovered`,
    ];
    if (hasDiscovery) evidence.push("Discovery/scan logic present");
    if (hasDynamicImport) evidence.push("Dynamic imports for runtime loading");
    if (hasReflection) evidence.push("Reflection/metadata introspection");
    ...
    return {
      ...
      evidence,
      description:
        "New modules are auto-discovered at runtime instead of manually registered. Drop a file in the right directory and the system finds it. Eliminates registration boilerplate and prevents orphaned modules.",
    };
  } catch {
    return null;
  }
...
function detectDeadCodeRemoval(
  changes: FileChange[],
): StructuralPattern | null {
...
function detectTestExpansion(changes: FileChange[]): StructuralPattern | null {
  const addedTests = changes.filter(
    (c) => c.status === "A" && c.path.includes("__tests__"),
  );
  if (addedTests.length < 2) return null;
...
function detectDependencyInjection(
  ...
  try {
    ...
    const hasStaticDeps =
      diff.includes("+  static readonly dependencies") ||
      diff.includes("+  static dependencies");
    const hasInject = diff.includes("inject") || diff.includes("resolve(");
    const hasConstructorDI =
      diff.includes("constructor(") &&
      (diff.includes("private ") || diff.includes("readonly "));

    if (!hasStaticDeps && !hasInject) return null;

    if (hasStaticDeps) evidence.push("Static dependency declarations added");
    if (hasInject) evidence.push("DI container / injection logic");
    if (hasConstructorDI) evidence.push("Constructor-based DI signatures");
    ...
    return {
      ...
      evidence,
      description:
        "Explicit dependency declarations replacing hardcoded imports. Enables testing with mocks, runtime configuration, and auto-wiring.",
    };
  } catch {
    return null;
  }
...
function detectStabilitySprint(
  changes: FileChange[],
): StructuralPattern | null {
  const fixChanges = changes.filter(
    (c) =>
      c.path.toLowerCase().includes("fix") ||
      c.path.toLowerCase().includes("patch"),
  );
  const totalModified = changes.filter(
    (c) => c.status === "M" || c.status === "A",
  ).length;

  const deleted = changes.filter((c) => c.status === "D");
  if (deleted.length < 3 && fixChanges.length < 3) return null;

  const netDeletions = changes.reduce((s, c) => s + c.deletions - c.insertions, 0);
  if (netDeletions < 100 && deleted.length < 3) return null;
  ...
  return {
    ...
    evidence,
    description:
      "Focus on removal and fixing over feature addition. Technical debt being paid down — a sign of mature development practice.",
  };
}
