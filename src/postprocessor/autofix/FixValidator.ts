...
export class FixValidator {
  ...
  async validateFixes(
    ...
    await frameworkLogger.log(
      ...
      "info",
      { jobId, fixesCount: appliedFixes.length },
    );
    ...
    try {
      ...
      await frameworkLogger.log(
        "fix-validator",
        `fix validation ${validationPassed ? "passed" : "failed"}`,
        validationPassed ? "success" : "error",
        { jobId, appliedFixes: appliedFixes.length },
      );
  ...
  async rollbackFixes(appliedFixes: AppliedFix[]): Promise<void> {
    const jobId = `fix-rollback-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
    ...
    try {
      ...
      for (const fix of appliedFixes) {
        await frameworkLogger.log(
          "fix-validator",
          `rolling back fix: ${fix.description}`,
          "info",
          { jobId, fixType: fix.type, files: fix.files.length },
        );
      }

      await frameworkLogger.log(
        "fix-validator",
        "fix rollback completed",
        "success",
        { jobId },
      );
    } catch (error) {
