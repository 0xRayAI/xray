...
describe('Agent Spawn Governor', () => {
  ...
  describe('Spawn Rate Limiting', () => {
    it('should deny spawn when rate limit exceeded', async () => {
      ...
      const result = await testGovernor.authorizeSpawn({ agentType: 'librarian' });
      expect(result.authorized).toBe(false);
    ...
    it('should allow spawns after rate limit window', async () => {
      ...
      const deniedResult = await testGovernor.authorizeSpawn({ agentType: 'librarian' });
      ...
      const allowedResult = await testGovernor.authorizeSpawn({ agentType: 'librarian' });
  ...
  describe('Statistics and Monitoring', () => {
    ...
    it('should track spawn history correctly', async () => {
      ...
      const result2 = await governor.authorizeSpawn({ agentType: 'orchestrator' });
  ...
  describe('Configuration Validation', () => {
    ...
    it('should use default limits when not specified', async () => {
      ...
      const result = await defaultGovernor.authorizeSpawn({ agentType: 'librarian' });
      expect(result.authorized).toBe(false);
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should maintain state consistency across operations', async () => {
      ...
      const authResult = await governor.authorizeSpawn({ agentType: 'librarian' });
      const trackingId = authResult.trackingId!;
      ...
      // Authorize another
      const authResult2 = await governor.authorizeSpawn({ agentType: 'librarian' });
      ...
      // Authorize third
      const authResult3 = await governor.authorizeSpawn({ agentType: 'librarian' });
