/**
 * 0xRay Framework - Oh-My-OpenCode Integration Tests
 *
...
const mockOhMyOpenCode = {
  ...
  loadPlugin: async (pluginPath: string) => {
    ...
    console.log("Loading mock 0xRay plugin for testing");
    const mockPlugin = {
      "experimental.chat.system.transform": async (input: any, output: any) => {
        output.system = output.system || [];
        output.system.unshift("Mock 0xRay Codex Context");
      },
  ...
  executeTool: async (toolName: string, args: any) => {
    ...
    const xrayPlugin = mockOhMyOpenCode.plugins.get("xray-codex-injector");
    if (xrayPlugin && xrayPlugin["tool.execute.before"]) {
      try {
        await xrayPlugin["tool.execute.before"]({ tool: toolName, args }, {});
      } catch (error: any) {
...
describe("Oh-My-OpenCode Integration", () => {
  beforeAll(async () => {
    // Load the 0xRay plugin into mock OpenCode environment
  ...
  test("should inject codex into system prompts", async () => {
    ...
    expect(
      output.system.some((item: string) => item.includes("0xRay Codex")),
    ).toBe(true);
  });
