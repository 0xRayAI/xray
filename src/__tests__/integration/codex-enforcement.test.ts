/**
 * StringRay AI v1.3.0 - Codex Enforcement Integration Tests
 *
...
describe("Codex Enforcement Integration", () => {
  ...
  test("should allow non-critical tools without enforcement", async () => {
    ...
    await expect(
      hook.hooks["tool.execute.before"](input, mockSessionId),
  ...
  test("should allow critical tools when no codex is loaded", async () => {
    const input = {
      tool: "write",
      args: { filePath: "test.txt", content: "test" },
    };
  ...
  test("should block actions with blocking codex violations", async () => {
    ...
    const result = await hook.hooks["tool.execute.before"](
      input,
      mockSessionId,
    );
  ...
  test("should allow actions with only non-blocking violations", async () => {
    const input = {
      tool: "edit",
      args: { filePath: "test.ts", oldString: "old", newString: "new" },
    };
        ...
        violations: [
          {
            term: {
              number: 17,
              title: "YAGNI",
              description: "Don't implement features you don't need",
            },
  ...
  test("should allow compliant actions", async () => {
    const input = {
      tool: "write",
      args: { filePath: "test.ts", content: 'const x: string = "hello";' },
    };
  ...
  test("should handle enforcement errors gracefully", async () => {
    ...
    const mockContextLoader = {
      loadCodexContext: vi
        .fn()
        .mockRejectedValue(new Error("Context load failed")),
      validateAgainstCodex: vi.fn(),
    };
