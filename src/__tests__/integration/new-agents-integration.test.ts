...
describe("New Agent Integration", () => {
  ...
  describe("Session Management", () => {
    it("should create sessions for agent workflows", () => {
      const session =
        mockSessionCoordinator.initializeSession("integration-test");
    ...
    it("should track agent usage in sessions", async () => {
      ...
      stateManager.set(`session:${session.sessionId}:agents`, [
        "multimodal-looker",
        "analyzer",
      ]);

      // Retrieve agent usage from state
      const storedAgents = stateManager.get(
        `session:${session.sessionId}:agents`,
      ) as string[];
  ...
  describe("Framework Compliance", () => {
    ...
    it("should maintain audit trails", () => {
      ...
      mockSessionCoordinator.trackAgentUsage(
        session.sessionId,
        "multimodal-looker",
      );
