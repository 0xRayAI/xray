...
test('registry source has 22 agents', () => {
  const keys = extractRegistryFromSource();
  if (keys.length !== 22) throw new Error(`Expected 22 agents, got ${keys.length}: ${keys.join(', ')}`);
  if (!keys.includes('multimodal-looker')) throw new Error('multimodal-looker missing from registry');
});
...
test('all registry entries have required fields', () => {
  ...
  for (const field of required) {
    ...
    if (count < keys.length) throw new Error(`Field "${field}" appears ${count} times, expected >= ${keys.length}`);
  }
});
...
const EXTRA_AGENT_FILES = ['librarian-agents-updater'];
...
test('routing-mappings.json loads and is valid JSON array', () => {
  const mappings = readSourceJSON('xray/routing-mappings.json');
...
test('routing rows are hand mappings, not leaked autoGenerated artifacts', () => {
  const mappings = readSourceJSON('xray/routing-mappings.json');
