/**
 * 0xRay Framework - Codex Injector Plugin Tests (Mock-Based)
 *
  ...
  return {
    ...
    hooks: {
      "agent.start": async (sessionId: string) => {
        ...
        console.log("✅ 0xRay Codex loaded: 3 terms, 1 sources");
      },
...
describe("0xRay Codex Injector (Mock-Based)", () => {
  ...
  describe("agent.start hook", () => {
    test("should load codex context and display startup message", async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        "✅ 0xRay Codex loaded: 3 terms, 1 sources",
      );
    });
  ...
  describe("tool.execute.before hook", () => {
    test("should allow valid tool execution", async () => {
      ...
      await expect(
        hook.hooks["tool.execute.before"](input, "session-123"),
  ...
  describe("tool.execute.after hook", () => {
    ...
    test("should handle injection errors gracefully", async () => {
      ...
      const result = await hook.hooks["tool.execute.after"](
        input,
        output,
        "session-123",
      );
