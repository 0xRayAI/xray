...
describe('lead cadence encode — peer can find the beat', () => {
  ...
  const scoring = path.join(root, 'docs', 'memory-scoring.md');
  ...
  it('ships the disk SSOT and catalogs it', () => {
    ...
    const scoringText = readFileSync(scoring, 'utf8');
    ...
    expect(byName.get('heat-is-not-conviction')).toMatch(/a local clause of four definition content words/);
    expect(byName.get('heat-is-not-conviction')).toMatch(/scattered through a long diary are not a hit/);
    expect(byName.get('heat-is-not-conviction')).toMatch(/floor observation count does not change a route/);
    expect(byName.get('heat-is-not-conviction')).toMatch(/learned-conviction\.json/);
