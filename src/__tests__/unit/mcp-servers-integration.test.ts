...
describe('MCP Server Files', () => {
  it('should have bug-triage-specialist server file', () => {
    const serverPath = path.join(process.cwd(), 'src/mcps/knowledge-skills/bug-triage-specialist.server.ts');
    expect(fs.existsSync(serverPath)).toBe(true);
  });

  it('should have log-monitor server file', () => {
    const serverPath = path.join(process.cwd(), 'src/mcps/knowledge-skills/log-monitor.server.ts');
    expect(fs.existsSync(serverPath)).toBe(true);
  });

  it('should have multimodal-looker server file', () => {
    const serverPath = path.join(process.cwd(), 'src/mcps/knowledge-skills/multimodal-looker.server.ts');
    expect(fs.existsSync(serverPath)).toBe(true);
  });

  it('should have analyzer server file', () => {
    const serverPath = path.join(process.cwd(), 'src/mcps/knowledge-skills/analyzer.server.ts');
    expect(fs.existsSync(serverPath)).toBe(true);
  });

  it('should have seo-specialist server file', () => {
    const serverPath = path.join(process.cwd(), 'src/mcps/knowledge-skills/seo-specialist.server.ts');
    expect(fs.existsSync(serverPath)).toBe(true);
  });

  it('should have marketing-expert server file', () => {
    const serverPath = path.join(process.cwd(), 'src/mcps/knowledge-skills/marketing-expert.server.ts');
    expect(fs.existsSync(serverPath)).toBe(true);
  });
});
