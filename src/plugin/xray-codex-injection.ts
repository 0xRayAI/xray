...
async function loadXrayComponents(): Promise<void> {
  if (_featuresConfigLoader) return;
...
export default async function xrayCodexPlugin(input: {
  ...
  return {
    ...
    "tool.execute.before": async (
      ...
      const gateResult = await beforeToolHook(tool, args as Record<string, unknown>);
    ...
    "tool.execute.after": async (
      ...
      if (tool && (tool.includes('write') || tool.includes('edit') || tool.includes('create') || tool.includes('modify'))) {
        try {
          const afterResult = await afterToolHook(tool, args as Record<string, unknown>, result, null);
