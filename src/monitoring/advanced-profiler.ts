...
export class AdvancedProfiler extends EventEmitter {
  ...
  private detectAnomalies(profileData: ProfileData): void {
    if (!profileData.duration) return;
  ...
  private calculateMemoryDelta(profileData: ProfileData): number {
    if (!profileData.memoryUsage.after) return 0;
  ...
  exportProfiles(): Record<string, ProfileData[]> {
    const exportData: Record<string, ProfileData[]> = {};
    for (const [agentName, profiles] of Array.from(this.profiles.entries())) {
