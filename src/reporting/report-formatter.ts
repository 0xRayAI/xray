...
export function formatAsMarkdown(data: ReportData): string {
...
5. **Codex Enforcement**: Rules implemented with zero-tolerance blocking

### System Performance Metrics
- **Event Processing**: ${data.summary.totalEvents} events processed over operational window
- **Coordination Efficiency**: ${data.metrics.totalDelegations} agent delegations with ${data.metrics.successRate}% completion rate
...
### Framework Status
**xray** operational metrics (compat shim):
- Complete agent-MCP integration
- Architectural integrity validation
- Enterprise-grade logging and monitoring
- ${data.metrics.successRate}% operation success rate
- ${data.summary.totalEvents} total events tracked
...
export function formatAsHtml(data: ReportData): string {
  return `
<!DOCTYPE html>
<html>
<head>
