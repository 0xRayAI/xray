...
import { getKernel, type KernelInferenceResult, type AdaptiveThresholds, type EmergentPattern, type PatternUpdate } from "../core/kernel-patterns.js";
...
export class AdaptiveKernelAnalyzer {
  private adaptiveConfig: AdaptiveKernelConfig;
  private baseKernel: ReturnType<typeof getKernel>;
  private lastLearningRun: Date = new Date();
  private cachedP9Analysis: P9AnalysisResult | null = null;
  private cacheValidMs = 60000; // 1 minute cache
   ...
   */
  private performP9Analysis(baseResult: KernelInferenceResult, _observation: string): P9AnalysisResult {
    const p9Result: P9AnalysisResult = {
      ...baseResult,
      patternDriftDetected: false,
      driftAnalyses: [],
      emergentPatterns: [],
    ...
    if (significantDrift.length > 0) {
      ...
      p9Result.driftAnalyses = significantDrift.map((a) => ({
        ...
        driftDirection: a.driftDirection as 'increasing' | 'decreasing' | 'unstable',
    ...
    if (timeSinceLastLearning >= this.adaptiveConfig.learningIntervalMs) {
      const learningResult = patternLearningEngine.learnFromData(
        [], // Will be populated from actual routing outcomes
  ...
  getLearningStats(): {
    ...
    return {
      ...
      driftDetected: allDrift.filter((a: { drifted: boolean }) => a.drifted).length,
...
// Singleton instance
let adaptiveKernelInstance: AdaptiveKernelAnalyzer | null = null;

export function getAdaptiveKernel(config?: Partial<AdaptiveKernelConfig>): AdaptiveKernelAnalyzer {
