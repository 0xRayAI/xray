 ...
 */
const DEFAULT_CONFIG: ContextSizeConfig = {
 ...
 */
function detectDuplicatePatterns(content: string): string[] {
  const patterns = [
 ...
 */
function compressCodexContent(content: string, maxLength: number): string {
...
export function validateContext(
  context: string,
  config: Partial<ContextSizeConfig> = {}
): ValidationResult {
 ...
 */
export function validateContextCollection(
  contexts: string[],
  config: Partial<ContextSizeConfig> = {}
): ValidationResult {
  const finalConfig = { ...DEFAULT_CONFIG, ...config };
 ...
 */
export function preventSystemPromptBloat(
  context: string,
 ...
 */
export function isPromptLikelyToOverflow(prompt: string): boolean {
 ...
 */
export function generateSafeSystemPrompt(
  baseTemplate: string,
  additionalContext: string,
  maxTokens: number = 6000
): { prompt: string; safe: boolean } {
