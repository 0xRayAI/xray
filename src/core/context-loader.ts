...
function isValidMatch(
  ...
  [K in keyof RegExpMatchArray]: K extends typeof index
    ? string
    : RegExpMatchArray[K];
} {
...
export class StringRayContextLoader {
  ...
  public async loadCodexContext(
    ...
    return {
      success: false,
      error:
        "No valid codex file found. Checked: " + this.codexFilePaths.join(", "),
      warnings,
    };
  }
  ...
  public getZeroToleranceTerms(context: CodexContext): CodexTerm[] {
    return Array.from(context.terms.values())
      .filter(
        (term) => term.zeroTolerance || term.enforcementLevel === "blocking",
      )
      .sort((a, b) => a.number - b.number);
  }
  ...
  public validateAgainstCodex(
    ...
    const codeContent =
      typeof actionDetails.code === "string" ? actionDetails.code : "";
    if (
      ...
      if (term7) {
        violations.push({
          term: term7,
          reason:
            "Unresolved tasks detected - violates Resolve All Errors principle",
          severity: "blocking",
        });
      }
    }
    ...
    if (actionDetails.isOverEngineered) {
      ...
      if (term3) {
        violations.push({
          term: term3,
          reason:
            "Solution appears over-engineered - violates simplicity principle",
