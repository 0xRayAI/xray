...
import type { ComplexityMetrics, ComplexityThresholds } from './complexity-analyzer.js';
...
export class ComplexityCalibrator {
  ...
  async calibrate(minSamples: number = 10): Promise<CalibrationResult | null> {
    ...
    const accuracyHistory = {
      underestimated: data.filter(d => d.accuracy === 'underestimated').length,
  ...
  private parseLogEntry(line: string): CalibrationData | null {
    ...
    const complexityScore = complexityMatch ? parseInt(complexityMatch[1] || '50') : 50;

    // Extract duration if available
    const durationMatch = line.match(/duration[":\s]+(\d+)/i);
    const actualDuration = durationMatch ? parseInt(durationMatch[1] || '0') : 0;

    // Extract timestamp
    const timestampMatch = line.match(/^(\d{4}-\d{2}-\d{2}T[\d:.]+Z)/);
    const timestamp = timestampMatch ? (timestampMatch[1] || new Date().toISOString()) : new Date().toISOString();
  ...
  private calculateWeightAdjustments(data: CalibrationData[]): {
    ...
    const adjustmentFactor = total > 0 
   ...
   */
  private calculateThresholdAdjustments(data: CalibrationData[]): ComplexityThresholds {
