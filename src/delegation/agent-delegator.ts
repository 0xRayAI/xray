...
export class AgentDelegator {
  ...
  private configLoader: typeof xrayConfigLoader;
  ...
  constructor(
    ...
    configLoader: typeof xrayConfigLoader,
  ) {
      ...
      try {
        if (fs.existsSync(p)) {
          ...
          for (const entry of parsed) {
            if ((entry as any).autoGenerated === true) {
              frameworkLogger.log("agent-delegator", "routing-filtered", "warning", {
                reason: "autoGenerated flag",
                agent: entry.agent,
              });
              continue;
            }
            if (entry.agent && !isAllowedAgent(entry.agent)) {
              frameworkLogger.log("agent-delegator", "routing-filtered", "warning", {
   ...
   */
  private static readonly ALLOWED_AGENTS = new Set(getActiveAgents());
