...
import type {
  VoteChoice,
  VotingResultDetail,
  HistoricalWeightConfig,
  AgentHistoricalPerformance,
  ConfidenceWeightConfig,
  WeightedVoteAggregation,
  AgentPerformanceEntry,
} from "./voting-types.js";
import { frameworkLogger } from "../core/framework-logger.js";

export const DEFAULT_HISTORICAL_CONFIG: HistoricalWeightConfig = {
  decayFactor: 0.95,
  minWeight: 0.1,
  maxWeight: 1.5,
...
export const DEFAULT_CONFIDENCE_CONFIG: ConfidenceWeightConfig = {
...
export class WeightedVotingAggregator {
  private historicalConfig: HistoricalWeightConfig;
  private confidenceConfig: ConfidenceWeightConfig;
  private agentPerformance: Map<string, AgentHistoricalPerformance>;

  constructor(
    historicalConfig: Partial<HistoricalWeightConfig> = {},
    confidenceConfig: Partial<ConfidenceWeightConfig> = {},
  ) {
  ...
  calculateHistoricalWeight(agentName: string): number {
    ...
    return Math.max(
      this.historicalConfig.minWeight,
      Math.min(this.historicalConfig.maxWeight, historicalWeight),
    );
  }
  ...
  aggregateVotes(votes: VoteChoice[]): WeightedVoteAggregation[] {
    const aggregated = new Map<string, WeightedVoteAggregation>();
    ...
    if (this.confidenceConfig.normalizeScores) {
      const totalWeight = Array.from(aggregated.values()).reduce(
        (sum, agg) => sum + agg.totalWeight,
        0,
      );
  ...
  updateAgentPerformance(
    ...
    const newAvg =
      (performance.averageConfidence * (performance.totalVotes - 1) + confidence) /
      performance.totalVotes;
    ...
    performance.weightedScore = Math.max(
      0,
      Math.min(
        100,
        successRate * 50 + confidenceFactor * 30 + streakBonus * 20 + 20,
      ),
    );
...
export const createWeightedVotingAggregator = (
  historicalConfig?: Partial<HistoricalWeightConfig>,
  confidenceConfig?: Partial<ConfidenceWeightConfig>,
): WeightedVotingAggregator => {
...
export function createDetailedVotingResult(
  votes: VoteChoice[],
  ...
  const details: VotingResultDetail[] = votes.map((vote) => ({
