...
export class AgentsMdValidationProcessor {
  ...
  private getSectionTemplate(section: string): string {
    ...
    const templates: Record<string, string> = {
      ...
      "## Available Skills": [
        ...
        "npx 0xray skill:install <name>       # Install from registry",
        "npx 0xray skill:registry list        # Show all registry sources",
        "```",
        "",
      ].join("\n"),
      "## Codex": [
      ...
      "## CLI Commands": [
        ...
        "",
        "```bash",
        "npx 0xray install       # Install and configure",
        "npx 0xray status       # Check configuration",
        "npx 0xray health        # Health check",
        "npx 0xray validate      # Validate installation",
        "```",
        "",
      ].join("\n"),
    };
  ...
  async appendMissing(): Promise<{
    ...
    try {
      ...
      await frameworkLogger.log(
        "agents-md-validation-processor",
        "-agents-md-sections-appended-",
        "info",
        { added: missingSections, path: this.agentsPath },
      );
