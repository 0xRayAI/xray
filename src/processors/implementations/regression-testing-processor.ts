...
export class RegressionTestingProcessor extends PostProcessor {
  ...
  protected async run(context: ProcessorContext): Promise<unknown> {
    ...
    if (result.passed) {
      frameworkLogger.log(
        ...
        "tests-passed",
        "info",
        { filePath, testsRun: result.testsRun, duration: result.duration }
      );
      ...
      frameworkLogger.log(
        ...
        "tests-failed",
        "error",
        { filePath, failures: result.failures, errors: result.errors }
      );
  ...
  private async runRegressionTests(filePath: string): Promise<RegressionTestResult> {
