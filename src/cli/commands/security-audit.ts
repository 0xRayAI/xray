...
} from "../../security/comprehensive-security-audit.js";
...
function parseArgs(args: string[]): SecurityAuditOptions {
...
async function runSecurityAudit(options: SecurityAuditOptions): Promise<SecurityAuditReport> {
...
function formatReport(report: SecurityAuditReport, format: "json" | "markdown"): string {
  if (format === "json") {
...
function printSummary(report: SecurityAuditReport): void {
...
function saveReport(content: string, outputPath: string): void {
...
export async function securityAuditCommand(args: string[] = process.argv.slice(2)): Promise<void> {
  try {
