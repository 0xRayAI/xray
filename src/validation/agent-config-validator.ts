...
export class AgentConfigValidator {
  ...
  async validateConfigFile(filePath: string): Promise<ValidationResult> {
    const jobId = `validate-config-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`;
   ...
   */
  async validateAllAgentConfigs(agentConfigDir = ".opencode/agents"): Promise<{
    ...
    frameworkLogger.log(
      "agent-config-validator",
      `Validated ${summary.total} agent configurations: ${summary.valid} valid, ${summary.invalid} invalid`,
      summary.invalid > 0 ? "error" : "info",
    );
