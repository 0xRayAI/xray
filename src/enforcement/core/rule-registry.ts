...
export class RuleRegistry implements IRuleRegistry {
  ...
  addRule(rule: RuleDefinition): void {
    if (this.rules.has(rule.id)) {
      // Idempotent: update existing rule instead of throwing on duplicate.
      // This handles async loaders that may re-register rules when the
      // enforcer is constructed multiple times (e.g. singleton + tests).
