...
export class PerformanceRegressionReportingValidator extends BaseValidator {
  ...
  async validate(context: RuleValidationContext): Promise<RuleValidationResult> {
    ...
    if (hasPerfCode) {
      ...
      if (!hasReporting) {
        return this.createFailureResult(
          "Performance code lacks measurement/reporting",
          ["Add performance.now() or console.time()", "Include metrics reporting"],
        );
      }
    }
    ...
    if (hasSecurityCode) {
      ...
      if (exposesSecrets) {
        return this.createFailureResult(
          ...
          ["Remove console.log of sensitive data", "Use secure logging"],
        );
      }
    }
