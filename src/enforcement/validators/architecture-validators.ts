...
/**
 * Validates boot wiring completeness (Codex Term #74).
 * Detects exported functions with boot-relevant naming patterns (register*, init*, boot*, setup*)
 * that are defined and possibly tested but may not be wired into any boot sequence.
 * Flags when new boot-like functions appear without corresponding calls in known boot files.
 */
export class BootWiringValidator extends BaseValidator {
  readonly id = "boot-wiring-validator";
  readonly ruleId = "boot-wiring";
  readonly category = "architecture" as const;
  readonly severity = "error" as const;

  /** Known boot entry point file patterns */
  private readonly bootFilePatterns = [
    /orchestrat[^/]*\.ts$/,
    /boot[^/]*\.ts$/,
    /init[^/]*\.ts$/,
    /kernel[^/]*\.ts$/,
  ...
  /** Function name patterns suggesting boot-relevant behavior */
  private readonly bootFuncPatterns = [
    /^register[A-Z]/,
    /^init[A-Z]/,
    /^boot[A-Z]/,
    /^setup[A-Z]/,
  ...
  async validate(context: RuleValidationContext): Promise<RuleValidationResult> {
    ...
    if (!newCode || !files || files.length === 0) {
      return this.createSuccessResult("No code to validate for boot wiring");
    }

    const violations: string[] = [];
    const suggestions: string[] = [];

    // Check if any changed files are putative boot files
    const hasBootFiles = files.some((f) =>
      this.bootFilePatterns.some((p) => p.test(f)),
    );

    // Collect unique exported function/const names
    const exportMatches = newCode.match(
      /export\s+(?:async\s+)?(?:function\s+(\w+)|const\s+(\w+)\s*=\s*(?:async\s*)?\(|default\s+(?:async\s+)?function\s+(\w+))/g,
    ) || [];

    const exportedNames: string[] = [];
    for (const match of exportMatches) {
      const name = match.match(/(?:function|const|default)\s+(\w+)/);
      if (name?.[1]) exportedNames.push(name[1]);
    }

    const namedExports = newCode.match(/export\s+const\s+(\w+)/g) || [];
    for (const match of namedExports) {
      const name = match.match(/export\s+const\s+(\w+)/);
      if (name?.[1]) exportedNames.push(name[1]);
    }

    // Flag exported boot-like functions not accompanied by boot file changes
    for (const name of [...new Set(exportedNames)]) {
      const isBootFunc = this.bootFuncPatterns.some((p) => p.test(name));
      if (!isBootFunc) continue;

      violations.push(
        `Exported boot-like function '${name}' detected without boot sequence updates`,
      );
      suggestions.push(
        `Ensure ${name}() is called in a boot entry point (orchestrator.bootSteps, init path, etc.)`,
      );
    }

    // When boot files ARE in the change set, also flag files containing
    // boot-like internal calls that reference a new export by name
    if (hasBootFiles && exportedNames.length > 0) {
      for (const name of [...new Set(exportedNames)]) {
        const isBootFunc = this.bootFuncPatterns.some((p) => p.test(name));
        if (!isBootFunc) continue;
        violations.push(
          `Modified boot files exist but no call to '${name}()' detected in them — verify wiring`,
        );
        suggestions.push(
          `Add a call to ${name}() in the appropriate boot sequence file`,
        );
      }
    }
    ...
    if (violations.length > 0) {
      return this.createFailureResult(
        `Boot wiring concerns: ${violations.join("; ")}`,
        [...new Set(suggestions)],
      );
    }

    return this.createSuccessResult("Boot wiring patterns validated");
  }
}
