...
import { XrayKnowledgeSkillBase } from '../mcps/shared/knowledge-skill-base.js';
 ...
 */
function toServerDescriptor(
  instance: XrayKnowledgeSkillBase,
  serverName: string,
): { name: string; tools: ToolDefinition[]; callTool: (toolName: string, args: Record<string, unknown>) => Promise<unknown> } {
  const srv = instance as unknown as {
...
function registerOne(
  label: string,
  create: () => XrayKnowledgeSkillBase,
  serverName: string,
): boolean {
  try {
    const instance = create();
    const descriptor = toServerDescriptor(instance, serverName);
...
function registerBatch(tag: string, servers: Array<{
  label: string;
  create: () => XrayKnowledgeSkillBase;
  serverName: string;
}>): number {
  ...
  for (const s of servers) {
    if (registerOne(s.label, s.create, s.serverName)) {
      count++;
    }
  }
 ...
 */
export function registerDefaultPlugins(): DefaultPluginsResult {
  const failed: string[] = [];

  // ── Batch 1: Governance-critical (priority) ──
  const batch1Count = registerBatch('governance', [
  ...
  const batch2Count = registerBatch('design-architecture', [
  ...
  const batch3Count = registerBatch('analysis-quality', [
  ...
  const batch4Count = registerBatch('operations-workflow', [
  ...
  const batch5Count = registerBatch('content-growth', [
  ...
  // ── Batch 6: Specialized ──
  const batch6Count = registerBatch('specialized', [
    { label: 'bug-triage-specialist', create: () => new BugTriageSpecialistServer(), serverName: 'bug-triage-specialist' },
    { label: 'skill-invocation',      create: () => new SkillInvocationServer(),     serverName: 'xray/skill-invocation' },
  ]);
