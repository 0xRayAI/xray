...
class XrayStateManagerServer extends XrayKnowledgeSkillBase {
  private state: Map<string, unknown> = new Map();
  private stateFile: string;
  private backups: Map<string, unknown> = new Map();

  constructor() {
    super("state-manager", "3.1.0");
  ...
  override setupToolHandlers(): void {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        frameworkLogger.log("mcps/state-manager", "tool", "error", { tool: name, error: String(error) });
        throw error;
      }
    });
  }
