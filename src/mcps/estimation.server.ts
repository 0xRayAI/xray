...
class EstimationServer extends XrayKnowledgeSkillBase {
  ...
  constructor() {
    super("estimation-validator", "3.1.0");
    this.tools = [
      {
    ...
    this.handlers = {
      ...
      "get-accuracy-report": async () => this.handleGetReport(),
    };
...
if (import.meta.url === `file://${fs.realpathSync(process.argv[1]!)}`) {
  ...
  server.run().catch((error) => frameworkLogger.log("mcps/estimation", "run", "error", { error: String(error) }));
}
