...
class StrRayRefactoringStrategiesServer {
  ...
  async run(): Promise<void> {
    ...
    await frameworkLogger.log('refactoring-strategies.server', '-strray-refactoring-strategies-mcp-server-running-', 'info', { message: "StrRay Refactoring Strategies MCP Server running..." });

    const cleanup = async (signal: string) => {
  await frameworkLogger.log('refactoring-strategies.server', '-received-signal-shutting-down-gracefully-', 'info', { message: `Received ${signal}, shutting down gracefully...` });
  ...
  try {
    ...
    await frameworkLogger.log('refactoring-strategies.server', '-strray-mcp-server-shut-down-gracefully-', 'info', { message: "StrRay MCP Server shut down gracefully" });
    process.exit(0);
  } catch (error) {
    clearTimeout(timeout);
...
const checkParent = async () => {
  try {
    ...
    await frameworkLogger.log('refactoring-strategies.server', '-parent-process-opencode-died-shutting-down-mcp-se', 'info', { message: 'Parent process (opencode) died, shutting down MCP server...' });
