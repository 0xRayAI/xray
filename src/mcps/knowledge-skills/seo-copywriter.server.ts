...
class SEOCopywriterServer {
  ...
  private tools: Tool[] = [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          contentType: { type: "string", enum: ["blog", "product", "landing", "email", "social"] },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          primaryKeyword: { type: "string", description: "Primary keyword to target" }
        },
  ...
  constructor() {
    this.server = new Server(
      { name: "seo-copywriter", version: "1.0.0" },
      { capabilities: { tools: {} } }
    );
      ...
      try {
        switch (name) {
          case "seo_content_generation": {
            ...
            const targetAudience = (argsObj.targetAudience as string) || "general";
            return {
              content: [{
                type: "text",
                text: JSON.stringify({
                  ...
                  optimized: true,
                  sections: [
                    { heading: "Introduction", words: Math.floor(wordCount * 0.2) },
                    { heading: "Main Content", words: Math.floor(wordCount * 0.5) },
                    { heading: "Conclusion", words: Math.floor(wordCount * 0.3) }
                  ],
                  keywords: [topic, `${topic} guide`, `best ${topic}`, `how to ${topic}`],
