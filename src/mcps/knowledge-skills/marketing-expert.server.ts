...
class MarketingExpertServer {
  ...
  private tools: Tool[] = [
    {
      ...
      inputSchema: {
        type: "object",
        properties: {
          campaignGoal: { type: "string", description: "Primary campaign objective" },
    ...
    },
    {
      ...
      inputSchema: {
        type: "object",
        properties: {
          pageType: { type: "string", enum: ["landing", "product", "checkout", "signup"] },
          currentConversion: { type: "number", description: "Current conversion rate %" }
        },
        required: ["pageType"]
      }
    },
  ...
  constructor() {
    this.server = new Server(
      { name: "marketing-expert", version: "1.0.0" },
      { capabilities: { tools: {} } }
    );
      ...
      try {
        switch (name) {
              ...
              content: [{
                ...
                text: JSON.stringify({
                  ...
                  strategy: {
                    ...
                    budgetAllocation: { content: 30, email: 20, social: 25, paid: 25 },
                    timeline: argsObj.timeline || "3 months",
                    kpis: ["CTR", "Conversion Rate", "CAC", "LTV", "ROAS"]
                  },
                  phases: [
                    { name: "Awareness", duration: "4 weeks", goal: "Reach 100k impressions" },
                    { name: "Consideration", duration: "6 weeks", goal: "5% conversion" },
                    { name: "Conversion", duration: "4 weeks", goal: "2% purchase" }
                  ]
                }, null, 2)
              }]
            };
              ...
              content: [{
                ...
                text: JSON.stringify({
                  ...
                  "dontMakeMeThink": {
                    score: 68,
                    issues: [
                      { priority: "high", issue: "Hero value prop not clear in 3 seconds", fix: "Above fold: headline + subhead + CTA" },
                      { priority: "high", issue: "Navigation unclear", fix: "Max 5 nav items, use familiar labels" },
                      { priority: "medium", issue: "Too many choices", fix: "Reduce to one primary CTA" }
                    ]
                  },
                  sections: {
                    hero: { status: "needs-work", recommendation: "Clear H1 + benefit statement + CTA in 3 seconds" },
                    socialProof: { status: "good", recommendation: "Add testimonials above fold" },
                    problem: { status: "needs-work", recommendation: "Lead with customer pain point" },
                    solution: { status: "good", recommendation: "Show product/screenshot clearly" },
                    benefits: { status: "good", recommendation: "Use bullet points, not paragraphs" },
                    cta: { status: "needs-work", recommendation: "Make CTA button stand out (color, size, text)" },
                    riskReversal: { status: "missing", recommendation: "Add guarantees, testimonials" },
                    faq: { status: "good", recommendation: "Address objections" }
                  },
          ...
          case "conversion_optimization": {
            ...
            const currentConversion = (argsObj.currentConversion as number) || 2;
            return {
              content: [{
                type: "text",
                text: JSON.stringify({
                  ...
                  recommendations: [
                    { priority: "high", change: "Single clear CTA", impact: "+25%" },
                    { priority: "high", change: "Social proof near CTA", impact: "+15%" },
                    { priority: "medium", change: "Remove navigation distractions", impact: "+10%" },
                    { priority: "medium", change: "Add urgency indicators", impact: "+10%" },
                    { priority: "low", change: "Reduce form fields", impact: "+5%" }
                  ],
                  abTests: [
                    { name: "CTA Color Test", hypothesis: "Contrast color increases clicks" },
                    { name: "Headline Test", hypothesis: "Benefit-driven headline outperforms feature" },
                    { name: "Social Proof Position", hypothesis: "Testimonial placement above vs below fold" }
                  ]
                }, null, 2)
              }]
            };
