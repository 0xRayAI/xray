...
class SEOSpecialistServer {
  ...
  private tools: Tool[] = [
    {
      ...
      description: "Audit codebase for technical SEO issues: schema markup, meta tags, heading structure, internal linking, performance",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description: "Generate JSON-LD schema markup for: Organization, Product, Article, FAQ, Breadcrumb, LocalBusiness",
      inputSchema: {
        type: "object",
        properties: {
          schemaType: { type: "string", enum: ["Organization", "Product", "Article", "FAQPage", "BreadcrumbList", "LocalBusiness"] },
          data: { type: "object", description: "Data to populate schema", properties: { name: { type: "string" }, url: { type: "string" }, description: { type: "string" } } }
        },
        required: ["schemaType"]
      }
    },
    {
      ...
      description: "Validate robots.txt for SEO best practices and AI crawler optimization",
      inputSchema: {
        type: "object",
        properties: {
          content: { type: "string", description: "Current robots.txt content" }
        }
      }
    },
    {
      name: "core_web_vitals_check",
      description: "Analyze and provide optimization recommendations for LCP, INP, CLS",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      description: "Optimize content for AI search engines (ChatGPT, Perplexity, Grok, Gemini)",
      inputSchema: {
        type: "object",
        properties: {
          content: { type: "string", description: "Content to optimize" },
          targetAI: { type: "array", items: { type: "string" }, description: "Target AI engines" }
        }
      }
    }
  ];
  ...
  constructor() {
    this.server = new Server(
      { name: "seo-specialist", version: "1.0.0" },
      { capabilities: { tools: {} } }
    );
            ...
            return {
              content: [{
                ...
                text: JSON.stringify({
                  score: 85,
                  issues: [
                    { priority: "high", issue: "Missing JSON-LD schema on homepage", fix: "Add Organization schema" },
                    { priority: "medium", issue: "LCP above 2.5s", fix: "Optimize hero image" }
                  ]
                }, null, 2)
              }]
            };
