...
interface BugReport {
  ...
  category: "runtime" | "logic" | "memory" | "performance" | "concurrency" | "type" | "configuration";
  file?: string;
  line?: number;
  stackTrace?: string;
  errorMessage?: string;
...
class BugTriageSpecialistServer {
  ...
  constructor() {
    this.server = new Server(
      { name: "bug-triage-specialist", version: "1.6.0" },
      { capabilities: { tools: {} } }
    );
  ...
  private setupToolHandlers() {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              context: {
                type: "object",
                properties: {
                  projectType: { type: "string", description: "Type of project (node, browser, etc.)" },
                  framework: { type: "string", description: "Framework being used" },
                  recentChanges: { type: "array", items: { type: "string" }, description: "Recent code changes" }
                },
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              sourceMap: { type: "object", description: "Optional source map for minified code" }
            },
            required: ["stackTrace"]
          }
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              existingCode: { type: "string", description: "The buggy code snippet" }
            },
        ...
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              sprintVelocity: { type: "number", description: "Available story points" }
            },
            required: ["bugs"]
          }
        },
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              existingBugs: { type: "array", items: { type: "string" }, description: "List of existing bug IDs" }
            },
            required: ["newBug"]
          }
        }
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          case "triage_bugs": {
            const result = this.triageBugs((params.errorLogs as string[]) || [], (params.context as Record<string, unknown>) || {});
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
          }
          case "analyze_stack_trace": {
            const result = this.analyzeStackTrace((params.stackTrace as string) || "", params.sourceMap);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
          }
          case "suggest_fixes": {
            const result = this.suggestFixes((params.bugId as string) || "", (params.language as string) || "typescript", (params.existingCode as string) || "");
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
          }
          case "prioritize_issues": {
            const result = this.prioritizeIssues((params.bugs as any[]) || [], (params.sprintVelocity as number) || 20);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
          }
          case "find_related_issues": {
            const result = this.findRelatedIssues((params.newBug as string) || "", (params.existingBugs as string[]) || []);
            return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
          }
          default:
  ...
  private triageBugs(errorLogs: string[], context: any): TriageResult {
    ...
    const rootCausePatterns = bugs.reduce((acc, bug) => {
    ...
    const summary = {
      ...
      avgConfidence: Math.floor(bugs.reduce((a, b) => a + b.confidence, 0) / bugs.length),
  ...
  private analyzeStackTrace(stackTrace: string, sourceMap?: any) {
    ...
    const probableCulprit = frames.find(f => 'file' in f && f.file && !f.file.includes('node_modules'));
    ...
    return {
      ...
      analysis: {
        entryPoint: frames[0],
        lastUserCode: frames.find(f => 'file' in f && f.file && !f.file.includes('node_modules') && !f.file.includes('internal/')),
        framework: frames.find(f => 'file' in f && f.file?.includes('node_modules')),
  ...
  private prioritizeIssues(bugs: any[], sprintVelocity: number) {
    const prioritized = bugs.map(bug => ({
      ...bug,
  ...
  private findRelatedIssues(newBug: string, existingBugs: string[]) {
    ...
    return {
      ...
      related: related,
      duplicateLikelihood: related.length > 0 ? Math.min(related.length * 30, 95) : 0,
      suggestion: related.length > 0 ? "Consider marking as duplicate" : "New unique issue"
    };
  }

  // Helper methods
  private estimateSeverity(log: string): "critical" | "high" | "medium" | "low" {
    const lower = log.toLowerCase();
    if (lower.includes('fatal') || lower.includes('crash') || lower.includes('memory')) return "critical";
  ...
  private categorizeBug(log: string): BugReport["category"] {
    const lower = log.toLowerCase();
    if (lower.includes('undefined') || lower.includes('null') || lower.includes('type')) return "type";
    ...
    if (lower.includes('async') || lower.includes('promise') || lower.includes('race')) return "concurrency";
  ...
  private estimateFixComplexity(category: string): "simple" | "moderate" | "complex" {
  ...
  private suggestFixFromTrace(frames: any[]): string {
    const userFrame = frames.find(f => 'file' in f && f.file && !f.file.includes('node_modules'));
    if (userFrame) {
  ...
  private calculatePriorityScore(bug: any): number {
    ...
    const impactWeight: Record<string, number> = { high: 30, medium: 20, low: 10 };
    return (severityWeight[bug.severity as string] || 10) + (impactWeight[bug.impact as string] || 10);
  }

  private estimateEffort(bug: any): number {
    const effortMap: Record<string, number> = { simple: 1, moderate: 3, complex: 8 };
