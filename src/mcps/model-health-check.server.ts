...
class XrayModelHealthCheckServer extends XrayKnowledgeSkillBase {
  constructor() {
    super("model-health-check", "3.1.0");
    ...
    this.handlers = {
      "model-health-check": async (args) => this.handleModelHealthCheck((args || {}) as ModelHealthCheckArgs),
    };
...
if (import.meta.url === `file://${fs.realpathSync(process.argv[1]!)}`) {
  ...
  server.run().catch((error) => frameworkLogger.log("mcps/model-health-check", "run", "error", { error: String(error) }));
}
