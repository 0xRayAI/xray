...
- **Publish OTP latch** — every time run `npm publish --access public` (no `--auth-type=web`, no `--otp=` from chat). Paste the clickable `auth/cli` URL. Do not ask for a 6-digit authenticator code in chat. If `EOTP`/`ENEEDAUTH` with no URL: `npm logout` and run the command again until the URL prints. Do not press Enter. Do not hand publish to a browser subagent. Poll `npm view` until the version is live. Then prove **fresh** and **upgrade** registry installs. `npm view` is not an install.
