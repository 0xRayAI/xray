...
### ✨ Features
- **Factory-blip Rippel v2** — sharp look (`stampFocusDisc`) + complex tempo/frequency animation on all five viz (`orb` `swirl` `snap` `waves` `spark`). Video LFOs share the crystal-mill genre tempo with the mandatory 4.44s bed. `still` is the Power Plant ident (hard-cut plates over 4.44s at ≥720p), not a frozen poster. No `@0xray/foundry` bump.
