...
### ✨ Features
- **Factory-blip Rippel v2** — sharp look (`stampFocusDisc`) + complex tempo/frequency animation on all five viz (`orb` `swirl` `snap` `waves` `spark`). One sparse seed cage (12 families, gait). Uniqueness lives in the field — stars, grids (`floor`/`meridian`/`ticks`), gradients, blinkers — not more wire in the middle. Receipt `visualConfig.mesh` + `visualConfig.field` are the fingerprints. Audio syncopates to the motion grid (same seed, tempo, phase0=0 — downbeat kick + AND hat). `still` is the Power Plant ident (hard-cut plates over 4.44s at ≥720p), not a frozen poster. No `@0xray/foundry` bump.
