...
- **Hot-swap station card** — `.xray/state/STATION.md` is the projection of `session-boot.json` (intent, git, plan, last host, Repertoire). PreCompact/PostCompact persist it. AGENTS.md tells the model to Read it. Host change sets `hotSwap`. Same-host compact **keeps** the last swap stamp. Intent strips `<user_query>` wrappers. Works without Repertoire.
