...
- **Repertoire project copy** — shipped `memory_routing.config.signalsPath` is `.xray/state/repertoire/curated_signals.json`. The factory tarball stays read-only. After compact the suit routes living laws, not hangar `repo-*` or the 8-name seed. Guides: `memory-wake`, `station-vs-repertoire`, `memory-routing`, `repertoire`, `features-since-3.1` Unreleased dest-is-laws.
