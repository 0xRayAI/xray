...
### Fixed
- **A governed proposal teaches.** When the inference cycle finishes a vote, it steps the signals that proposal's own sessions already named. Approve steps the average up by a tenth. Reject steps it down by a tenth. Needs-revision, a governance error, and a proposal whose sessions named nothing do not write. A route lookup does not write. Not a new skill.
