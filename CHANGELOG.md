...
- **Hero invariants.** Capture priority is an invariant you would otherwise re-read the source to relearn. Names: `heat-is-not-conviction`, `inference-cycle-invariant`, `governance-vote-invariant`, `thin-dispatch-invariant`. Heat at 0.55 is not conviction. Repertoire `mergeStackOverlay` does not pass `fleshGeneric`, so an existing stack definition stays until the xray wake hook `refreshStackLaw` refreshes it and keeps observation stats. Not a new skill.
