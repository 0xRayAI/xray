...
### ✨ Features
- **Factory-blip Rippel v2** — sharp look (`stampFocusDisc`) + complex tempo/frequency animation on all five viz (`orb` `swirl` `snap` `waves` `spark`). Seed mesh fills the frame (12 families, gait / shells / faces / ghost) so each 4.44s mint is unique NFT art. Receipt `visualConfig.mesh` is the fingerprint. Video LFOs share the crystal-mill genre tempo with the mandatory 4.44s bed. `still` is the Power Plant ident (hard-cut plates over 4.44s at ≥720p), not a frozen poster. No `@0xray/foundry` bump.
