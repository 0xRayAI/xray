...
### ✨ Features
- **Factory-blip Rippel v2** — sharp look (`stampFocusDisc`) + complex tempo/frequency animation on all five viz (`orb` `swirl` `snap` `waves` `spark`). Video LFOs share the crystal-mill genre tempo with the mandatory 4.44s bed. Still stays the Power Plant plate. No `@0xray/foundry` bump.
