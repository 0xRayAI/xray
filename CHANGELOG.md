...
### Fixed
- **A governed proposal teaches.** A route lookup does not write a lesson. When a vote finishes, the cycle grades only the signals that proposal's sessions already named, including the pattern name on a codify proposal. Approve steps those up by a tenth. Reject steps them down by a tenth. Needs-revision, an abstain, a governance error, and a proposal whose sessions named nothing do not write. Not a new skill.
