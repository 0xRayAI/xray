...
### ✨ Features
- **Factory-blip Rippel v2** — two seed-selected looks (`--look focus|cage`): `focus` is the sharp-dogfood solid cyan disc + gold pupil + orbiting satellites; `cage` is the sparse Wu hairline mesh + field accents (stars, grids, gradients, blinkers). Receipt fingerprints `visualConfig.lookKind` + mesh + field. Audio is stereo AAC on the motion grid (same seed, tempo, phase0=0 — downbeat kick + AND hat). Missing stream or inaudible bed FAILs inspect. `still` is the Power Plant ident (hard-cut plates over 4.44s at ≥720p), not a frozen poster. No `@0xray/foundry` bump.
